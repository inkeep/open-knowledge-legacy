...
/**
 * M3 update-listener preload-pattern test.
 *
 ...
 * `bridge.test.ts:19` works around by testing the `createInvoker` factory
 * instead of the preload export.
 *
 * This file exercises the same listener-subscribe / unsubscribe pattern the
 * preload uses for its three new M3 event subscriptions (`onUpdateDownloaded`,
 ...
 * in the bridge, not just the M3 three. Passing this, plus the existing
 ...
 * closure detaches the listener" AC for US-004 without booting a real
 * Electron context.
 */
