...
interface OnboardingTelemetryAttributes {
  ...
  /** Count of `writeProjectAiIntegrations` per-editor `outcome === 'failed'`
   *  results from this flow (silent or dialog path). Defaults to 0 when the
   *  helper didn't run (cancel) or when it ran with no failures. */
  failedCount?: number;
}
