 ...
 *   3. **Extension blocklist** (D-A5) via `EXECUTABLE_BLOCKLIST_EXTENSIONS`
 ...
 *
 * Research anchors:
 * - `reports/electron-os-integration-patterns/` D4 — four-step path-
 *   containment pattern (realpath + prefix check + extension allowlist +
 *   IPC origin validation).
 * - `reports/electron-os-integration-patterns/` D5 Joplin case study — 6-step
 *   gate with extension allowlist is the gold standard; our gate is the
 *   "blocklist" variant that's strictly more permissive but matches the
 *   dispatcher's NG4 accept-all posture.
 */
...
function resolveAndContain(
  ...
  // the symlink-escape window (D4 of electron-os-integration-patterns).
 ...
 * so executable-blocklist doesn't apply (D-A5 scope is `openPath` only).
 */
