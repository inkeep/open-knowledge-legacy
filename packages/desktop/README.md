...
| `ok:shell:record-handoff`  | Append one JSONL line per dispatch to `~/.ok/stats.jsonl`. Local-only telemetry; failures are logged and swallowed so dispatch never depends on telemetry success. Zero network. Named under the `ok:shell:*` namespace to match the `shell.recordHandoff` bridge location.                                                                                                                                                                                                                                        |
