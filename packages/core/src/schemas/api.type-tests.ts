/**
 * Negative type tests for canonical Zod API schemas (FR11 a / D19).
 *
...
// Bare kebab tokens (pre-D38 form) must NOT satisfy ProblemType.
...
// Relative-URI form (pre-D38 alternative) must NOT satisfy ProblemType.
...
// Missing `title` (REQUIRED per D14) must NOT compile.
...
// `type` widened to a free-form string (regression of NG1 closed union) must NOT compile.
...
// (D22 dropped the wrapper). The Zod schema is `.loose()` so the inferred TS
...
// `ok: true`) and by the FR17 allowlist meta-test, not by the inferred type.
...
// ---------------------------------------------------------------------------
// PrincipalResponse / ServerInfoResponse (existing schemas, regression guards)
// ---------------------------------------------------------------------------

const _validPrincipal: PrincipalResponse = {
  id: 'principal-abc',
  display_name: 'Miles',
  display_email: '',
  source: 'git-config',
...
const _invalidSource: PrincipalResponse = {
  id: 'p-1',
  display_name: 'Miles',
  display_email: '',
