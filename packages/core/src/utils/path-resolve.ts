 ...
 *
 * SPEC §6 FR-3b. Path shapes used throughout are contentDir-relative posix
 * strings (e.g. `docs/photo.png`) — leading `./` and `/` are normalized away.
 *
 * D-D LOCKED: plain Map — no TrieMap dependency at our scale; index rebuilds
 * at server startup and on CC1 ch:'files' signals, no disk persistence.
 *
