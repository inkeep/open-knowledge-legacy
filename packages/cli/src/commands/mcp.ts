 ...
 *
 * Lifecycle coupling (FR-1.4 / D-003 / D-009 revised / D-018 revised):
 * - Explicit `--port` override wins over everything; otherwise we try lock-based
 *   discovery against `<contentDir>/.open-knowledge/server.lock`.
 * - If no live server is running AND auto-start is not opted out (env
 *   `OK_MCP_AUTOSTART=0` > config `mcp.autoStart: false`) we detach-spawn
 *   `npx @inkeep/open-knowledge start` as a sibling process. The sibling has no
 *   parent-lifetime dependency on MCP stdio (D-003), so Claude Code's
 *   kill-on-session-end cannot take it down.
 * - Spawn stderr is captured via kernel `stdio[2]` redirect to
 *   `<lockDir>/last-spawn-error.log`; on poll timeout we surface the captured
 *   contents in the first tool-result error instead of silently falling to
 *   disk-only.
 */
  ...
  const cmd = new Command('mcp')
    ...
    .action(async (opts: { port?: string }) => {
      try {
        ...
        const { serverUrl, message } = await ensureServerRunning({
          lockDir,
          contentDir,
