 ...
 *
 * When the optional `dir` is set, also surfaces folder-level metadata
 * computed locally (no extra server roundtrip) per FR4 of spec
 * 2026-05-01-folder-level-metadata-and-templates: `frontmatter_defaults`,
 * `templates_available`, and (for `depth > 1`) recursive `subfolders[]`.
 */
import { z } from 'zod';
...
export function register(server: ServerInstance, deps: ListDocumentsDeps): void {
  server.tool(
    ...
    async (args: { dir?: string; depth?: number; cwd?: string }) => {
      ...
      // FR4: when a `dir` was passed, enrich it locally with folder-level
      // metadata + templates + (optionally) recursive subfolders. Skipped
      // when no dir filter is set — the project root listing case stays
      // structurally compatible with prior callers.
      ...
      if (args.dir) {
        ...
        try {
          ...
          // Folder enrichment is best-effort — surface the doc list either way.
          folder = undefined;
        }
      }
