/**
 * `delete_document` MCP tool — managed page deletion via the server API.
 *
 * Calls POST /api/delete-path with kind: 'file'. Closes all open agent
 * sessions for the doc, unloads it from Hocuspocus, and removes the file
 * from disk. Inbound wiki-links to the deleted doc are NOT rewritten —
 * they become redlinks (consistent with how the file-tree UI delete works).
 */
import { z } from 'zod';
...
interface DeleteDocumentSuccess {
  ok: true;
  /** docNames removed from the index (always a single-entry array for kind: file). */
