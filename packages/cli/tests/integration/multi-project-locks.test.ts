 ...
 *   2. **Cross-process (real A1):** spawn N independent worker processes
 *      (each with its OWN pid) holding their own server.lock + ui.lock.
 *      This is the assumption the spec flagged as "active" (SPEC §12 A1):
 *      "5 concurrent ok mcp spawns across 5 content dirs". It's the only
...
// ---------------------------------------------------------------------------
// Cross-process coverage (the actual A1 verification)
// ---------------------------------------------------------------------------
