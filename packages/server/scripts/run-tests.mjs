...
const scheduleExitAfterSummary = () => {
  ...
  forcedExitTimer = setTimeout(() => {
    console.error('server test runner completed summary; terminating remaining test process group');
    terminateChildGroup();
...
child.on('exit', (code, signal) => {
  ...
  console.error(`server test runner exited via signal ${signal ?? 'unknown'}`);
  process.exit(1);
});
