...
export const DESCRIPTION = [
  ...
  '',
  '**`title` is required** in the frontmatter (D14) — it is the menu surface agents pick from. Missing or empty title returns `TEMPLATE_TITLE_REQUIRED`. `description` is recommended (soft warning when absent) — it disambiguates similarly-named templates.',
  '',
  '**Substitution allowlist (D5 / FR17).** The body MAY contain `{{date}}` (today, ISO-8601) and `{{user}}` (calling principal display name). Any other `{{...}}` token is rejected at write time with `TEMPLATE_UNKNOWN_VARIABLE`. Substitution happens at instantiation time (when `write_document({ template })` materializes the doc), not at template-write time — templates on disk show the raw `{{date}}` token.',
  '',
  '**Parameters:**',
...
const InputSchema = {
  ...
  frontmatter: z
    .object({
      title: z
        .string()
        .min(1, 'Template `title` is required (D14) — it is the menu surface agents pick from.')
        .describe(
    ...
    .describe(
      'Template menu metadata. `title` MUST be present (D14 — hard error if missing). `description` SHOULD be present (warning).',
    ),
