...
export function readOriginGitHubRepo(projectDir: string): OriginResult {
  const gitDir = resolveGitDir(projectDir);
  if (!gitDir) return { kind: 'no-remote' };
  const configPath = join(gitDir, 'config');
  if (!existsSync(configPath)) return { kind: 'no-remote' };
  ...
  if (!originUrl) return { kind: 'no-remote' };
  const parsed = parseGitHubOriginUrl(originUrl);
  if (parsed) return { kind: 'ok', owner: parsed.owner, repo: parsed.repo };
...
export function branchExistsOnOrigin(projectDir: string, branch: string): boolean {
  ...
  const loosePath = join(gitDir, 'refs', 'remotes', 'origin', branch);
