...
export function emitSharePublishLog(
  ...
  const parts: string[] = [`action=${action}`, `result=${result}`];
  if (extras?.count !== undefined) parts.push(`count=${extras.count}`);
  if (extras?.available !== undefined) parts.push(`available=${extras.available}`);
