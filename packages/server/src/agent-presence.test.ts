...
describe('AgentPresenceBroadcaster', () => {
  ...
  test('setPresence opportunistically evicts entries beyond BROADCASTER_EVICTION_MS', () => {
    ...
    // drops the long-stale agent-A entry in the same sweep. See review
    // pass 0 finding #4.
    const now = Date.now();
  ...
  // ──────────────────────────────────────────────────────────────────
  // Handler try/finally contract (regression for review pass 0 #1 fix)
  // ──────────────────────────────────────────────────────────────────
  //
  ...
  // `mode:'idle'`. The review pass 0 fix was structural (moving setPresence
  ...
  // the same `bun test` process (see `standalone.test.ts:437-438`), so a
  ...
  test('contract: touchMode before any setPresence is a no-op (handler finally on pre-setPresence throw)', () => {
    ...
    // already tested above, but re-asserted here in the handler-shape
  ...
  test('structural: every agent write handler pairs setPresence("writing") + touchMode("idle")', () => {
    ...
    // `standalone.test.ts:437-438`; this test reads the handler source and
