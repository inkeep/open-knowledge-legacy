 ...
 * server-observers Observer B, the `bindFrontmatterDoc` form path) call
 * into one set of instruments.
 *
 * The `form` surface used to fire from the `/api/frontmatter-patch` HTTP
 * handler; that endpoint is gone — form writes now flow through the CRDT
 * via `bindFrontmatterDoc` and the originating client is responsible for
 * its own span if it wants one. The bounded enum keeps `'form'` so the
 * Prometheus label dimension stays stable across the migration.
 */
 ...
 * Prometheus index.
 */
type FrontmatterEditSource = 'form' | 'source-mode' | 'mcp-write' | 'mcp-patch' | 'file-watcher';
