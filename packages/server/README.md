...
## CRDT server-restart recovery — instance ID + Yjs binary sidecar

Full rationale + prior-art analysis in [`reports/crdt-server-restart-recovery/REPORT.md`](../../reports/crdt-server-restart-recovery/REPORT.md). Architecture ports Jupyter Real-Time Collaboration's `.jupyter_ystore.db` pattern onto Hocuspocus: markdown stays the source of truth (precedent #1), a binary Yjs sidecar preserves CRDT identity across restart.

### The bug class

A browser tab holds its Y.Doc in memory. The Open Knowledge server restarts. Yjs identifies items by `(clientID, clock)` — content equality is NOT checked; merge is additive union. On reconnect within the ProviderPool's 4s recycle debounce, the client's pre-restart items (under the original clientID) merge with the server's fresh items (under a new clientID generated by `persistence.onLoadDocument`'s `updateYFragment`). Every content marker appears twice on disk after the next L1 flush.

### Two-layer defense

**Layer 1 — Server instance ID** (`serverInstanceId: string`, readonly on `ServerInstance`).
- Generated once per `createServer()` via `randomUUID()`.
- Advertised via `GET /api/server-info` → `{ ok: true, serverInstanceId }`.
- Broadcast via the `__system__` CC1 `server-info` channel at startup (`CC1Broadcaster.emitServerInfo(id)`).
- Zod-typed at the wire boundary: `HocuspocusAuthTokenSchema` (in `auth-token-schema.ts`) ships `.loose()` so unknown fields survive, all fields optional for legacy compat, parsed via `parseHocuspocusAuthToken()`.
- Enforced in `onAuthenticate`: when `parsed.expectedServerInstanceId` is non-empty and doesn't match this server's id, throws with `{ reason: 'server-instance-mismatch' }`. Hocuspocus sends PermissionDenied; client sees `authenticationFailed` with the reason.
- Client-side `ProviderPool.onAuthenticationFailed` (on reason `server-instance-mismatch`) nulls the cached id and calls `recycleAllEntries()` — every pool entry gets a fresh Y.Doc BEFORE Yjs sync can merge ghost state. Idempotent via `cachedServerInstanceId === null` short-circuit.

**Layer 2 — Yjs binary sidecar** (`sidecar.ts`).
- Path: `<contentDir>/.open-knowledge/ystate/<docName>.bin`. Gitignored via `init.ts` scaffolding.
- Format: `[uint32 BE header length][JSON header bytes][Y.encodeStateAsUpdate body]`. Atomic temp+rename write.
- Header Zod schema: `{ yjsVersion, formatVariant: 'v1', schemaVersion, writtenAt, clientIdToWriter? }`.
- `writeSidecar(contentDir, docName, doc)` runs in `onStoreDocument` AFTER the markdown atomic rename — sidecar-write failure emits `sidecar-write-failed` warn but does NOT fail the L1 cycle.
- `readSidecar(contentDir, docName)` validates the header (Yjs MAJOR match, formatVariant v1, schemaVersion ≤ current) then returns `{header, bodyBytes, applyFn}`. `applyFn` wraps `Y.applyUpdate` in a 1s `Promise.race` timeout — defends against Yjs #479 infinite-loop cases.
- `onLoadDocument` prefers the sidecar when present + content converges with disk (modulo whitespace via `normalizeBridge`). Strategy A divergence handling: mismatch → `deleteSidecar` + `sidecar-divergent-reload` warn + markdown fallback. Corrupt body or timeout → `deleteSidecar` + `sidecar-load-failed` warn + markdown fallback. Happy path: `Y.applyUpdate(liveDoc, bodyBytes)` preserving clientIDs intact.
- `deleteSidecarsForBranch(contentDir)` runs in `onBatchBegin` before `parkBranch` — sidecars are branch-scoped; a switch wipes the directory. Fresh sidecars regenerate on the first post-switch L1 debounce (Design A, not per-branch dirs).

### Composition with existing primitives

- **Markdown stays canonical.** The sidecar never writes on its own — it rides on the L1 pipeline's markdown write. Any sidecar failure degrades to markdown-only.
- **reconciledBase** (the three-way merge base) is unchanged. Sidecar load updates it the same way markdown load does.
- **parkBranch / restoreBranchWIP** are unchanged — sidecars are content cache, not parked WIP state.
- **server-lock / server-info** share the `__system__` CC1 channel; `serverInstanceId` emission happens after `__system__` doc materializes (see `CC1Broadcaster.emitServerInfo`).

### Test coverage

- Unit: `sidecar.test.ts` (22 tests — round-trip, header validation, corruption, version skew, delete helpers).
- Integration: `persistence.test.ts::onLoadDocument — sidecar load paths` (4 tests — happy / missing / corrupt / divergent).
- Server-side auth: `standalone.test.ts::onAuthenticate rejects 'server-instance-mismatch'` (5 tests — mismatch / match / legacy / missing / empty-string claim).
- Client-side pool: `provider-pool.test.ts::ProviderPool authenticationFailed handling` (4 tests — recycle on mismatch / cache null after / other reasons no-op / idempotent on second event).
- End-to-end: 11-test bug-class suite under `packages/app/tests/integration/` — T1-T11 cover fast restart, multi-client restart, slow restart, unsynced local edits, branch switch, agent write during restart, rollback, managed rename, external disk edit, Y.Text source-mode, mid-drain restart. All 11 PASS.

### Deferred (out of scope for v1)

- Buffer-and-replay for edits that never reached the server before kill (client-side queue). Happy path (edit synced, L1 fired) preserves across restart; degraded path (edit never reached server) loses the edit. Rare intersection per the research.
...
- Per-branch sidecar directories (Design B from the research). Design A (delete-on-switch) is simpler and matches T5's semantics.
- Sidecar integration into save-version named checkpoints. Ephemeral clientIDs have no value across named restores.
