...
test.describe('FR-7a: source-mode toggle disabled during disconnect', () => {
  ...
  test('disconnected state: source toggle becomes disabled', async ({ page }) => {
    ...
    // Visual toggle remains enabled (the spec only disables source during disconnect).
    await expect(visualToggle(page)).toBeEnabled();
  });
  ...
  test('disconnected state: tooltip text matches spec', async ({ page }) => {
    // FR-7a mandates the tooltip text: "Source mode requires a live
    ...
    // reliably, so the implementation wraps it in a <span tabIndex={0}>
    // which the TooltipTrigger asChild pattern uses as the hover target
    // (see EditorHeader.tsx lines 94-107). Hovering the parent span
    // fires the tooltip. Using .locator('..') walks up one DOM level
    // from the button to the wrapping span.
