/**
 * QA-001 / QA-002 / QA-003 / QA-041 — Browser-fidelity verification for the
 ...
 *
 *   QA-002 — file rename via /api/rename-path { kind:'file' } updates the
 *            FileTree and rewrites inbound wiki-links across siblings.
 *   QA-001 — folder rename via /api/rename-path { kind:'folder' } updates
 *            the FileTree and rewrites cross-folder backlinks.
 *   QA-003 — rollback writes a contributor entry; timeline UI renders the
 *            principal display name.
 *   QA-041 — timeline panel for a renamed doc shows the rename: subject and
...
test.describe('rename-consolidation — browser-fidelity outcomes', () => {
  ...
  test('QA-003 / QA-041: principal-driven rename → /api/history endpoint reachable + reports rename', async ({
    ...
    // Browser-fidelity slice for QA-003 + QA-041. The principal-attribution
    // semantics + writerId='principal-<uuid>' shape are proven end-to-end
    // against real shadow git in api-rename-rollback-summary.test.ts and
    // api-rollback-actor-identity.test.ts. This test verifies the browser
    ...
    // server-side by api-rename-rollback-summary.test.ts (real shadow git +
