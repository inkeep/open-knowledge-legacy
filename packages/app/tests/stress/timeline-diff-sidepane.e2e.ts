...
// FIXME(test-infra): the per-worker fixture in `_helpers/fixtures.ts` does
// not initialize a git/shadow repo for the Timeline history pipeline, so
// /api/history returns no entries and these scenarios cannot land entries
// in the Timeline panel. Code-level verification of the underlying behavior
// (data-testid wiring, hover-destructive class, `Restore` semantics, lazy
// import, AbortController, ConflictResolver retention) is recorded in
// tmp/ship/qa-progress.json with shell-level evidence. Unblock by extending
// the fixture to git-init the contentDir + ensure projectRoot detection
// finds it (today the dev server appears to walk past the tmpdir).
test.describe
  .fixme('Timeline inline diff — side pane', () => {
    // ── AC-D1 ──────────────────────────────────────────────────────────────────
