/**
 * Per-handler narrow-integration smoke test for `handleAgentWriteMd` (FR10 / D16, US-006).
 *
...
describe('agent-write-md envelope (RFC 9457)', () => {
  ...
  test('empty markdown string fails schema (min(1))', async () => {
    const res = await postWriteMd({ markdown: '', position: 'append' });
    expect(res.status).toBe(400);
    const body = await res.json();
    const parsed = ProblemDetailsSchema.safeParse(body);
    expect(parsed.success).toBe(true);
