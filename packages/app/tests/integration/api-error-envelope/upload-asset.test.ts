/**
 * Per-handler narrow-integration smoke test for `handleUploadAsset` (FR10 / D16).
 *
...
describe('upload-asset envelope (RFC 9457)', () => {
  test('happy path emits flat success body with application/json', async () => {
    ...
    // Wire shape post-D22: no `ok: true` discriminator on success bodies.
  ...
  test('missing parentDocName emits ProblemDetails with application/problem+json', async () => {
    ...
    if (parsed.success) {
      ...
      // RFC 9457 instance is a UUID per D13; same UUID is mirrored in the
      ...
      // body.status MUST equal HTTP response status (D22).
