 ...
 * (FR10 / D16, US-013). Asserts the canonical RFC 9457 wire shape for
 * `GET /api/sync/status`:
...
import { ProblemDetailsSchema, SyncStatusSuccessSchema } from '@inkeep/open-knowledge-core';
...
describe('sync-status envelope (RFC 9457)', () => {
  test('happy path emits flat dormant status with application/json', async () => {
    ...
    const parsed = SyncStatusSuccessSchema.safeParse(body);
