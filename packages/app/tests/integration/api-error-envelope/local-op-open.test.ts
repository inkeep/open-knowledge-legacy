/**
 * Per-handler narrow-integration smoke test for `handleLocalOpOpen` (FR10 / D16, US-012).
 *
 * Covers pre-validation 400s (malformed JSON, missing dir, dir outside home)
 * and 405 method-not-allowed with `Allow: POST`. The 504 server-open-failed
 * happy-path / sad-path requires a real CLI subprocess + 45s polling deadline
 * — covered by host-network tests, not the per-handler smoke.
 */
