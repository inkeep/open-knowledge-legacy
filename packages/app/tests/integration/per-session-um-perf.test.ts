/**
 * US-029: Per-session UndoManager performance gate (NFR-7).
 *
...
describe('NFR-7: per-session UndoManager proportional cost', () => {
  test('N=10 sessions × 100 transacts completes within proportional bounds vs single-session baseline', async () => {
    ...
    // NFR-7a: time must not grow super-linearly (< 20× baseline for 10× sessions)
    ...
    // NFR-7b: heap delta must not grow super-linearly (<10× baseline for 10× sessions).
    ...
    // NFR-7c: absolute wall-clock ceiling — 10 sessions × 100 transacts must finish in < 30s
    expect(load.durationMs).toBeLessThan(30_000);

    // NFR-7d: absolute heap ceiling — < 200 MB for 10 sessions (not 10× 200 MB)
