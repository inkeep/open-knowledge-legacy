 ...
 * (whether via Mutation F removing `OBSERVER_SYNC_ORIGIN.skipStoreHooks`,
 ...
 *
 * Scope note — Mutation F coverage:
 *   The single-agent-write path is fully early-exit-protected by the
 *   already-in-sync origin guard, so Mutation F (dropping skipStoreHooks)
 *   does not manifest in this specific scenario — the observer never
 *   writes. The genuine Mutation F blast-radius is a concurrent-writer
 *   scenario (e.g., C1 concurrent WYSIWYG + race-initiated observer
 *   write) where the observer DOES write and skipStoreHooks is what
 *   prevents the cascade. Those scenarios are covered by C1-C5.
 *   This gate catches the broader class: "any disk-write amplification
 *   introduced by a future regression." A Mutation-F-specific test that
 *   provokes a non-early-exit observer write is meta/mutation-validation.md's
 *   job, not this file's.
 *
...
describe('Mutation F gate: OBSERVER_SYNC_ORIGIN.skipStoreHooks=true prevents disk-write amplification', () => {
  test('single agent-write produces exactly 1 persistence disk write (no observer amplification)', async () => {
    ...
    try {
      ...
      //
      // Under Mutation F (skipStoreHooks:false), any server observer that
      ...
      // Mutation F gate). 3 fires/direction catches any origin-guard
