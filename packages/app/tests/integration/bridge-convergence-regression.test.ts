/**
 * PRIMARY regression gate for bridge-convergence-under-concurrent-writes spec.
 *
 * 4 tests encoding Bug-A + Bug-B pre-fix failures — all must pass post-fix.
 *
 *   P0: human WYSIWYG typing + agent MCP write on same line (Bug-A)
 *   P0-stress: rapid interleaved user + agent writes (Bug-A stress)
 *   P1: human WYSIWYG typing + file-watcher disk update (unaffected path)
 *   CONTROL: peer+peer XmlFragment edits (Bug-B)
 *
 * @see specs/2026-04-14-bridge-convergence-under-concurrent-writes/SPEC.md
 */
