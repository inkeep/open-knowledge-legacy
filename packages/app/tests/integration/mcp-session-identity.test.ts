/**
 * Integration test for US-008 / D-10 — per-MCP-session identity from
 ...
 * Code'` against one running `ok start`. The end-to-end assertion (R-4
 * mitigation) is that downstream surfaces — agent-presence broadcaster +
 * activity-log Y.Map — see two distinct AgentIdentity values flow through:
 *
 *   - identical `displayName` (`'Claude Code'`)
...
test('two simultaneous Claude Code MCP sessions land with identical displayName but distinct connectionIds', async () => {
  // Two MCP sessions, both reporting clientInfo.name === 'Claude Code'. The
  // server should derive identity per-session from `clientInfo.name` +
  // `connectionId` (US-008). A regression to AGENT_LABEL or any other shared
  ...
  try {
    ...
    //    presence entries, both with `displayName === 'Claude Code'`.
