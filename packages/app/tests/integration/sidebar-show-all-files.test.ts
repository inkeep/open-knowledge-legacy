...
describe('/api/documents?showAll=true', () => {
  ...
  test('?showAll=true surfaces .gitignored, .okignored, and BUILTIN_SKIP_DIRS content', async () => {
    ...
    expect(docNames).toContain('node_modules/pkg/README');
  });
  ...
  test('?showAll=true surfaces non-md / non-asset files as kind=asset', async () => {
    ...
    expect(pkgJson?.mediaKind).toBe(null);
  ...
  test('?showAll=true emits folder entries for every walkable directory (including BUILTIN_SKIP_DIRS)', async () => {
    const res = await fetch(`http://localhost:${server.port}/api/documents?showAll=true`);
    expect(res.ok).toBe(true);
    const body = DocumentListSuccessSchema.parse(await res.json());
    ...
    expect(folderPaths).toContain('node_modules');
    expect(folderPaths).toContain('node_modules/pkg');
    expect(folderPaths).toContain('.git');
    expect(folderPaths).toContain('.ok');
  });
