/**
 * SH01 + SH05 regression coverage — pre-widening `jsxComponent` (atom=true
 ...
 * safely through `@tiptap/y-tiptap`'s `initProseMirrorDoc` under the R13
 * patch.
 *
 ...
 * drives generic unknown-nodeName / unknown-mark throws to prove the R13
 ...
 * document) would replay — exercising the exact blast radius that
 * `specs/2026-04-14-component-blocks-v2/` SH01 and SH05 specify.
 */
...
describe('SH01: pre-widening jsxComponent (atom=true, raw-content-in-attrs) materialization', () => {
  test('pre-widening shape — legacy raw-content attrs materialize without tombstoning Y.Items', () => {
    ...
    // pre-widening shape but not in the new one. The R10 snapshot test
