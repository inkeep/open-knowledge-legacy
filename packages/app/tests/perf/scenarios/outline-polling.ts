/**
 * S4 reproduction — OutlinePanel polls `/api/page-headings` every 2s while
 ...
 * idle window.
 *
 * Pre-fix baseline (AC15): apiCallCount ≥ 10 across 30s idle.
 * Post-fix target (AC19):  apiCallCount === 0 across 30s idle (invalidation
 * moves to HocuspocusProvider `update` event per D9 LOCKED).
 *
