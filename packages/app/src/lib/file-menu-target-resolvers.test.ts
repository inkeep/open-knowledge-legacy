...
describe('resolveActiveTargetAbsPath', () => {
  ...
  test('asset / missing scopes fall back to contentDir', () => {
    expect(
      resolveActiveTargetAbsPath(
        { kind: 'asset', target: 'foo.png', assetPath: 'foo.png', mediaKind: null },
        null,
        WORKSPACE,
      ),
...
describe('resolveActiveTargetParentDirAbsPath', () => {
  ...
  test('null + asset + missing scopes return contentDir', () => {
...
describe('resolveActiveTargetRelativePath', () => {
  ...
  test('null / asset / missing scopes return empty string (contentDir convention)', () => {
...
describe('buildSendToAiInputForActiveTarget', () => {
  ...
  test('folder scope returns folder-scoped handoff input with abs path + relative path', () => {
    ...
    expect(input?.projectDir).toBe('/Users/test/project/reports');
  });
