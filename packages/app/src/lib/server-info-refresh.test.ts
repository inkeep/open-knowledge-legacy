 ...
 * the late-join recovery contract that T15 verifies end-to-end.
 *
 * Per Round 11 reviewer feedback: T15's manual `refreshServerInfo`
 * call structurally can't exercise the gate (dispose + recreate
 * isn't a WebSocket reconnect, so the gate's same-provider-lifetime
 * contract is never observed). These targeted unit tests close the
 * coverage gap by testing the gate directly.
 */
