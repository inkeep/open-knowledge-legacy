...
function OkignoreSectionBody({ binding }: { binding: OkignoreBinding }) {
  ...
  useEffect(() => {
    return subscribeToConfigIgnoreNestedError((event) => {
      toast.error(`Nested .okignore error in ${event.path}`, {
...
function OkignoreAdvancedEditor({ binding, text }: { binding: OkignoreBinding; text: string }) {
  ...
  return (
    <textarea
      ...
      aria-label="Ignore patterns (raw text)"
      placeholder={
...
function RejectionBanner({ rejection }: { rejection: RejectionState }) {
  const lineSuffix = rejection.lineNumber ? ` (line ${rejection.lineNumber})` : '';
  return (
    <div
      role="status"
      aria-live="polite"
    ...
    >
      ...
      <span>
        Pattern syntax error{lineSuffix}: {rejection.message}
      </span>
    </div>
  );
}
...
function OkignoreEmptyState({
  ...
  return (
    ...
    >
      <p className="text-sm text-muted-foreground">
        No patterns yet. Type a folder or file name below to start hiding files.
      </p>
      <AddPatternRow
        onAdd={onAdd}
    ...
    >
      <div className="flex items-center gap-1">
        <Button
          ...
          aria-label={`Drag ${pattern.text} to reorder`}
          {...attributes}
          {...listeners}
        ...
        <Button
          ...
          aria-label={`Remove ${pattern.text}`}
    ...
    <div className="flex min-w-0 flex-1 flex-col">
      <div className="flex min-w-0 items-center gap-1">
        <Input
          ...
          aria-label={`Pattern ${patternIndex + 1}`}
          className={cn(
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          role="status"
          aria-live="polite"
          aria-label={`Pattern warnings: ${summary}`}
...
function PatternPreview({ count }: { count: number | null }) {
  ...
  return (
    ...
    >
      matches {count} {count === 1 ? 'file' : 'files'}
      {isZero ? (
        <span className="ml-1 text-muted-foreground/60">
          (some may already be hidden by other rules)
        </span>
      ) : null}
    </span>
  );
