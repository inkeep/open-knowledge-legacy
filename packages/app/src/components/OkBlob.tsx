 ...
 *
 * Click reaction escalates with rapid clicks: a single click gives the baseline
 * ^^ eyes + bounce; 2 rapid clicks (within ~600ms) squint tighter + bounce
 * harder; 3+ rapid clicks trigger a monochrome sparkle burst. Level decays back
...
export function OkBlob({ size = 48, className, trackMouse = true }: OkBlobProps) {
  ...
  function handleClick() {
    ...
    setClickLevel(nextClickLevel(clickLevel, dt));
  ...
  useEffect(() => {
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        let oy = 0;
        if (dist >= 1) {
          const scale = Math.min(dist / 150, 1) * MAX_EYE_OFFSET;
          ox = (dx / dist) * scale;
          oy = (dy / dist) * scale;
        }

        leftEyeRef.current?.setAttribute('cx', String(LEFT_EYE_CX + ox));
        leftEyeRef.current?.setAttribute('cy', String(EYE_CY + oy));
        rightEyeRef.current?.setAttribute('cx', String(RIGHT_EYE_CX + ox));
        rightEyeRef.current?.setAttribute('cy', String(EYE_CY + oy));
      });
    }
  ...
  return (
    <svg
      ...
      onClick={handleClick}
    >
      {/* Blob body — key re-mounts to replay the bounce on every click */}
      <path
        key={`body-${clickSeq}`}
        ...
        className={cn('ok-blob-body', bounceClass)}
      />

      {/* Normal eyes — vertical ellipses, hidden when clicked */}
      <ellipse
        ref={leftEyeRef}
        cx={LEFT_EYE_CX}
        cy={EYE_CY}
        rx={1.2722}
        ry={1.9083}
        className={cn('ok-blob-eye', isClicked && 'ok-blob-eye-hidden')}
      />
      <ellipse
        ref={rightEyeRef}
        cx={RIGHT_EYE_CX}
        ...
        rx={1.2722}
        ry={1.9083}
        className={cn('ok-blob-eye ok-blob-eye-right', isClicked && 'ok-blob-eye-hidden')}
      />
      ...
      <path
        ...
        strokeLinecap="round"
        fill="none"
        className={cn('ok-blob-happy-eye', !isClicked && 'ok-blob-eye-hidden')}
      />
      <path
      ...
      {/* Sparkle burst at level 3 — three monochrome dots popping around the blob */}
      {clickLevel === 3 && (
        <g key={`sparkles-${clickSeq}`}>
          <circle cx={15} cy={-2} r={0.7} className="ok-blob-sparkle ok-blob-sparkle-0" />
          <circle cx={-2} cy={22} r={0.7} className="ok-blob-sparkle ok-blob-sparkle-1" />
          <circle cx={32} cy={22} r={0.7} className="ok-blob-sparkle ok-blob-sparkle-2" />
        </g>
      )}
    </svg>
  );
