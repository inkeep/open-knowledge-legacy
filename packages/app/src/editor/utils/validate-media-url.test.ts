...
describe('validateMediaUrl — embed-provider rejection (Vimeo / Loom + image / audio)', () => {
  test('rejects Vimeo URLs for video kind', () => {
    ...
    expect(validateMediaUrl('https://player.vimeo.com/video/123456789', 'video')).toEqual({
      valid: false,
      reason: 'embed-provider',
      provider: 'vimeo',
    });
    expect(validateMediaUrl('https://www.vimeo.com/123', 'video')).toEqual({
      valid: false,
      reason: 'embed-provider',
      provider: 'vimeo',
    });
  ...
  test('rejects Loom URLs for video kind', () => {
    ...
    expect(validateMediaUrl('https://loom.com/share/abc', 'video')).toEqual({
      valid: false,
      reason: 'embed-provider',
      provider: 'loom',
    });
    expect(validateMediaUrl('https://www.loom.com/embed/abc', 'video')).toEqual({
      valid: false,
      reason: 'embed-provider',
      provider: 'loom',
    });
