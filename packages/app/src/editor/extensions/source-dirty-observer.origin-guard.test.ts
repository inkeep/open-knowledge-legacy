 ...
 *
 * Precedent #1 (typed transaction origins) exists because Bug-A/B/D shipped
 ...
 *      pristine (the NG12 reconstruction path applies per-node, so any
...
describe('SourceDirtyObserver origin guard', () => {
  ...
  test('CRDT-origin transaction with ySyncPluginKey meta does NOT mark dirty', () => {
    ...
    // reconstruction on save (NG12) for content the local user never
