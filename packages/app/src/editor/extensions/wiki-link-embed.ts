 ...
 *   Electron-vs-web branching consumed by `handlePrimary`.
 */
import {
...
// attribute used by InteractionLayer's event delegation. Mirrors
// `wiki-link.ts` line 32 + `RawMdxFallback`'s `nextRawMdxNodeId`.
...
export const WikiLinkEmbed = BaseWikiLinkEmbed.extend({
  addNodeView() {
    return ({ editor, node, getPos }) => {
      ...
      // `handlePrimary` routes to `dispatchAssetClick` for both bare click
      // (web opens new tab, Electron calls `shell.openAsset`) and
      // Cmd/Ctrl+click (forces OS delegation via `forceOsDelegation`).
      ...
      layer.register({
        ...
        // to the dispatcher (web: new tab; Electron: OS default app).
        // A prop-panel would be a follow-up if users need to edit the
        // target or alias from the editor without going to source mode.
        controls: {},
