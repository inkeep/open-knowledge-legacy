 ...
 * lifecycle to an InteractionLayerHandle so mark chip extensions (US-005
 * InternalLink) can route per-mark interactions through the shared editor-root
 * React plane.
 *
 * Sits between two already-shipped primitives:
 *   - `markIdentityPlugin` (US-004) assigns stable IDs to PM marks and fires
 *     register/deregister callbacks on mark lifecycle via its view update.
 *   - `InteractionLayer` (US-003) hosts the singleton PropPanel/Toolbar/
 ...
 *
 * Consumer pattern (targeted by US-005 `internal-link.ts` port):
 *
 ...
 *
 * No consumers wired in this module today — ships as US-005 scope-reduction
 * (iter-20 playbook): concentrates the wiring pattern + correctness handling
 * in one tested place so the eventual US-005 atomic refactor is smaller.
 *
