 ...
 * InteractionLayer (US-003) needs a stable string id to key `data-mark-id`
 * chip attributes and to fire register/deregister on mark lifecycle.
 *
 * Solution (FR4b, V2 SPEC §9.2 / Audit §B4): a PM plugin maintains
 * PluginState<{ byId: Map<id, MarkInfo>, counter: number }>. On every
 * `docChanged` transaction, it carries previous IDs forward via
 * `tr.mapping` and walks the new doc to produce the updated map:
 *
 *   - previously-known range that remains in the doc → keep its ID
 ...
 * Precedent #9 add-only schema preserved (cross-ref: CB-v2 §9.15 Q10
 * Option A, bridgeId stored in PluginState rather than schema attr).
 *
 ...
 * into InteractionLayer (US-005/006).
 */
