 ...
 *
 * When a paste goes through the rich-HTML branch (Branch D) and any
 * pipeline stage throws, the dispatcher falls through to a lower-
 * fidelity path (Branch E / PM default / plain text). Telemetry captures
 ...
 * `sonner` before the helper runs (see `paste-failure-toast.test.ts`).
 */
 ...
 *
 * Default `message` matches the soft-notice UX from the spec: the paste
