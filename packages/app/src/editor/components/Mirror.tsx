...
export function Mirror(props: MirrorProps) {
  ...
  if (status.kind === 'loading') {
    return (
      <StatusFrame tone="placeholder">
        Mirror loading <code>{src && anchor ? `${src}#${anchor}` : ''}</code>
      </StatusFrame>
    );
  }
  ...
  if (status.kind === 'source-removed') {
    return (
      <StatusFrame tone="error">
        <span className="font-medium">Mirror source removed.</span> The doc <code>{src}</code> is no
        longer available.
      </StatusFrame>
    );
  }
  ...
  if (status.kind === 'anchor-not-found') {
    return (
      <StatusFrame tone="error">
        ...
        <code>{'<MirrorSource id="'}</code>
        <code>{anchor}</code>
        <code>{'">'}</code> exists in <code>{src}</code>.
      </StatusFrame>
    );
  }
