...
export function Pdf(props: PdfProps) {
  ...
  }, [pages, layoutMode, zoomScale, containerWidth, containerHeight]);
  ...
  return (
    <div className="ok-pdf" style={{ height: `${heightPx}px` }}>
      <div className="ok-pdf-toolbar" contentEditable={false}>
        ...
        {totalPages > 0 && (
          <div className="ok-pdf-controls">
            ...
            <span className="ok-pdf-page-of">of {totalPages}</span>
      ...
      <div className="ok-pdf-body">
        {showThumbs && (
          <aside className="ok-pdf-sidebar" aria-label="Page thumbnails">
            {pages.map((info, i) => (
              <button
                type="button"
                key={info.pageNumber}
                className="ok-pdf-thumb"
                data-active={currentPage === info.pageNumber || undefined}
                onClick={() => goToPage(info.pageNumber)}
                aria-label={`Jump to page ${info.pageNumber}`}
              >
                <canvas
                  ref={(el) => {
                ...
                />
                <span className="ok-pdf-thumb-num">{info.pageNumber}</span>
              </button>
            ))}
          </aside>
        )}
