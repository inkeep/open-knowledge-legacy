...
**Hard cutover, no migrator** (pre-release license to break):
...
**For pre-existing OK projects:** rename your `.open-knowledge/` directory to `.ok/`, lift any `content.include`/`content.exclude` patterns into a project-root `.okignore`, delete the orphan `.git/open-knowledge/` shadow repo, and re-authenticate. The dogfood team will see one re-prompt for stored credentials (`~/.ok/auth.yml`), first-launch consent (`~/.ok/mcp-status.json`), and skill-installed marker on first run after merging — expected behavior for the hard cutover.
