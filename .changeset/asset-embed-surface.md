...
- `POST /api/upload` — upload endpoint. Success response: `{ ok, src, path, deduped }` where `src` is the asset's basename and `path` is the contentDir-relative location (reflects `upload.attachmentFolderPath`). 413 response shape: `{ ok: false, error: 'max-bytes', message, attemptedBytes, maxBytes }`.
