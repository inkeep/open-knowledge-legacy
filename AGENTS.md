...
- **Yjs binary sidecar is disposable — must never fail the L1 cycle.** `writeSidecar` / `deleteSidecar` / `deleteSidecarsForBranch` failures emit structured warn events (`sidecar-write-failed`, `sidecar-load-failed`, `sidecar-divergent-reload`, `sidecar-branch-delete-failed`) but MUST NOT throw past their wrapping `try/catch`. Markdown is source of truth (precedent #1); sidecar is recovery cache. On load, a divergent sidecar is deleted and the flow falls through to markdown. Reference: [`packages/server/README.md`](packages/server/README.md) §"CRDT server-restart recovery". Research: [`reports/crdt-server-restart-recovery/REPORT.md`](reports/crdt-server-restart-recovery/REPORT.md).
