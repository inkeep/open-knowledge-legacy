...
- **Paired-write origins MUST declare `context.paired: true`.** Any new origin that atomically mutates BOTH Y.XmlFragment and Y.Text in a single `doc.transact(..., ORIGIN)` opts in via the typed marker (precedent #1 extension). `isPairedWriteOrigin(origin)` is a structural check. Omitting re-surfaces the observer-amplification class.
- **Don't add Y.js observers inside an `<Activity>` subtree without `ACTIVITY_MOUNT_LIMIT`-style bounding.** Y.js observers are NOT React effects and do not pause when Activity flips to `hidden` — a hidden entry with a live provider still processes every remote-peer update. Wire per-document observers off the bounded pool (precedent #18(c); ref `EditorActivityPool.computeActivityMountList`).
- **Don't collapse the hybrid render tree** (`DocumentErrorBoundary` → `Suspense` → `EditorActivityPool` → `Activity` → `DocumentBoundary`) to `<Editor key={activeDocName} />`. The hybrid is load-bearing for flash-free UX (precedent #18(b)). New write surfaces wrap in their own `DocumentBoundary`. Canonical: `packages/app/src/components/EditorArea.tsx`.
