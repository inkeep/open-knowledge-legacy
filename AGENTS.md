...
- **Don't collapse the hybrid render tree** (`DocumentErrorBoundary` → `Suspense` → `EditorActivityPool` → `Activity` → `DocumentBoundary`) to `<Editor key={activeDocName} />` — load-bearing for flash-free UX (precedent #18(b)). Canonical: `packages/app/src/components/EditorArea.tsx`.
