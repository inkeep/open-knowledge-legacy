...
- `src/registry/` — Component descriptor registry subsystem (single source of truth for descriptors; `built-ins.ts` is the hand-authored manifest, `createRegistry()` returns the runtime `ComponentRegistry` with wildcard `'*'` fallback)
  ...
  - `built-ins.ts` — Manifest of **5 built-in component descriptors** — the 5-pack foundation (Callout, Image, Video, Audio, Accordion), each with a researched superset prop shape and a DIY React renderer using OK's brand (shadcn/Tailwind); zero fumadocs-ui React imports. Shapes: Callout 7 props (GFM 5-type enum + title/icon/color/collapsible/defaultOpen per D-MF17), Image 8 props (src/alt/width/height/caption/title/loading/zoom per FR-2; renamed from ImageZoom in US-003/FR-20), Video 9 props (pure HTML5 `<video>` per D-MF12 — no URL sniffing, no iframe emission), Audio 7 props (HTML5 `<audio>`; controls always on), Accordion 6 props (standalone per D-MF14/D-MF16; HTML5 `<details>` substrate; cross-browser exclusive grouping via HTML5 `<details name>`). Mermaid was removed 2026-04-21 (placeholder stub was non-functional); existing `<Mermaid />` content and names outside the 5-pack (`<Tabs>`, `<Cards>`, `<Steps>`, `<Banner>`, `<Files>`, `<TypeTable>`, `<InlineTOC>`, etc.) auto-convert to `rawMdxFallback` via the wildcard path. Un-defer frameworks: `specs/2026-04-14-component-blocks-v2/evidence/mermaid-audio-rendering-deferred.md` for Mermaid; `specs/2026-04-23-cb-v2-md-foundation/SPEC.md` §15 NG19/NG20 for compound-tier (Tabs/Tab, Accordions-group, Steps/Step) and other deferred descriptors. Cut components' preserved design lives on PR #165 branch at commit `e56f33c3`.
