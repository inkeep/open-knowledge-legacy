...
**E2E perf baselines.** Playwright perf assertions (currently QA-022 `paste-fidelity.e2e.ts`) read `packages/app/tests/stress/perf-baseline.json`. The assertion shape is `p50 < max(2 × p50Baseline, absolute-floor)` — 2× the median locks in regression signal, the absolute floor (e.g. 32ms for 60fps frame-time) absorbs CI runner-speed variance without tripping. Baselines are captured from the **median-of-5 p50** across consecutive post-merge CI runs (never local), are append-only with git blame trail, and updates require user approval per the protocol in `packages/app/tests/stress/perf-baseline-update.md`. Follow the same shape for any future Playwright perf test — add a new top-level key (`qaXXX` for QA-numbered tests, or a descriptive key like `v2gN<GoalName>` for V2 sprint gates) and extend the assertion; do not invent a second baseline file. Current V2 sprint keys (p50 TBD — populated post-V2-integration by median-of-5 prod CI runs): `v2g1WarmSwitch` (G1 < 200 ms), `v2g2ColdPoolWarm` (G2 < 500 ms), `v2g3ColdLoad` (G3 < 1500 ms), `v2g4ModeToggle` (G4 < 200 ms), `v2g5PerceivedFirstPaint` (G5 < 500 ms). The `absoluteFloorMs` on each key is the spec's G-target; when `p50Ms` is null the assertion uses the floor alone until the baseline is captured.
...
18. **Hybrid Activity + Suspense + `use(promise)` for subscription-source async primitives.** Five sub-principles, narrow scope:
    ...
    (g) **CodeMirror 6 caching via reparent-without-destroy.** Module-level `Map<docName, {view, ydoc, ytext, provider}>` with `parkEditor()` detaching `view.dom` to a parking node (NOT calling `view.destroy()`) and `mountEditor()` re-attaching + calling `view.focus()` + optional `view.scrollSnapshot()` dispatch. Only `evictEditor()` on LRU eviction calls `view.destroy()`. Observers (Mutation/Resize/Intersection/EditContext) survive within-Document reparent per W3C specs. y-codemirror.next's 3 plugins have zero DOM coupling. Cross-Document (iframe/ShadowRoot) requires `EditorView.setRoot()`; within-Document does not. Empirical: 12/12 probe tests. Full contract at `specs/2026-04-20-perf-v2-editor-cache-and-cold-load-ux/evidence/cm6-reparent-contract.md`.
    (h) **TipTap caching via `Editor.mount/unmount` (or raw `editor.view.dom` reparent fallback).** Symmetric to (g) but with TipTap's named API surface. Module-level `Map<docName, Editor>` with `parkEditor()` calling `editor.unmount()` (or detaching `editor.view.dom` to a parking node) and `mountEditor()` calling `editor.mount(container)` (or re-attaching `editor.view.dom`) + restoring focus. Only `evictEditor()` on LRU eviction calls `editor.destroy()`. Defeats `useEditor.scheduleDestroy(1ms)` — the cache holds the canonical Editor instance, the React binding becomes a thin attach/detach wrapper. Pre-V2 `useEditor` is forbidden in cached surfaces. Reference: `packages/app/src/editor/editor-cache.ts`. Forward-compat with CB-v2 `JsxComponentView` per §F8 of the V2 spec.
    (i) **Provider connect/disconnect cap on cached-but-Activity-hidden editors.** Editors held in the V2 module-level cache that are NOT in the `ACTIVITY_MOUNT_LIMIT = 3` MRU mount list disconnect their `HocuspocusProvider` (`provider.disconnect()`) so the WebSocket stops accepting peer CRDT updates. Local Y.js observers still fire on local mutations, but remote-peer load is bounded to the `ACTIVITY_MOUNT_LIMIT` actively-observed editors regardless of cache size (preserves precedent #18(c) CPU bounds even with V2 cache). On promotion from hidden to visible, `provider.connect()` re-establishes WebSocket + sync; awareness re-broadcasts local presence. Trade-off: hidden-doc collaborator typing is not seen in real-time while doc is cached-but-hidden. Acceptable per Activity contract ("return to doc, pick up state"). Reference: `packages/app/src/editor/editor-cache.ts`.
    Applies to any future subscription-source async surface — agent status panels, CRDT-snapshot loading, WebSocket handshake gates, graph-layout computations that resolve from a streaming source. Does NOT apply to HTTP-fetch panels (BacklinksPanel, OutlinePanel, GraphPanel, ForwardLinksPanel, TimelinePanel) which stay on TanStack Query. See `specs/2026-04-16-page-render-optimization/SPEC.md` §9 (system design), §10 D1 (hybrid), §10 D2 (hand-rolled vs TanStack), §10 DX9 (ACTIVITY_MOUNT_LIMIT decoupled from MAX_POOL), §F14 (precedent scope). For editor caching across nav (sub-rules (g)-(i)), see `specs/2026-04-20-perf-v2-editor-cache-and-cold-load-ux/`.
...
25. **V2 editor cache + InteractionLayer + Option E split walker (2026-04-20).** Seven coordinated primitives make cold-load + warm-switch + per-instance React portal cost bounded for large documents. Sub-rules (a)-(c) are the load-bearing primitives; (d)-(g) are the supporting mechanisms that wire the primitives to the existing chip / mode-toggle / pre-warm surfaces:
    ...
    - `mark-identity-plugin.ts` (d) — assigns + carries stable IDs.
    - `mark-identity-decoration-plugin.ts` — exposes IDs as `data-mark-id` attributes for event delegation (required because `renderHTML` runs BEFORE `appendTransaction` assigns the ID — decoration is the only hook that can mutate a mark's rendered DOM post-assignment).
    - `link-resolution.ts` + `link-resolution-decoration-plugin.ts` — pure helpers + PM plugin that read `page-list-cache` and emit `data-resolution-state` decorations so CSS can style resolved/folder/unresolved chips without React round-trips.
...
- `src/markdown/to-react.ts` — **pure** mdast → element-tree walker (V2 SPEC FR11 Option E backend). NO `react` import; accepts a `createElement` factory + `componentMap` as parameters. Covers every mdast node type including OK-specific `wikiLink` + `rawMdxFallback`. Environment-agnostic: the React binding at `packages/app/src/editor/mdast-to-react.tsx` threads `React.createElement`; MCP render-preview / CLI export / SSR consumers use their own factories. Security: `html` nodes pass through as TEXT (auto-escaped by the factory); JSX expression attrs use best-effort `JSON.parse` with raw-string fallback (MDX trust level).
...
**STOP — `/api/document-disk` MUST NOT create a Y.Doc session.** Unlike `/api/document` (which returns live Y.Text via a session) this endpoint reads disk bytes directly so the V2 Suspense fallback paints pre-sync. Reusing the file-watcher's canonical-path index is allowed (read-only); mutating the index or admitting the doc to a session defeats the fallback's purpose (precedent #25(e), V2 SPEC §16).
...
- `src/api-extension.ts` — HTTP API; includes save-version, rescue buffer, link-graph, metrics, and the V2 `GET /api/document-disk` fallback endpoint. `handleDocumentDiskRead()` reads `fs.readFile` directly (no session, no index mutation); symlink escape → 403, missing file → 404.
...
- `src/editor/TiptapEditor.tsx` — WYSIWYG editor, HocuspocusProvider; under V2 cache (precedent #25(a)) the component binds to the module-cached Editor via `mountTiptapEditor(...)` on mount and `parkTiptapEditor(entry)` on unmount — it does NOT call `editor.destroy()` (only `evictTiptapEditor` does)
- `src/editor/SourceEditor.tsx` — CodeMirror 6 with y-codemirror.next; wires `createSourcePolishExtension()` + `codeLanguages` allowlist + GFM. Symmetric V2 cache binding via `mountCmEditor`/`parkCmEditor`/`evictCmEditor`
- `src/editor/editor-cache.ts` — V2 editor cache (precedent #25(a),(b); precedent #18(g)/(h)/(i)). Module-level `Map<docName, Entry>` for TipTap + CM6; symmetric `mount{Tiptap,Cm}Editor` / `park{Tiptap,Cm}Editor` / `evict{Tiptap,Cm}Editor` + `setActivityMountList()` (provider connect/disconnect cap per FR3b) + `shouldCacheEditor({viewCount, bytes})` size gate + `CACHE_ENABLED` kill switch (FR15). `editor.destroy()` ONLY runs in `evict*` — park detaches `view.dom` to a lazy parking node. TipTap uses raw `editor.editorView.dom` reparent (Phase 1.0 spike proved `Editor.mount/unmount` is blocked by `@tiptap/extension-drag-handle@4.x`'s proxy-capture); CM6 uses the within-Document reparent pattern (H1 12/12 probe).
- `src/editor/interaction-layer.tsx` — Generic editor-root React plane (precedent #25(c)). `createInteractionLayer({editor})` returns `{register, deregister, destroy, setActiveNode}`. Pure `InteractionLayerStore` with identity-stable snapshot for `useSyncExternalStore`. Event delegation on `editor.view.dom` + document-level outside-click dismiss. Resolves click target via `closest('[data-mark-id]') || closest('[data-node-id]')` with mark-id > node-id precedence. `InteractionControls` bag: `propPanel` (V2), `toolbar` + `breadcrumb` (CB-v2 extension points).
- `src/editor/interaction-layer-host.ts` — `getInteractionLayer(editor)` — per-editor singleton host keyed in `WeakMap<Editor, InteractionLayerHandle>`. First chip extension to call `getInteractionLayer(editor)` creates the layer; subsequent extensions reuse it. Editor destroy → `handle.destroy()` + WeakMap delete. Ensures the 4 chip extensions (internal-link, wiki-link, raw-mdx-fallback, jsx-component) share one plane per editor.
- `src/editor/mdast-to-react.tsx` — Thin React binding for `packages/core`'s pure `mdastToReact` walker (precedent #25(e)). Exports `markdownToReact(md: string): ReactElement`. Module-level `MarkdownManager` singleton amortizes processor construction. Threads `React.createElement` + `getMDXComponents()` into the walker.
- `src/editor/componentMap.ts` — Fumadocs-compatible placeholder components (Callout, Tabs, Accordion, Steps, Card, Cards, Files, Folder, ImageZoom, Mermaid, TypeTable) that paint in the Suspense fallback without pulling `fumadocs-ui` into the editor bundle. Swaps to real fumadocs imports when CB-v2 lands it as a direct dep. Mermaid carve-out: `aspectRatio 16/9, role="status"` placeholder (1.5 MB mermaid lib deferred to post-paint hydration).
- `src/editor/page-list-cache.ts` — Module-level side-channel store (precedent #25(c) support). PM plugins that render plain-DOM chips cannot access React context; `setPageListCache({pages, folderPaths})` is the single-writer API (called from `PageListContext`), `subscribePageListCache(fn)` + `getPageListCache()` are the consumer APIs. Content-equality gate prevents render-frequent writes from storming subscribers. DEV-gated `window.__okPageListCache` write for debugging (precedent #20(b)).
- `src/editor/provider-pool.ts` — LRU-bounded HocuspocusProvider pool (`MAX_POOL = 10`); sets client-side `forceSyncInterval: 5000` (SPEC D8, secondary defense against `synced`-never-fires; primary safety net is the 30s `syncPromise` timeout); emits pool-change notifications consumed by `DocumentContext`; invalidates `syncPromise` cache entries on provider destroy/recycle. `prewarm(docName)` (V2 SPEC FR12 Option G) admits a cold provider at LRU-oldest position without touching active — caller enforces intent-debounce + concurrency cap.
...
- `src/components/FallbackDocumentRender.tsx` — V2 Option E Suspense fallback (precedent #25(e)). Consumes `markdown` bytes (fetched from `GET /api/document-disk`) + renders via `markdownToReact`. Emits `ok/render/fallback` telemetry with `{bytes, docName}` — the perceived first-paint signal (V2 SPEC G5 < 500 ms prod P95 target). Pure-render (no state, no effects beyond telemetry mark); skip-fallback fast path + 30 s hydration-timeout overlay are owned by the Suspense wiring in `EditorActivityPool`.
...
- `src/editor/extensions/JsxComponentView.tsx` — JsxComponent NodeView (per-instance React render still required for fumadocs component mounting — bifurcation per precedent #25(c)); PropPanel/Toolbar migrated to singleton InteractionLayer plane (US-007). Forward-compat stake for CB-v2's JsxComponentView — BridgeStore persists across nav because V2 cache preserves Editor identity.
- `src/editor/extensions/mark-identity-plugin.ts` — PM PluginState carrying `{byId: Map<id, MarkInfo>, counter}`. On every `docChanged` transaction carries IDs forward via `tr.mapping`, assigns `m${++counter}` for new spans, drops deleted ranges. Fires `onRegister`/`onDeregister` callbacks for InteractionLayer wiring (FR4b). Schema NOT touched — precedent #9 add-only preserved.
- `src/editor/extensions/mark-identity-decoration-plugin.ts` — Materializes `markIdentityPlugin`'s IDs as `data-mark-id` inline decorations (required because mark `renderHTML` runs before `appendTransaction` assigns IDs). Event delegation reads `closest('[data-mark-id]')`.
- `src/editor/extensions/link-resolution.ts` — Pure link-resolution helpers (`computeLinkResolutionState`, `makeLinkResolutionAttrsComputer`). Classifies `external` / `anchor` / `loading` / `resolved` / `folder` / `unresolved` from href + sourceDocName + page-list-cache snapshot. No React, no DOM, no window globals — consumed by the decoration plugin's `computeAttrs` callback.
- `src/editor/extensions/link-resolution-decoration-plugin.ts` — Emits `data-resolution-state` decorations for tracked link marks. Subscribes to page-list-cache; dispatches a self-keyed meta transaction on cache changes to force PM to re-run `props.decorations`. Stacks with `mark-identity-decoration-plugin` (disjoint attr keys).
- `src/editor/extensions/mark-interaction-bridge.ts` — Wires `markIdentityPlugin` → `InteractionLayer.register/deregister`. Concentrates three correctness points: live position lookup via `getCurrentMarkInfo(state, id)`, context augmentation (`{editor, nodeId, deactivate}` → PropPanel), synchronous deregister ordering. Every mark chip extension (internal-link, wiki-link) consumes `createMarkInteractionBridgePlugin({editor, markTypes, renderPropPanel})`.
- `src/lib/perf/cold-mount-instrumentation.ts` — Diagnostic monkey-patches around TipTap + y-prosemirror + PM sync cost centers (`Editor.mount`, `Editor.createView`, `Editor.createNodeViews`, `EditorView.updateState`, `EditorView.setProps`, `ProsemirrorBinding._forceRerender`, `PureEditorContent.init`). Emits `ok/cold/*` marks + PerformanceObserver `paint` mirror. Installed ONCE from `main.tsx`. Safe in prod (mark() no-ops its collector in PROD).
...
- **WARN:** With V2 editor cache (precedent #18(g)/(h)), the editor instance OUTLIVES the React subtree that mounted it. `editor.destroy()` no longer happens on React unmount — only on `evictEditor()`. Two consequences: (1) Local Y.js observers on the cached editor's Y.Doc keep firing on every CRDT update even while the React subtree is unmounted (this is the precedent #18(c) class amplified across N cached editors); the FR3b provider-disconnect cap (precedent #18(i)) bounds remote-peer load to `ACTIVITY_MOUNT_LIMIT`. (2) Code that assumed "editor exists ⇔ React component is mounted" is wrong — `editor.commands.*`, plugin transactions, and any side-effects that fire from extension handlers will execute against a cache-resident editor whose host React subtree is unmounted. Audit any new code that calls `editor.commands.*` from a non-render context (effects, event handlers fired from outside the editor's DOM, async callbacks): if it assumes the React subtree is alive, gate on `entry.activeMountKey === currentDocName` from the cache entry.
