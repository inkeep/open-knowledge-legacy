...
19. **Floating UI is the canonical positioning primitive for selection-anchored overlays.** For any UI that floats relative to editor-content coordinates — link editor popovers, image caption toolbar, bubble menu, future collaborator presence pins, action toolbars — use the `useSelectionAnchoredPopover` hook (`packages/app/src/editor/hooks/use-selection-anchored-popover.ts`) wrapping `@floating-ui/dom`. Virtual elements derive from PM selection via the `computeSelectionAnchor` pure function (`packages/app/src/editor/hooks/compute-selection-anchor.ts`): prefer the selected jsxComponent wrapper's DOM rect via `view.nodeDOM(pos)`, fall back to `posToDOMRect(view, from, to)` for text/cell selections. `autoUpdate` handles scroll/resize/intersection/layout-shift tracking automatically. No ad-hoc `position: absolute` with hand-rolled listeners. Precedent consumers scale naturally: the link editor, image caption, and any future collaborator-presence pin reuses the hook without re-solving positioning.
...
- `globals.css` defines dark overrides via Tailwind's `.dark` selector for ProseMirror content, callouts, and custom components. Also declares `--color-fd-*` CSS variable bridge mapping fumadocs-ui's CSS variable namespace to shadcn design tokens (FR-25), plus fumadocs utility classes (`fd-steps`/`fd-step`, `fd-accordion-*`/`fd-collapsible-*` keyframes) and a `@source` directive for Tailwind to scan fumadocs-ui dist.
...
### Component blocks subsystem

Block-level MDX JSX editing with descriptor-dispatched rendering, prop panels, and CRDT-safe serialization.

```
jsxComponent PM node
├── Descriptor Registry (runtime lookup: name → JsxComponentMeta)
│   ├── 18 built-in fumadocs-ui components
│   └── Wildcard '*' fallback (UnregisteredBadge + editable children)
├── JsxComponentView (three-branch NodeView: registered, wildcard, error)
│   ├── PropPanel (auto-generated controls from PropDef)
│   ├── ComponentErrorBoundary → nested CodeMirror on render failure
│   └── NodeViewContent (always visible — Precedent #14)
├── Source-dirty observer (origin-aware user-intent dirty tracking)
├── Bridge-ID plugin (PluginState for stable identity, not schema attr)
├── Context Bridge Registry (cross-NodeView React Context propagation)
│   ├── Store (publish/subscribe/get per bridgeId)
│   └── ContextCapture + ContextBridgeProvider helpers
└── γ Serialization (sourceRaw pristine / reconstruction dirty)
```

**γ serialization pattern.** Pristine nodes (`sourceDirty: false`) serialize via `sourceRaw` — byte-identical round-trip. Edited nodes (`sourceDirty: true` or `hasDirtyDescendant`) serialize via `mdxJsxFlowElement` mdast reconstruction from structured attrs + recursively-serialized children. A custom flush-left to-markdown handler prevents CommonMark 4-space code-block hazard at nesting depth 2+.

**SideMenu.** `@tiptap/extension-drag-handle-react` provides a drag handle grip + "+" insertion button on block hover. The "+" inserts a paragraph with `/` to trigger the slash command menu (TipTap vendor-endorsed pattern).

**Keyboard navigation (L1-L4).** L1: Esc → `selectParentNode`. L2: Arrow Up/Down between blocks in nav mode. L3: custom Enter for isolating-boundary container exit. L4: Escape priority chain (Suggestion collapses before L1 fires).

**Compound component wrappers (US-015).** Editor-local Tabs/Accordion wrappers (`compound-wrappers.tsx`) use DOM data-attribute state management rather than React Context to work across NodeView portal boundaries. Pattern-copied from fumadocs-ui to avoid Context-crossing limitations.
...
- `src/editor/extensions/JsxComponentView.tsx` — Three-branch descriptor-dispatch NodeView (registered → live React + PropPanel, wildcard → UnregisteredBadge, error → nested CM)
- `src/editor/extensions/RawMdxFallbackCMView.tsx` — Nested CodeMirror 6 for rawMdxFallback + error-state editing (Precedent #12: direct PM dispatch, not y-codemirror.next)
- `src/editor/extensions/source-dirty-observer.ts` — PM plugin tracking user-intent edits to jsxComponent nodes; deny-lists sync/agent/rollback origins
- `src/editor/extensions/bridge-id-plugin.ts` — PM PluginState assigning stable `bridgeId` per jsxComponent keyed by Y.XmlElement identity (not schema attr)
- `src/editor/context-bridge/store.ts` — Editor-scoped bridge store for cross-NodeView React Context propagation (`useSyncExternalStore` protocol)
- `src/editor/slash-command/component-items.ts` — Slash-command items for registered block components with FR-14a default props ladder
- `src/editor/components/compound-wrappers.tsx` — Editor-local Tabs/Accordion wrappers (DOM data-attribute state, not React Context)
- `src/editor/registry/types.ts` — App-level registry types (extends core `JsxComponentMeta` with React `Component` field)
...
- Parses Y.Text markdown via `mdManager.parseWithFallback()` (never throws — broken spans become `rawMdxFallback` nodes in place), applies to XmlFragment via `updateYFragment()`
...
- **Always-live (G9):** No document-level freeze on parse failure. Broken blocks degrade to `rawMdxFallback` chrome; structured siblings stay rendered. Eliminates the pre-v2 behavior where one broken JSX region anywhere in the doc could freeze all WYSIWYG preview.
...
- **WARN:** γ serialization `effectiveDirty` walk is load-bearing. A pristine jsxComponent parent whose descendant is dirty MUST NOT emit its own stale `sourceRaw` — that silently drops the descendant's edit. The `hasDirtyDescendant(node)` walk ensures reconstruction cascades upward. See I16 fidelity invariant.
- **WARN:** `bridgeId` lives in PM PluginState (`bridgeIdPluginKey`), NOT in schema attrs. Reading `node.attrs.bridgeId` will return `undefined`. Use `getBridgeId(editorState, node, getPos)` from `bridge-id-plugin.ts`.
...
- **NG12:** Edited-node quoting normalization. When a `jsxComponent` is edited (`sourceDirty: true`), the γ serialization path reconstructs the tag via `mdast-util-mdx-jsx`, which produces minor normalization: single → double quotes for string attrs, intra-tag whitespace normalization, blank-line count inside container bodies. Verified idempotent (converges on first serialize; double round-trip stable). `jsxInline` is exempt — the thin shape has no reconstruction path.
...
- `packages/core/src/markdown/parse-with-fallback.ts` — `parseWithFallback()` + `findFallbackRegion()` (single-pass stack-based enumeration via `enumerateFallbackRegions` + `scanTagEvents`). Broken spans become `rawMdxFallback` in place; structured siblings preserved (G9 ancestor-chain locality).
