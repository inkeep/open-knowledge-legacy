...
- **`isSystemDoc()` + `isConfigDoc()` gates at every documentName-keyed entry point.** Any subsystem keying off `documentName` MUST short-circuit on `isSystemDoc(name) || isConfigDoc(name)` at entry (see `cc1-broadcast.ts`). Synthetic docs: `__system__` (CC1); `__config__/project` + `__user__/config.yml` (Y.Text-only Settings transport — markdown bridge gated in `server-observer-extension.ts`). `ContentFilter` + `POST /api/create-page` reject these prefixes.
- **Server-side observer cross-CRDT writes use `OBSERVER_SYNC_ORIGIN`** (precedent #14). Do NOT re-add client-side cross-CRDT write paths in `observers.ts`.
- **Only one `BridgeMergeContentLossError` catch site** in `server-observers.ts` Observer A Path B (emits telemetry, queues silent `saveInMemoryCheckpoint`, applies merge as-computed). A second catch site silently drops the observability signal. Ref: [`specs/2026-04-16-bridge-correctness/SPEC.md`](specs/2026-04-16-bridge-correctness/SPEC.md) §6 R7/R7b.
- **Paired-write origins MUST declare `context.paired: true`** (precedent #1 extension). Origins that atomically mutate both Y.XmlFragment and Y.Text opt in via the typed marker; `isPairedWriteOrigin(origin)` is a structural check.
- **Don't add Y.js observers inside `<Activity>` subtrees without `ACTIVITY_MOUNT_LIMIT`-style bounding** (precedent #18(c); ref `EditorActivityPool.computeActivityMountList`). Y.js observers do not pause on Activity hidden.
- **Don't collapse the hybrid render tree** (`DocumentErrorBoundary` → `Suspense` → `EditorActivityPool` → `Activity` → `DocumentBoundary`) to `<Editor key={activeDocName} />` — load-bearing for flash-free UX (precedent #18(b)). New write surfaces wrap in their own `DocumentBoundary`. Canonical: `packages/app/src/components/EditorArea.tsx`.
- **HTTP API discipline (RFC 9457).** Errors via `errorResponse(res, status, type, title, ...)` (`packages/server/src/http/error-response.ts`) — never inline `json(res, NNN, { ok: false, ... })`. `type` is URN `urn:ok:error:<kebab>` from `ProblemTypeSchema` (closed). Successes drop `{ ok: true }`. Request bodies via `withValidation(Schema, ...)`. DUs end in `default: assertNeverXyz(target)`. Pattern: [`packages/server/src/http/README.md`](packages/server/src/http/README.md).
- **Agent-undo single-path:** `applyAgentUndo(session, scope)` in `agent-sessions.ts` — only sanctioned server-side undo write surface. Contract in JSDoc on the function.
- **`recordContributor` summaries route through `normalizeSummary`** (`agent-write-summary.ts`). Single API-boundary truncation point. Whitespace-only inputs classify as `absent`. SPEC: [`specs/2026-04-21-agent-write-summaries/SPEC.md`](specs/2026-04-21-agent-write-summaries/SPEC.md).
- **`handleRenamePath` / `handleRollback` use `extractActorIdentity(body, getPrincipal)`**, NOT `extractAgentIdentity`. Body `agentId` → `agent-<id>`; absent + loaded principal → `principal-<uuid>`; neither → anonymous (NG12: never default to claude-1). Body `principalId` silently ignored. Spec: [`specs/2026-04-29-rename-consolidation/SPEC.md`](specs/2026-04-29-rename-consolidation/SPEC.md).
- **Don't narrow PM mark `excludes` fields** (precedent #9; mark attrs are add-only). US-017 widened `Code` via `CodeMarkFidelity` (`excludes: ''`); reverting via a Tiptap upgrade reintroduces idempotence failures.
- **`shell.openPath` single-source via `openAssetSafely`** (`packages/desktop/src/main/asset-allowlist.ts`); three-check gate + `ok:shell:open-asset` IPC handler. Unguarded is RCE-class. Don't narrow `EXECUTABLE_BLOCKLIST_EXTENSIONS` in `core/src/constants/upload.ts`.
- **No `upload.*` config + no runtime `.obsidian/app.json` reader.** Values are constants in `core/src/constants/upload.ts`.
- **Server-side disk writes go through `fs-traced.ts` wrappers** (`tracedWriteFile` / `tracedRename` / etc.). `@opentelemetry/instrumentation-fs` does not work on Bun (oven-sh/bun#6546). Test-only code exempt.
- **Don't emit unbounded-cardinality span/metric attributes.** Paths via `normalizeFsPath` + `classifyFsPath`; identifiers via pre-validated UUIDs / enums. Safe pre-normalized: `doc.name`, `shadow.writer`, `agent.write_position`, `http.route`.
- **Server-side asset admission via `createAssetServeMiddleware`** (`packages/server/src/asset-serve-middleware.ts`). Don't drop the CD dispatch or SPA-fallback 404 guard.
- **Client-persistence ordering on `server-instance-mismatch`:** buffer → `clearData()` → `recycleAllEntries`. Reversing duplicates markers. Ref: `provider-pool.ts`.
- **No OK sidecars in user-content paths.** State lives in `<contentDir>/.ok/`; no `.frontmatter.yml` / `_meta.json` / `_index.md`. Spec: [`specs/2026-04-25-config-edit-paths/SPEC.md`](specs/2026-04-25-config-edit-paths/SPEC.md).
- **`ConfigSchema` leaves: `.register(fieldRegistry, ...)` BEFORE `.default()`/`.optional()`/`.nullable()`** — Zod v4 wrappers drop `_zod.parent`. Coverage test enforces.
...
- Layer A unit tests use `transaction.local=true` — NOT the same code path as production (`local=false` on WebSocket updates). Unit coverage alone misses remote-peer divergence.
- `hocuspocus.configure({ extensions: [...] })` REPLACES the array. Use `hocuspocus.configuration.extensions.push()` to add.
- TipTap's `editor.view` is a throwing proxy pre-ProseMirror-mount; touching `view.dom` during recycle/remount crashes ErrorBoundary. Use `editor.editorView` and subscribe to `'create'` before `view.dom` access. Ref: `editor/TiptapEditor.tsx`.
- React 19.2 `<Activity mode="hidden">` unmounts subtree DOM — scroll containers wrapping multiple Activity mounts lose `scrollTop` on flip. **Each Activity mount owns its own scroll container** (`EditorActivityPool.tsx` + `ScrollPreservingContainer`).
- Playwright `_electron.launch({ args: [url] })` does NOT fire macOS `open-url` Apple Event — URL arrives via `process.argv` only. True Apple-Event tests must `execSync('open -g "openknowledge://..."')`. Ref: `packages/desktop/tests/smoke/deep-link.e2e.ts`.
- `syntaxTreeAvailable()` reflects DEEPEST pending sublanguage, not outer markdown tree — gating decorations on it silently disables them when a fenced-code block enters the viewport. ViewPlugin: detect `syntaxTree(update.startState) !== syntaxTree(update.state)`; StateField: early-return on `!tr.docChanged`.
- Buffer-and-replay on `server-instance-mismatch` is memory-only: tab crash mid-recycle loses the buffer; null-SV guard discards unacked state by design.

**Logging conventions.** Two `console.warn` styles: (1) bracket-prefix (`[file-watcher] ...`) for human-read ops; (2) structured JSON for events counted in aggregate or asserted in tests. Don't convert without knowing consumers.

## Observability (OpenTelemetry)

Opt-in, dev-focused. Default builds: SDK disabled on the server (`OTEL_SDK_DISABLED=false` enables — must be the literal string `"false"`; any other value keeps it disabled), bundle-eliminated on the frontend (`VITE_OTEL_ENABLED=true` enables; build-time env). Zero overhead when off.
...
**Adding a span / metric:** wrap the call site with `withSpan('my.operation', { attributes }, async () => { … })`; for disk writes use the `traced*` wrappers; namespace repo-specific attributes / metrics under `ok.*` / `agent.*` / `shadow.*` / `persistence.*` / `doc.*` / `config.*` and follow OTel semconv for `http.*` / `fs.*` / `db.*`. Cardinality discipline is the STOP rule above; HTTP-initiated paths inherit `traceparent` automatically via `onRequest`, WS-initiated paths are independent roots.
...
**Invariants I1-I11** (PBTs in `packages/app/tests/fidelity/invariant-i{1..10}.test.ts`; I11 at `autolink-void-html-guard.precision.test.ts`) + six handler-specific PBTs cover identity, character preservation, idempotence, Layer A===B, multi-client preservation, cross-path consistency, crash resistance, guard completeness. Full list + irreducible gaps NG1-NG11: [`specs/2026-04-16-markdown-pipeline-engineering-health/SPEC.md`](specs/2026-04-16-markdown-pipeline-engineering-health/SPEC.md).
