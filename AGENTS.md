...
**Nightly E2E stability surveillance.** `.github/workflows/nightly-e2e-stability.yml` runs the full Playwright suite with `--repeat-each=3 --workers=1` at 09:00 UTC daily (off-minute from `nightly.yml`'s 06:17 UTC to avoid GHA contention). On failure it auto-opens a GitHub issue labeled `e2e-flake` with the run URL and artifact pointers. This is NOT a correctness tier — it does not block PR merges — but it catches slow-burn drift that PR-time `failOnFlakyTests: true` misses (a test that passes 99/100 accumulates a 1% tail only visible under `--repeat-each`). Label setup is one-time: `gh label create e2e-flake --color FBCA04 --description "Surfaced by Nightly E2E stability workflow"`.
...
20. **E2E test infrastructure conventions.** Playwright tests under `packages/app/tests/stress/*.e2e.ts` follow seven sub-rules. The mechanical STOP rule at `packages/app/tests/integration/e2e-stop-rules.test.ts` enforces them — no annotated exceptions (D-Q14/D-Q15 LOCKED in `specs/2026-04-17-e2e-observability-determinism/SPEC.md`).
    ...
    (e) **CI artifact upload on failure (14d retention).** `.github/workflows/ci.yml` uploads `playwright-report/` (`if: !cancelled()`) and `test-results/` (`if: failure()`) via `actions/upload-artifact@v7`. 14-day retention. Trace + video + screenshot are configured via `playwright.config.ts` `use.trace: 'on-first-retry'`, `use.video: { mode: 'retain-on-failure', size: 1280×720 }`, `use.screenshot: 'only-on-failure'`. Combined with `retries: 2` (CI only) + `failOnFlakyTests: true` (CI only), every CI failure is debuggable from artifacts alone — no local re-run needed.
