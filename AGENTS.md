...
**CC1 push-over-awareness** — pure-signal push primitive for derived views (file list, backlinks, graph). Contract v1: `{v:1, ch:string, seq:number}`. 100 ms trailing-edge debounce per channel. Emitter: `packages/server/src/cc1-broadcast.ts`. Every subsystem keyed off `documentName` MUST short-circuit via the single `isSystemDoc()` helper in that file (STOP rule below). Channels: `server-info` (serverInstanceId on `onConnect`; fixed seq:0, unlike seq-incrementing channels), `files` / `backlinks` / `graph` (derived-view invalidation), `branch-switched` (synchronous emit on branch rollover; clients clear IDB per doc + recycle all providers).

### Client-side Yjs persistence

Each browser tab's `ProviderPool` wraps every `HocuspocusProvider` with a `ClientPersistenceProvider` backed by upstream `y-indexeddb` (patched — `patches/y-indexeddb@9.0.12.patch` fixes issues #31 + #44). Database name is `ok-ydoc:${docName}`; `__system__` is skipped. The typed wrapper is `packages/app/src/editor/client-persistence.ts`: branded `DocName`, `createClientPersistence(docName, doc)`, `captureStateVector(doc)`, `computeUnsyncedUpdate(doc, lastAckedSV)`.

On every server `synced` event, the pool captures `entry.lastServerSyncedSV = captureStateVector(doc)`. On `authenticationFailed({reason: 'server-instance-mismatch'})`:

1. **Buffer** unsynced delta per entry via `computeUnsyncedUpdate(doc, lastServerSyncedSV)` — but **only** when `lastServerSyncedSV !== null`. A null SV means auth-fail arrived before any successful sync (typical: populated IDB on first connect against a mismatched server); in that case IDB content is stale and must be discarded, not preserved.
2. **Clear** each entry's IDB via `persistence.clearData()` (awaits `indexedDB.deleteDatabase` — upstream's chain races against subsequent opens).
3. **Recycle** all entries via `pool.recycleAllEntries()`.
4. **Replay** buffered updates under `TAB_REPLAY_ORIGIN` on the next fresh provider's first `synced` event.

Branch rollovers take a simpler path: `handleBranchSwitched` clears all persistences then recycles — no buffering, because unsynced edits on branch A are semantically invalid against branch B (reintroducing them corrupts target state). Consumer wiring: `packages/app/src/components/SystemDocSubscriber.tsx` parses CC1 stateless frames via `parseCC1BranchSwitched` in `packages/app/src/lib/cc1.ts` and dispatches through a `DocumentContext` ref.
...
- **Client-persistence `clearData` MUST run BEFORE provider recycle on `server-instance-mismatch`.** Buffer unsynced edits FIRST via `computeUnsyncedUpdate(doc, lastServerSyncedSV)`; THEN `persistence.clearData()` per entry; THEN `recycleAllEntries`. Reversing the order reintroduces the content-duplication bug class — a fresh provider hydrates from stale IDB under a new server clientID, merges with server's markdown-rebuilt state, and every marker lands twice on disk. The guard `lastServerSyncedSV === null ⇒ skip buffer` is load-bearing: null SV means IDB state has no acked baseline and is by definition stale. Reference: `packages/app/src/editor/provider-pool.ts#handleServerInstanceMismatch`. Spec: [`specs/2026-04-24-client-persistence-replaces-sidecar/SPEC.md`](specs/2026-04-24-client-persistence-replaces-sidecar/SPEC.md).
- **`expectedServerInstanceId` claim on the Hocuspocus auth token is Zod-validated via `parseHocuspocusAuthToken`.** Never `JSON.parse` the token and check fields ad-hoc. The schema is `.loose()` for forward-compat — unknown fields survive. `onAuthenticate` throws `{reason: 'server-instance-mismatch'}` on non-empty non-matching claim; client-side `ProviderPool.onAuthenticationFailed` nulls the cache + recycles all entries. Empty-string claim treated identically to absent (legacy path).
...
- Buffer-and-replay on `server-instance-mismatch` is MEMORY-only during the recycle window. A tab crash within the 50-500ms window between `handleServerInstanceMismatch` buffering and the fresh provider's first `synced` event loses the unsynced buffer. Accepted trade-off vs. the alternative (localStorage-persisted buffer with own eviction policy). Users typing in the 50-500ms window between page load and first successful sync AT THE SAME TIME the server rejects their instance-id claim also lose those keystrokes (the `lastServerSyncedSV === null` guard discards rather than replays unacked state). Reference: `packages/app/src/editor/provider-pool.ts`.
