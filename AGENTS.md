...
- **WARN:** Obsidian vault detection is **vault-first** on the two fields it knows about. `packages/cli/src/commands/start.ts` merges the `detectObsidianVault` partial via `vaultPartial.X ?? config.upload.X` — when the vault has a value, it overrides the user's `config.upload` key. Zod always materializes defaults, so there's no way to distinguish "user kept default" from "user never touched upload"; the pragmatic interpretation is "vault is authoritative when present." If a user hand-edits `.open-knowledge/config.yml` to pin `upload.attachmentFolderPath` AND the project has an `.obsidian/app.json` with a different path, the vault wins. Document this in any operator-facing guide; flag to users who are surprised.
...
- **WARN:** The Vite dev plugin (`packages/app/src/server/hocuspocus-plugin.ts`) does NOT construct a `resolveEmbed` closure — it calls `createServer()` directly without the basename-index wiring that `packages/cli/src/commands/start.ts` does. In dev mode (`bun run dev`), `![[photo.png]]` embeds in a doc two dirs deep fall back to literal-target resolution and the `<img src=photo.png>` can 404 if sirv doesn't find `photo.png` at the current doc's URL. Published CLI (`ok start`) does the full wiring. If dev-mode fidelity matters for a given session, run `ok start` against the same content directory instead (mutual exclusion enforced by `server.lock`).
