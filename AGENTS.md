...
- `src/constants/upload.ts` — `UploadConfig` / `EmitFormat` / `DedupMode` / `DedupUIMode` type surface; `DEFAULT_UPLOAD_CONFIG` (single source of truth for the resolved `upload.*` subtree; CLI Zod schema mirrors via structural `_UploadConfigShapeMatches` compile-time check); `ASSET_EXTENSIONS` (ContentFilter admit-set: images + PDF + video + audio + fonts + zip); `ALLOWED_IMAGE_MIME_TYPES` retained as a reference list for any future MIME/extension mapping helper (no runtime gate consumer post-D-M)
...
**sanitizeFilename (`api-extension.ts`).** Unicode-preserving: strip-on-sight first (path separators `/\\`, C0 controls, DEL), then whitelist via Unicode category classes (letters, numbers, marks, punctuation, emoji pictographs). Run-collapse `_+` → `_`, trim leading dots, strip trailing dots. Preserves non-ASCII filenames byte-for-byte while blocking path escape. Path-escape guards (`..`, absolute, `\x00`) run separately at request time. `writeUploadAtomic` preserves extension when adding collision suffixes.

**sha256 dedup (FR-2).** Same-directory scope. Server computes `sha256Hex(buffer)`, scans the destination directory for asset-extension siblings (via `ASSET_EXTENSIONS` from core), compares hashes, returns existing path on match. `upload.dedup.mode: 'off'` bypasses. Cross-directory dedup is NG1 — not in scope.
...
`shortestImageRef(assetPath, mdPath)` returns the minimal relative reference under four cases: same-dir → basename; ancestor → `./<sub>/<name>`; parent chain → `../<name>`; cross-tree → `../…/<name>`.
