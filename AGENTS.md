...
- `src/editor/use-editor-mode.ts` — `useEditorMode()` hook. `EDITOR_MODE_VALUES` const array is the single source of truth — both `EditorModeValue` and `isEditorModeValue()` derive from it. `useState` initializer prefers `window.__OK_EDITOR_MODE__` (typed `unknown` — every reader MUST validate via `isEditorModeValue`), falls back to `localStorage`, then `'wysiwyg'`. `focus` listener on `window` re-reads storage and updates state when persisted value differs (functional-update form short-circuits no-op writes). Every call to the hook's setter persists immediately; session-local paths that bypass the hook (e.g. `RAW_MDX_NAV_EVENT` → `setEditorMode('source')` directly in `EditorPane`) do not persist. Invalid persisted values produce a `[editor-mode] invalid persisted value` `console.warn` (FR-8); storage throws stay silent to avoid per-focus spam.
- `src/components/EditorPane.tsx` — consumer. `EditorMode = EditorModeValue | 'diff'` structurally encodes "diff is the only non-persistable mode." Seeds session-local `editorMode` from `persistedMode`, tracks it in `editorModeRef`, and re-applies `persistedMode` via a dedicated `useEffect` guarded by the pure `shouldApplyPersistedMode(editorModeRef.current)` helper (returns `false` when current is `'diff'`) so diff exit still restores the session pre-diff mode via `modeBeforeDiffRef` (audit-surfaced H1 race — SPEC §7.4 R1). Dep array is `[persistedMode]` alone; never add `editorMode` to it.
...
- E2E coverage: `packages/app/tests/stress/editor-mode-persistence.e2e.ts` (T1-T9, in the CI `test:e2e` file list).
- **Drift-guard:** PRs that touch `use-editor-mode.ts`, `EditorPane.tsx` cross-window sync, or `packages/desktop/src/main/window-manager.ts` must re-run SPEC §8.4 MQ1–MQ3 manual Electron multi-window QA (the primary motivation is multi-window Electron users; only manual QA covers it — there is no `_electron.launch()` Playwright harness at baseline).
