...
**Playwright isolation (`OK_TEST_CONTENT_DIR`).** When the dev plugin detects an isolated tmpdir, it initializes a per-worker shadow repo at `<tmpdir>/.git/open-knowledge/` by default and sets `gitEnabled: true` (SPEC 2026-04-22 FR1) so the full-app topology under Playwright matches production — TimelinePanel, Save Version, rollback, and classified-writer attribution all exercise real shadow commits. All shadow-init errors (`ProjectGitInitError` from a missing `git` binary, EEXIST races, disk-full, corrupt `.git/`, etc.) fail-fast via `process.exit(1)` under isolation (FR5 / D13) — no silent coverage loss. Production (non-isolated) boot retains the existing degraded-warn path for non-`ProjectGitInitError` failures.

All `PROJECT_ROOT`-derived bindings in the dev plugin resolve through a single module-level `projectRoot = isTestIsolated ? CONTENT_DIR : PROJECT_ROOT` binding (D12). This keeps per-worker isolation airtight — BacklinkIndex cache, WIP-ref branch name, server-lock metadata, and `/api/save-version` all scope to the tmpdir, never the developer's OK repo. See the STOP rule under Known Pitfalls before adding new consumers.
...
- `createTestServer(options?)` → spins up real Hocuspocus with HTTP/WebSocket on OS-assigned random port. Options: `{ withShadow?: boolean; contentDir?: string; keepContentDir?: boolean; keepaliveGraceMs?: number; debounce?; maxDebounce? }`. `withShadow` defaults to `false` — the harness's own `ensureProjectGit` subprocess + contributor-tracker drain + `shadowDir` exposure are all skipped under the default, so shadow-orthogonal tests skip those ~50-200ms of cost per test (SPEC 2026-04-22 D2 tier-appropriate defaults). Note: `createServer` itself still runs `initShadowRepo(projectDir)` unconditionally (standalone.ts), so a bare-repo subtree is created regardless — `gitEnabled: false` only suppresses the L2 commit scheduler, not the bare-repo init. A `createServer`-level opt-out is §16 EXCLUDE for this spec and is noted in SPEC §15 Future Work.
...
**Shadow opt-in (`withShadow: true`, SPEC 2026-04-22):**

- `createTestServer({ withShadow: true })` initializes a per-test shadow repo at `<contentDir>/.git/open-knowledge/` before the server boots, sets `gitEnabled: true` on persistence, and exposes `shadowDir` on the returned `TestServer` so assertions can reach it via `simpleGit(server.shadowDir).log(...)` or `simpleGit(server.shadowDir).raw('for-each-ref', ...)`.
- The harness auto-wires `swapContributors()` (the preferred atomic-drain API; `clearContributors()` is `@deprecated`) in both setup and cleanup (D10 / FR7), plus `destroyShadowRepo(shadow)` before `rmSync(contentDir)` in cleanup — serial-execution tests cannot leak contributor-tracker state across assertions, and the bare-git subtree is torn down cleanly before the content dir is removed.
- **STOP:** tests using `withShadow: true` must NOT use `test.concurrent()` within the same file. `contributor-tracker`'s `pendingContributors` is module-level state (`packages/server/src/contributor-tracker.ts:54`); the auto-drain assumes serial execution. Concurrent tests would mix writers across assertions (test A's `recordContributor` landing in test B's L2 drain). Per-server-instance contributor-tracker state is the long-term fix and is tracked as architectural follow-up; the constraint is enforced by convention until then.
- Opt in when your assertions touch `refs/wip/*`, `refs/checkpoints/*`, `/api/history`, `/api/save-version`, writer-ID attribution, or the `ok-actor:` commit body. Otherwise leave off — each opt-in pays ~50–200 ms of shadow init, which adds up across the suite.
- Reference tests (FR8 acceptance set): `shadow-harness-agent-write.test.ts` (T1 — `refs/wip/main/agent-<connId>` + `wip:` subject), `shadow-harness-save-version.test.ts` (T2 — checkpoint ref + parent-repo commit + tag), `shadow-harness-external-write.test.ts` (T3 — `file-system` writer + `reconcile:` subject), `shadow-harness-history-roundtrip.test.ts` (T6 — agent write → `GET /api/history` round-trip). Plus `persistence-fan-out.test.ts` (FR6 migration, replaces the prior hand-forked `initShadowRepo` boot code).
- Playwright workers get shadow automatically under `OK_TEST_CONTENT_DIR` isolation via the dev plugin (FR1) — no fixture-side opt-in needed. The test harness `withShadow` flag is only for Tier 1 integration tests.
...
- **STOP:** Do not add a new `PROJECT_ROOT`-derived binding in `packages/app/src/server/hocuspocus-plugin.ts`. The single module-level `const projectRoot = isTestIsolated ? CONTENT_DIR : PROJECT_ROOT` binding (SPEC 2026-04-22 D12 / §16 ASK_FIRST) is already threaded through every `projectRoot` consumer in the dev plugin — see `specs/2026-04-22-per-worker-shadow-repo-test-harness/evidence/projectdir-couplings.md` for the authoritative enumeration. Notable call-site classes include `acquireServerLock.worktreeRoot`, `runDevShadowInit`, `contentFilter.projectDir`, `BacklinkIndex.projectDir`, persistence `projectDir` + `getCurrentBranch`, api-extension `getCurrentBranch` + `projectDir`, and server-observer `getCurrentBranch`; the evidence file is the source of truth for the exact list and line numbers. A new unconditional `PROJECT_ROOT` consumer under isolation would leak into the developer's own OK repo (writing real `ok/v*` tags on `/api/save-version`, dirty backlink cache in `.open-knowledge/cache/`, WIP refs under the dev's checked-out branch instead of `main`). If you need a new `projectRoot` derivation, thread the existing binding; never re-introduce raw `PROJECT_ROOT` outside the single binding-site. This STOP rule is intentionally narrow to the dev plugin: the CLI (`packages/cli/src/commands/start.ts`) and standalone `createServer` (`packages/server/src/standalone.ts`) already take `projectDir` / `cwd` as explicit arguments from their callers, so they have no `PROJECT_ROOT` derivation to leak. The dev plugin's issue is structurally unique because Vite loads it as a module and the `import.meta.dirname`-derived `PROJECT_ROOT` is baked in at plugin load time.
