...
- `src/constants/upload.ts` — fixed module-level constants for the asset-embed surface (SPEC 2026-04-24 amendment — zero user-facing upload config): `DEFAULT_ATTACHMENT_FOLDER_PATH = './'`, `DEFAULT_EMIT_FORMAT = 'wikiembed'`, `DEFAULT_DEDUP_MODE = 'same-dir'`, `DEFAULT_DEDUP_UI = 'toast'`, `WIKI_EMBED_EXTENSIONS` (15-entry `ReadonlySet` consumed by client `pickInsertShape` + server mdast→PM dispatch). Internal enum types (`EmitFormat` / `DedupMode` / `DedupUIMode`) for `pickInsertShape` dispatch typing, not re-exported from the core barrel. `ASSET_EXTENSIONS` (ContentFilter admit-set: images + PDF + video + audio + fonts + zip). `EXECUTABLE_BLOCKLIST_EXTENSIONS` (D-A5 asset-click-dispatcher amendment blocklist). `ALLOWED_IMAGE_MIME_TYPES` retained as a reference list for any future MIME/extension mapping helper (no runtime gate consumer post-D-M).
...
- `src/extensions/wiki-link-embed.ts` — `WikiLinkEmbed` PM node (`group:inline, inline:true, atom:true`). Client-insert-only shape emitted by `pickInsertShape` at drop time; `renderHTML` dispatches by target extension — image extension → `<img>`, non-image → `<a>` plain-link fallback (P0; Phase 2 typed-component-nodes spec will promote non-image typed extensions to `<Video>` / `<Audio>` / `<PDFViewer>` at render time per D-F read-time promotion). Server-side mdast→PM never emits this node post-round-trip; it's transient between drop and next save.
- `src/utils/identity.ts` — getIdentity, generateRandomName, generateRandomColor
- `src/utils/path-resolve.ts` — `createBasenameIndex()` (browser+Node compatible; no `node:fs` / `node:path` imports). In-memory `Map<lowerCasedBasename, string[]>` with `add` / `remove` / `rename` / `resolveEmbed` / `snapshot` / `size`. `resolveEmbed(basename, sourcePath)` returns the Foam-style shortest-path winner: (1) in-subtree of sourcePath's dirname → shallowest depth; (2) out-of-subtree → minimum relative hops; (3) alphabetical tiebreak on tied candidates. Case-insensitive lookup key, original-case path preserved in bucket for faithful disk I/O. D-D LOCKED: plain Map (no TrieMap dep). Server-side construction and CC1 `ch:'files'` wiring lives in `packages/server/src/standalone.ts`.
...
├── File Watcher (@parcel/watcher → chokidar fallback — owns in-memory file index; emits markdown + asset DiskEvents)
├── Basename Index (in-memory, rebuilt on asset events — feeds wiki-embed resolution)
...
**Channel ownership.** `ch:'files'` fires on markdown `create | delete | rename` DiskEvents AND on `asset-create` / `asset-delete` DiskEvents (`update` / `conflict` do not change the file list). Asset events feed the in-memory basename index (see "Upload surface" below) and invalidate the client's file-sidebar file list. V0-3 will emit `ch:'backlinks'` from the backlink-index update path inside `persistence.ts`. Each channel's semantics are owned by its emitter.
...
Full spec: [`specs/2026-04-16-editor-asset-and-embed-surface/SPEC.md`](specs/2026-04-16-editor-asset-and-embed-surface/SPEC.md). Operator-facing documentation: [Assets and embeds](docs/content/guides/assets-and-embeds.mdx).

**Endpoints:**

- `POST /api/upload` — primary upload handler (`handleUploadImage` in `api-extension.ts`). Multipart body via busboy, streamed end-to-end through `HashingPassThrough` + `stream.pipeline` to a tempfile at `<contentDir>/.open-knowledge/tmp/upload-<uuid>` (memory footprint is O(1), not O(fileSize); see `reports/streaming-upload-refactor/REPORT.md`). Accept-all under D-M: every file is accepted; `file-type` magic-byte sniff is consulted only to (a) preserve SVG `<img>`-only routing for NFR-3 and (b) recover a file extension when the client filename is a generic clipboard name. Non-sniffable bytes are accepted under the client-supplied filename. On dedup miss the tempfile is promoted via POSIX `linkSync(tempPath, destPath)` with a 99-attempt collision retry (matches the buffer-era `-1`…`-99` suffix semantic). Response: `{ ok: true, src: '<basename>', path: '<contentDir-relative>', deduped: boolean }` on success. Error envelope: `{ ok: false, error: <reason>, message }` where `reason ∈ { 'malformed-upload' (400), 'storage-full' (507), 'storage-readonly' (500), 'collision-exhaustion' (500), 'storage-error' (500) }` — see the `UploadWriteReason` union in `packages/server/src/upload-errors.ts`. `src` is the basename the server wrote; `path` is the contentDir-relative location (always colocated with the referencing doc per `DEFAULT_ATTACHMENT_FOLDER_PATH = './'`). Dedup runs BEFORE filename synthesis so identical bytes return the existing path; `deduped: true` triggers the client's dedup toast.

**Accept-all posture (D-M).** There is no MIME allowlist gate in the upload handler. Type-based rejection was removed alongside D-A's refutation on 2026-04-21 — every editor in the Obsidian class accepts all file drops. Post-streaming (2026-04-22) there is no user-facing byte cap either — disk fullness (`storage-full` → 507) is the only rejection axis. The buffer-to-memory anti-pattern `upload.maxBytes` guarded is gone (see `reports/streaming-upload-refactor/REPORT.md` §D8). Post-2026-04-24 amendment the entire `upload.*` user-facing config is gone — every value is a module-level constant in `packages/core/src/constants/upload.ts`.

**sanitizeFilename (`api-extension.ts`).** Unicode-preserving: strip-on-sight first (path separators `/\\`, C0 controls, DEL), then whitelist via Unicode category classes (letters, numbers, marks, punctuation, emoji pictographs). Run-collapse `_+` → `_`, trim leading dots, strip trailing dots. Preserves non-ASCII filenames byte-for-byte while blocking path escape. Path-escape guards (`..`, absolute, `\x00`) run separately at request time. `linkTempToFinalWithCollisionRetry` (in `upload-streaming.ts`) preserves extension when adding collision suffixes.

**sha256 dedup.** Same-directory scope. Always on — no config knob. The streaming pipeline hashes during the download — `HashingPassThrough` emits the digest when the upload completes, so no separate pass over the bytes is needed. Server scans the destination directory for asset-extension siblings (via `ASSET_EXTENSIONS` from core), compares hashes, returns existing path on match and unlinks the tempfile. Cross-directory dedup is out of scope.

**Orphan tempfile sweep.** `cleanupOrphanUploadTempfiles(contentDir)` runs once at server boot (`standalone.ts` inside `initAsync()`, right after `recoverPendingManagedRename`). Default TTL is 24h — anything older than that is reaped with a log entry. Mid-upload crashes (SIGKILL, OOM, ENOSPC inside `pipeline`) can leave tempfiles behind; the boot sweep is the floor. Failure surfaces as `degraded.push('upload-tempfile-sweep')` — matches the shape of `managed-rename-recovery`.

**File watcher DiskEvent union.** The watcher emits two event families:
- **Markdown events** (`MarkdownDiskEvent`): `create | update | delete | rename | conflict` — carry `docName` + `content`, go through `classifyEvents` and the reconciliation loop.
- **Asset events** (`AssetDiskEvent`): `asset-create | asset-delete` — carry `relativePath` (not `docName`). Skip content reading and the self-write tracker; dispatch straight to basename-index maintenance + CC1 `ch:'files'`. Finder-style renames arrive as `asset-delete` + `asset-create` pairs; the basename-index add/remove pair is idempotent and produces the correct end state without a heavyweight rename-via-content-hash probe.

**Basename index runtime (FR-3b + FR-6).** Constructed in `standalone.ts` via `createBasenameIndex()` (from `@inkeep/open-knowledge-core/path-resolve`). Seeded at boot via `seedBasenameIndex` (`asset-walk.ts`) which walks the content directory using the `ContentFilter`'s dir-count (so assets only index if a markdown sibling admits the subtree). Asset DiskEvents flow through `handleDiskEvent` → `basenameIndex.add/remove` + `cc1Broadcaster.signal('files')`. The single `resolveEmbed(basename, sourcePath)` closure is threaded through `createApiExtension`, `createServerObserverExtension`, `setupServerObservers`, `createPersistenceExtension`, `applyExternalChange`, and `applyAgentMarkdownWrite` so every server-side mdast→PM dispatch resolves `![[...]]` embeds consistently. The Vite dev plugin (`packages/app/src/server/hocuspocus-plugin.ts`) manually reproduces the same surface — `createBasenameIndex()` + `seedBasenameIndex()` + the `resolveEmbed` closure + asset-event arms in the watcher callback — without going through `createServer()`. Unifying dev plugin + `createServer()` is known architectural debt tracked as a follow-on spec.

**Managed-rename behavior for refs (FR-7 / D-K).** When a markdown doc moves, `managed-rename-rewrite.ts`:
- **Rewrites** plain markdown image refs `![alt](relative-src)` via `readImageRef` + `recomputeRelativeImageHref` (posix `path.resolve` + relative). Absolute-path refs (`![alt](/docs/photo.png)`) and URL refs (`http(s):`, `data:`) are detected and left unchanged — they survive the move byte-for-byte.
- **Does NOT rewrite** `![[file.ext]]` wiki-embed refs. The basename index resolves them dynamically from the containing doc's new dirname, so the on-disk ref stays byte-identical. D-K LOCKED refs-only: assets do NOT follow the doc. Shared-asset breakage risk (one `logo.png` referenced by five docs) is the reason.

The paired-write marker `context.paired: true` on `MANAGED_RENAME_ORIGIN` ensures Observer A + Observer B both refresh their baseline in one atomic drain (precedent #1 extension).

**Client emit-dispatch matrix** (`packages/app/src/editor/image-upload/index.ts`): after a successful upload, `pickInsertShape(filename)` dispatches by extension against `WIKI_EMBED_EXTENSIONS` + `IMAGE_EXTENSIONS`:

| Extension                              | PM insert                                                     |
| -------------------------------------- | ------------------------------------------------------------- |
| `.md` / `.mdx` (first-class OK doc)    | `wikiLink` node — `{target: basename, alias, anchor}` (link)  |
| Image-ext in `WIKI_EMBED_EXTENSIONS`   | `wikiLinkEmbed` node — `{target: src}`                        |
| Non-image in `WIKI_EMBED_EXTENSIONS`   | `wikiLinkEmbed` node — `{target: src}`                        |
| Opaque (zip/docx/txt/…)                | text + `link` mark — `{href: relPath}`                        |

`DEFAULT_EMIT_FORMAT = 'wikiembed'` is the only shape. The `'markdown-image'` branch of `pickInsertShape` is typed but unreachable by construction (never called with non-default emit); it stays as a compile-time pattern for future spec work. `shortestImageRef(assetPath, mdPath)` returns the minimal relative reference under four cases: same-dir → basename; asset in ancestor of doc dir → `../<name>`; asset in subtree of doc dir → `./<sub>/<name>`; cross-tree → `../…/<name>`. (Matches the JSDoc at `packages/app/src/editor/image-upload/index.ts:101-110` and the dirname-matrix tests — the ancestor direction emits `../`, descent emits `./<sub>/...`.)

**Client click-dispatch matrix** (`packages/app/src/editor/asset-dispatch/dispatcher.ts`, SPEC 2026-04-23 amendment): after a user clicks an on-disk reference, `dispatchAssetClick(ctx)` routes in this order:

| Step | Check | Action |
|------|-------|--------|
| 1 | `ctx.forceOsDelegation` (Cmd/Ctrl/middle-click) | Skip registry (D-A6 escape hatch) |
| 2 | `assetViewerRegistry.lookup(ctx.ext)` | Invoke viewer.render(ctx) if registered |
| 3 | `window.okDesktop` present (Electron) | `shell.openAsset(projectRelPath)` via IPC |
| 4 | Otherwise (web) | `openHashHrefInNewTab(ctx.url)` (`window.open`) |

Registry is empty at landing (D-A11); follow-up PRs register PDF.js / image lightbox / video-audio viewers. Callers (`internal-link.ts handlePrimary`, `InternalLinkPropPanel handleNavigate`, `plugins/asset-context-menu.ts`) all build the `AssetClickContext` from the clicked href via `classifyMarkdownHref` + `resolveAssetProjectPath` — the path validation + blocklist enforcement fires at the main-process IPC handler (`openAssetSafely`), not at the renderer. Cmd/Ctrl+click routes the same way but sets `forceOsDelegation: true` to bypass a registered viewer; standard browser muscle memory preserved.

Right-click on any on-disk ref (asset chip, wiki-link chip, image) goes through `plugins/asset-context-menu.ts` → `window.okDesktop.shell.showAssetMenu({relPath, title, kind})` → main-process `popAssetMenu` builds a native `Menu.buildFromTemplate` with Reveal + Open + Copy link. Same path-containment gate via `openAssetSafely` / `revealAssetSafely`. In web (no `okDesktop`), the plugin is a no-op — Chromium's default context menu surfaces.
...
| POST   | `/api/upload`                 | Upload an asset (multipart, streamed to disk). Response: `{ok, src, path, deduped}`. Accept-all under D-M; rejection axes are all server-side (malformed multipart, disk full, etc. — see `UploadWriteReason`). |
...
- `src/file-watcher.ts` — `startWatcher()` + writeTracker; emits `DiskEvent = MarkdownDiskEvent | AssetDiskEvent` (markdown: `create | update | delete | rename | conflict`; asset: `asset-create | asset-delete`). Markdown events carry `docName`+`content`; asset events carry `relativePath` and skip the reconcile loop.
- `src/asset-walk.ts` — `seedBasenameIndex()` walks `contentDir` after `startWatcher` primes the `ContentFilter` dir-count so sibling-asset admission is correct; bounded by `visitedInodes` to prevent symlink cycles.
- `src/managed-rename-rewrite.ts` — image-ref branch via `readImageRef` + `recomputeRelativeImageHref`; wiki-embed refs untouched (basename index resolves dynamically per D-K).
...
Never call `ipcMain.handle` / `ipcRenderer.invoke` directly. Use `createHandler` / `createInvoker` from `packages/desktop/src/shared/ipc-*.ts`. Enforcement is a Bun integration test (`packages/desktop/tests/integration/no-loosely-typed-webcontents-ipc.test.ts`) — Biome 2.4 doesn't ship custom lint plugins yet, so the test is the source of truth (see its header comment for context). The test's `ALLOWLIST` constant enumerates the wrapper-implementation files permitted to call raw electron IPC primitives — consult the test file directly when extending or auditing rather than relying on a list duplicated here.

**Shell channels (2026-04-23 asset-click dispatcher addition).** Three new channels join the existing `shell.*` surface: `ok:shell:open-asset` (→ `openAssetSafely` → `shell.openPath`), `ok:shell:reveal-asset` (→ `revealAssetSafely` → `shell.showItemInFolder`), `ok:shell:show-asset-menu` (→ `popAssetMenu` → `Menu.buildFromTemplate`). Each is typed via `RequestChannels` with a structured result envelope (`{ok:true} | {ok:false, reason:…}` for open/reveal). The bridge-contract drift-guard (`m1-smoke.test.ts`) requires all three to appear symmetrically across `packages/core/src/desktop-bridge.ts` + `packages/desktop/src/shared/bridge-contract.ts` + `packages/app/src/lib/desktop-bridge-types.ts`. Don't add a new `shell.*` verb without (a) adding it to `RequestChannels`, (b) mirroring across the three bridge files, (c) updating `REQUIRED_SHELL_MEMBERS` in the drift-guard test.
...
- **STOP:** Do not re-add a MIME allowlist gate to the upload handler (`/api/upload`). D-M LOCKED accept-all: every file is accepted — post-streaming (2026-04-22) there is no user-facing byte cap either. The old `ALLOWED_MIME_TYPES.has(detectedMime)` reject path was removed alongside D-A's refutation on 2026-04-21; every Obsidian-class editor accepts all file drops, and a rejection gate re-introduces the "Unsupported file type" dead-end. A security-focused hard-block allowlist is documented in SPEC §15 Future Work → Explored and requires a multi-tenant trigger to revisit. Operators who want a narrow allowlist today do it at the network boundary, not inside the editor.
- **STOP:** Do not re-introduce `upload.maxBytes` or any buffer-to-memory upload pattern. The handler reads multipart bytes via `stream.pipeline(busboy, HashingPassThrough, fs.createWriteStream(tempPath))` — memory is O(1) regardless of file size. The buffer-to-memory `chunks: Buffer[]` + `Buffer.concat(chunks)` anti-pattern it replaced (pre-2026-04-22) forced a user-facing `upload.maxBytes` cap purely as an OOM guard. Streaming eliminates the rationale; the field was deleted from `UploadConfig`, `DEFAULT_UPLOAD_CONFIG`, the Zod schema, the `/api/upload-config` response, and the client byte-size toast. Reintroducing either (the cap, or the buffer) re-creates the OOM risk and dresses it up as a product choice. If a future product need demands an explicit cap (multi-tenant quota, bandwidth metering), design it as a separate feature with its own spec — not as a revival of the pre-streaming pattern. See `reports/streaming-upload-refactor/REPORT.md` §D8 and `packages/server/src/upload-streaming.ts`.
- **STOP (2026-04-24 amendment):** Do not re-introduce the `upload.*` user-facing config surface. `ConfigSchema` has no `upload` section; `UploadConfig` interface, `DEFAULT_UPLOAD_CONFIG`, `PartialUserUploadConfig`, `resolveUploadConfig`, and `detectObsidianVault` were all deleted in the 2026-04-24 amendment. Every value lives as a module-level constant in `packages/core/src/constants/upload.ts` (`DEFAULT_ATTACHMENT_FOLDER_PATH`, `DEFAULT_EMIT_FORMAT`, `DEFAULT_DEDUP_MODE`, `DEFAULT_DEDUP_UI`, `WIKI_EMBED_EXTENSIONS`). Re-adding any knob requires a spec with a concrete user request — the "user-first precedence" apparatus (`.optional()` fields + structural guards + user/vault/default merger + `/api/upload-config` endpoint) has been shown to be overspec. See `specs/2026-04-16-editor-asset-and-embed-surface/SPEC.md` §Post-finalization amendment (2026-04-24). Similarly, do NOT re-add a runtime reader of `.obsidian/app.json` — the correct shape for Obsidian-refugee onboarding is the future one-shot `ok migrate --from-obsidian-vault` CLI, not runtime coupling to a proprietary closed-source schema.
- **STOP:** Do not remove the SVG extension-fallback at `packages/server/src/api-extension.ts` (`handleUploadImage` — the `<svg` text-sniff block). `file-type` can't detect SVG (text-based, no magic bytes). This fallback is LOAD-BEARING for NFR-3 — SVG must render via `<img>`, never inline DOM. If a future change replaces `file-type` with a detector that handles SVG natively, the fallback can go away with the replacement; never remove it in isolation.
- **STOP:** Do not rewrite absolute-path image refs (`![alt](/docs/photo.png)`) or URL refs (`http(s):`, `data:`) during managed-rename. `readImageRef` + `recomputeRelativeImageHref` (`packages/server/src/managed-rename-rewrite.ts`) MUST detect these and leave them byte-identical on doc move. Reference-fixture tests cover a pre-existing absolute-path ref surviving a rename unchanged; any regression that rewrites them breaks SPEC FR-7 and silently breaks shared-across-branch diffs.
- **STOP:** Do not rewrite `![[file.ext]]` wiki-embed refs during managed-rename. D-K LOCKED refs-only: assets do NOT follow the doc. The basename index resolves wiki-embed references dynamically from the containing doc's dirname, so a moved doc's refs continue to resolve without a rewrite pass. Adding asset-relocation requires re-opening D-K with evidence of concrete user demand (SPEC §10 D-K revisit trigger: 12-month orphan-asset density audit paired with an `openknowledge gc` CLI — both Future Work).
- **STOP — wikiLinkEmbed schema is add-only (precedent #9 applied to a specific type).** The `wikiLinkEmbed` PM node and mdast type were introduced in this spec run. Any future attr (`size`, `anchor`, modifiers like `|640x480`) is an ADD; never remove `target` / `alias` / `anchor` or narrow their defaults. The server-side mdast→PM reverse path checks `sourceForm === 'wikiembed'` on `image` / `link` nodes to re-emit atomic `wikiLinkEmbed` mdast — removing the `sourceForm` attr from `ImageFidelity` / `LinkFidelity` would silently break round-trip for every existing `![[...]]` in every vault. Precedent #9 (y-prosemirror destructive-delete) applies: a narrowing change would tombstone Y.Items on load across all peers.
- **STOP:** Server-side mdast→PM MUST NOT emit a PM `wikiLinkEmbed` node. Post-round-trip the shape is always PM `image` (image-ext) or PM text+link (non-image / opaque). The PM `wikiLinkEmbed` node exists only as a transient client drop-time shape between `pickInsertShape` and the first save. A server-side emitter would round-trip through y-prosemirror and tombstone the user-content Item. Regression gate: `packages/app/tests/fidelity/invariant-wiki-link-embed-conversion.test.ts` I7 asserts two paths (hand-authored parse vs. drop-emitted serialize-then-parse) converge on the same PM shape.
- **STOP:** There are exactly two `shell.openPath` call sites in the main process — `openAssetSafely` (`packages/desktop/src/main/asset-allowlist.ts`) and the `ok:shell:open-asset` handler registration (`packages/desktop/src/main/index.ts`) that delegates to it. Do NOT add a third. `shell.openPath` dispatches to the OS content handler by extension — every unguarded call site is a potential RCE vector (Joplin/Obsidian/Zettlr research in `reports/electron-os-integration-patterns/` D5 catalogues this class). Any new renderer-initiated "open this file on disk" surface must route through the existing `ok:shell:open-asset` IPC → `openAssetSafely` three-check gate (containment + existence + `EXECUTABLE_BLOCKLIST_EXTENSIONS`). The right-click menu's "Open in default app" action is the only consumer today; if a future feature needs another path, extend the IPC channel OR add a sibling handler that reuses `openAssetSafely`. Don't call `shell.openPath` directly.
- **STOP:** All asset click interception MUST go through `createMarkInteractionBridgePlugin` (for `link` marks) or `createAssetContextMenuPlugin` (for contextmenu) — NEVER `handleClickOn` / `handleDOMEvents` on a ProseMirror plugin (precedent #19(b)). The renderer dispatcher (`dispatchAssetClick`) is the single entry point for "click on asset fired"; every bridge / plugin routes there. Adding a second click-intercept mechanism fragments the routing surface and re-introduces the Gap 3b / Gap 4 failure mode class (pre-2026-04-23 the mark path and drop-time path had independent click handling; now they share `dispatchAssetClick`).
...
- **WARN:** Obsidian vault detection was removed in the 2026-04-24 SPEC amendment (see `specs/2026-04-16-editor-asset-and-embed-surface/SPEC.md` §Post-finalization amendment). There is no runtime coupling to `.obsidian/app.json` — OK never reads it at startup. Refugees whose vault uses non-default `attachmentFolderPath` or `useMarkdownLinks: true` wait for a future one-shot `ok migrate --from-obsidian-vault` CLI (separate spec). Do not re-add `detectObsidianVault` or any runtime parser of `.obsidian/app.json` to the CLI / Vite dev plugin / desktop boot paths without opening a new spec: runtime coupling to a proprietary closed-source schema is architectural debt, and the migration-tool shape is the established correct pattern.
- **WARN:** Opaque-extension round-trip is lossy by design (SPEC §6 FR-1a emit-dispatch matrix). `![[archive.zip]]` hand-authored into markdown parses to a `wikiLinkEmbed` mdast, dispatches to PM text+`link` mark without `sourceForm`, and serializes back to `[archive.zip](archive.zip)` — byte-NOT-identical. The wiki-embed extension list is fixed (`WIKI_EMBED_EXTENSIONS` in `packages/core/src/constants/upload.ts`); widening it requires a spec with a concrete user request. Covered by a dedicated deviation test in `packages/app/tests/fidelity/invariant-wiki-link-embed-conversion.test.ts`; I1/I4 invariants are scoped to renderable extensions.
- **WARN:** The Vite dev plugin (`packages/app/src/server/hocuspocus-plugin.ts`) does NOT route through `createServer()` — it manually wires Hocuspocus + persistence + session manager + CC1 broadcaster + api/observer extensions. Asset-embed feature parity (basename index + `resolveEmbed` closure + `seedBasenameIndex` at startup + asset-event arms in the disk-bridge callback) is manually maintained; any change to `standalone.ts`'s extension wiring must be mirrored here to keep dev parity. Full unification of dev plugin + `createServer()` is still pending (architectural debt tracked as a follow-on spec). Shadow repo is disabled when `OK_TEST_CONTENT_DIR` is set (timeline features unavailable in isolated test runs).
...
- **NG12:** `![[file.ext]]` wiki-embed round-trip is byte-identical for extensions in the fixed `WIKI_EMBED_EXTENSIONS` constant (images + pdf + mp4 + mp3 + wav + ogg + …); opaque extensions (`.zip`, `.docx`, arbitrary) hand-authored as `![[archive.zip]]` normalize to `[archive.zip](archive.zip)` markdown-link on first save. This is the SPEC §6 FR-1a matrix's stated deviation. Widening the extension list requires a new spec with a concrete user request — it is no longer an operator knob post-2026-04-24 amendment. Similarly, embed size modifiers (`![[image.png|640x480]]`) are not parsed today; the `|modifier` suffix would pass through the micromark tokenizer as alias text.
...
- **Tier C (custom):** link/linkReference, definition (R12 override), html, MDX nodes, wikiLink, wikiLinkEmbed

**wikiLinkEmbed mdast→PM dispatch** (`handlers.wikiLinkEmbed` in `markdown/index.ts`): image extension → PM `image` node with `attrs.sourceForm = 'wikiembed'`; non-image wikiembed extension → PM text with `link` mark carrying `sourceForm = 'wikiembed'`; opaque extension → PM text with plain `link` mark (no `sourceForm`). Resolved `src` comes from `parseCtx.current.resolveEmbed(target, sourcePath)` when provided, else the literal target. **Server-side mdast→PM NEVER emits a PM `wikiLinkEmbed` node post-round-trip** — that PM type exists only client-side as a transient drop-time shape. The reverse path (`nodeHandlers.image` + `markHandlers.link`) checks `sourceForm === 'wikiembed'` and re-emits atomic `wikiLinkEmbed` mdast; plain markdown images/links without the marker serialize as `![alt](src)` / `[text](href)`. `ParseContext` is a module-level holder (`{current}`) that `mdManager.parse(md, opts)` sets before the pipeline runs and clears in `finally`. Opaque-extension round-trip is lossy by design: `![[archive.zip]]` normalizes to `[archive.zip](archive.zip)` on first save.
...
- `packages/core/src/markdown/wiki-link-micromark.ts` — micromark tokenizer for `[[Page]]` wiki-links AND `![[file.ext]]` wiki-embeds. Dedicated construct keyed off `CODE_BANG` (33) registered in the `text` map so `!` prefix enters a distinct state machine that emits `wikiLinkEmbed` mdast (separate from the `wikiLink` emitter). Both state machines share the inner target/anchor/alias token shape; compile handlers dispatch on the outer token. `MICROMARK_EXT` stays a module-level singleton (precedent #15 idempotent attacher).
