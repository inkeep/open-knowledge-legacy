...
**Port sentinel.** `port: 0` means "starting, not yet bound." CLI/Vite callers invoke `updateProcessLockPort({lockName, lockDir, port})` (or the typed `updateServerLockPort({lockDir, port})` / `updateUiLockPort({lockDir, port})` adapters) after `http.listen()` resolves so downstream tools (MCP, preview-url) read the real port. Mutation is ownership-guarded — a process whose pid does not match the lock's refuses to rewrite.
