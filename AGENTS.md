...
- `src/editor/extensions/mark-identity-decoration.ts` — Materializes `markIdentityPlugin`'s IDs as `data-mark-id` inline decorations (required because mark `renderHTML` runs before `appendTransaction` assigns IDs). Event delegation reads `closest('[data-mark-id]')`.
...
- `src/editor/extensions/link-resolution-decoration.ts` — Emits `data-resolution-state` decorations for tracked link marks. Subscribes to page-list-cache; dispatches a self-keyed meta transaction on cache changes to force PM to re-run `props.decorations`. Stacks with `mark-identity-decoration` (disjoint attr keys).
