...
- `src/constants/upload.ts` — `UploadConfig` / `EmitFormat` / `DedupMode` / `DedupUIMode` type surface; `DEFAULT_UPLOAD_CONFIG` (single source of truth for the resolved `upload.*` subtree; CLI Zod schema mirrors via structural `_ResolvedFieldsMatch` compile-time check at `packages/cli/src/config/schema.ts:93-106` — the check `Omit`s `attachmentFolderPath` and `emitFormat` because those two fields are intentionally `.optional()` per US-018 user-wins precedence); `ASSET_EXTENSIONS` (ContentFilter admit-set: images + PDF + video + audio + fonts + zip); `ALLOWED_IMAGE_MIME_TYPES` retained as a reference list for any future MIME/extension mapping helper (no runtime gate consumer post-D-M)
...
├── Obsidian Vault Detect (non-destructive `.obsidian/app.json` read at boot)
...
**sha256 dedup (FR-2).** Same-directory scope. The streaming pipeline hashes during the download — `HashingPassThrough` emits the digest when the upload completes, so no separate pass over the bytes is needed. Server scans the destination directory for asset-extension siblings (via `ASSET_EXTENSIONS` from core), compares hashes, returns existing path on match and unlinks the tempfile. `upload.dedup.mode: 'off'` bypasses. Cross-directory dedup is NG1 — not in scope.
...
**Basename index runtime (FR-3b + FR-6).** Constructed in `standalone.ts` via `createBasenameIndex()` (from `@inkeep/open-knowledge-core/path-resolve`). Seeded at boot via `seedBasenameIndex` (`asset-walk.ts`) which walks the content directory using the `ContentFilter`'s dir-count (so assets only index if a markdown sibling admits the subtree). Asset DiskEvents flow through `handleDiskEvent` → `basenameIndex.add/remove` + `cc1Broadcaster.signal('files')`. The single `resolveEmbed(basename, sourcePath)` closure is threaded through `createApiExtension`, `createServerObserverExtension`, `setupServerObservers`, `createPersistenceExtension`, `applyExternalChange`, and `applyAgentMarkdownWrite` so every server-side mdast→PM dispatch resolves `![[...]]` embeds consistently. The Vite dev plugin (`packages/app/src/server/hocuspocus-plugin.ts`) manually reproduces the same surface — `createBasenameIndex()` + `seedBasenameIndex()` + the `resolveEmbed` closure + asset-event arms in the watcher callback + `detectObsidianVault()` + `resolveUploadConfig()` — without going through `createServer()`. Unifying dev plugin + `createServer()` is known architectural debt tracked as a follow-on spec; US-018 honors feature parity for the asset-embed surface in both paths.

**Obsidian vault detection (FR-4).** `detectObsidianVault(contentDir)` in `obsidian-vault-detect.ts` reads `.obsidian/app.json` when present, maps three fields to a `Partial<UploadConfig>`: `attachmentFolderPath` (1:1 passthrough per D-J free-form string — `"/"`, `"./"`, `"./subdir"`, bare name all preserved verbatim); `useMarkdownLinks: boolean → emitFormat: 'markdown-image' | 'wikiembed'`; `newLinkFormat` surfaced but not consumed (Foam-style shortest is the OK default). Symlink-escape guarded via `realpath`-vs-boundary. Read-only — never writes to `.obsidian/`. Called in `packages/cli/src/commands/start.ts` and `packages/app/src/server/hocuspocus-plugin.ts`; both paths pass the vault partial through `resolveUploadConfig()` (from `@inkeep/open-knowledge-core/resolve-upload-config`). Precedence is **user > vault > default** (US-018): any explicit value in `.open-knowledge/config.yml` wins over `.obsidian/app.json`, and vault fills in only where the user was silent. This works because `upload.attachmentFolderPath` and `upload.emitFormat` are `.optional()` on the Zod schema (no default materialized), so "user didn't set it" is observably `undefined`. Malformed JSON → structured WARN log (`{event: 'obsidian-vault-detect', reason}`) + fall through to defaults.
...
| GET    | `/api/upload-config`          | Resolved `upload.*` subtree for client emit-dispatch (`emitFormat`, `wikiEmbedExtensions`, `dedup.ui`, `attachmentFolderPath`). |
...
- `src/obsidian-vault-detect.ts` — `detectObsidianVault(contentDir)` — read-only `.obsidian/app.json` parser. Returns `Partial<UploadConfig>` (maps `useMarkdownLinks` → `emitFormat`, 1:1 passthrough of `attachmentFolderPath`) or `null` on missing/malformed/symlink-escape.
...
- **WARN:** Obsidian vault detection is **user-first** on the two fields it knows about (US-018, updated from the pre-US-018 vault-first posture). `resolveUploadConfig` in `@inkeep/open-knowledge-core` merges `user ?? vault ?? default` — an explicit `upload.attachmentFolderPath` in `.open-knowledge/config.yml` wins over `.obsidian/app.json`; the vault only fills in where the user was silent. This works because `upload.attachmentFolderPath` and `upload.emitFormat` are `.optional()` on the Zod schema (no default materialized), so `config.upload.<field> === undefined` is a reliable "user didn't set it" signal. If a user hand-edits `.open-knowledge/config.yml` to pin `upload.attachmentFolderPath` AND the project has an `.obsidian/app.json` with a different path, the user's value wins. Both `packages/cli/src/commands/start.ts` (`ok start`) and `packages/app/src/server/hocuspocus-plugin.ts` (`bun run dev`) invoke `resolveUploadConfig` on the boot path so the precedence is identical across the two entry points.
- **WARN:** Opaque-extension round-trip is lossy by design (SPEC §6 FR-1a emit-dispatch matrix). `![[archive.zip]]` hand-authored into markdown parses to a `wikiLinkEmbed` mdast, dispatches to PM text+`link` mark without `sourceForm`, and serializes back to `[archive.zip](archive.zip)` — byte-NOT-identical. Users who want a specific extension to round-trip as `![[...]]` add it to `upload.wikiEmbedExtensions`. Covered by a dedicated deviation test in `packages/app/tests/fidelity/invariant-wiki-link-embed-conversion.test.ts`; I1/I4 invariants are scoped to renderable extensions.
- **WARN:** The Vite dev plugin (`packages/app/src/server/hocuspocus-plugin.ts`) does NOT route through `createServer()` — it manually wires Hocuspocus + persistence + session manager + CC1 broadcaster + api/observer extensions. US-018 added asset-embed feature parity (basename index + `resolveEmbed` closure + `seedBasenameIndex` at startup + asset-event arms in the disk-bridge callback + `detectObsidianVault` + `resolveUploadConfig` + `/api/upload-config`) so `bun run dev` and `ok start` produce identical behavior for the upload + embed surface. The full unification of dev plugin + `createServer()` is still pending (architectural debt tracked as a follow-on spec); until that lands, any change to `standalone.ts`'s extension wiring must be mirrored here to keep dev parity. Shadow repo is still disabled when `OK_TEST_CONTENT_DIR` is set (timeline features unavailable in isolated test runs).
...
- **NG12:** `![[file.ext]]` wiki-embed round-trip is byte-identical for extensions in `upload.wikiEmbedExtensions` (images + pdf + mp4 + mp3 + wav + ogg + …); opaque extensions (`.zip`, `.docx`, arbitrary) hand-authored as `![[archive.zip]]` normalize to `[archive.zip](archive.zip)` markdown-link on first save. This is the SPEC §6 FR-1a matrix's stated deviation; operators widen the allowlist via `upload.wikiEmbedExtensions` if they need wiki-embed shape preserved for a custom extension. Similarly, embed size modifiers (`![[image.png|640x480]]`) are not parsed today (SPEC NG12 — not in scope); the `|modifier` suffix would pass through the micromark tokenizer as alias text.
