...
| GFM table | — | — | ✓ delimiters | **Negative AC**: reads as plain paragraph text |
...
### GFM tables — negative AC

Table lines MUST render as plain paragraph text per §6.6. No row tint, no cell bands, no accent bar, no compact sizing.

| Column A | Column B | Column C |
|----------|----------|----------|
| alpha    | beta     | gamma    |
| one      | two      | three    |
| a long cell with more text | short | medium-length |

Inspect any table line — its class list should be `cm-line` (plus CM6's `.ͼ*` highlight classes for the pipe characters) and nothing else from this spec.
...
- Table row tint / cell bands / accent bar / compact sizing
