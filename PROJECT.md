...
| TQ25 | Source toggle sync: bidirectional observer sync between Y.XmlFragment and Y.Text | Technical | P0 | Open | **Next spike.** Add Y.Text('source') to Y.Doc alongside Y.XmlFragment('default'). Bind CodeMirror to Y.Text via y-codemirror.next. Bidirectional observers: XmlFragment→Text (serialize on WYSIWYG change) and Text→XmlFragment (parse + updateYFragment on source change). Transaction origin guards prevent infinite loops. **Research findings:** shimmer does not occur (3 independent prevention mechanisms confirmed by source-code analysis — see /reports/yjs-dual-key-shimmer-analysis/). y-codemirror.next binding internals traced (see /reports/yjs-constrained-observer-sync/). Both Y.Types coexist in same Y.Doc with zero Hocuspocus config changes. **Not yet validated in practice.** The shimmer analysis is based on source code tracing, not empirical testing. The spike must confirm: shimmer doesn't occur under real editing, observer performance is acceptable at document sizes up to 50KB, void nodes (jsx-component fenced code blocks) round-trip correctly through the observer cycle. See specs/next-sync-explorations.md (test matrix: 103 scenarios). |
| TQ26 | Disk↔CRDT bridge for external editor interop (Cursor, VS Code) | Technical | P0 | Open | **Next spike (independent of TQ25).** @parcel/watcher monitors content directory. External file changes → parse markdown → updateYFragment into Y.Doc. Content-hash feedback loop prevention (distinguish our own persistence writes from external editor saves). **Research findings:** @parcel/watcher delivers events in 2-52ms on macOS. Content-hash handles 4/5 race scenarios; the 5th (concurrent CRDT+disk write within 50ms coalescing window) is a genuine concurrent conflict needing three-way merge. See /reports/parcel-watcher-crdt-disk-bridge/. **Not yet implemented or validated.** |
