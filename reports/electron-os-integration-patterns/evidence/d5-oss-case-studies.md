...
| Property | Value |
|---|---|
| Interception | Main process. `src/electron/electron/handler.cljs:63-66` defines `:openFileInFolder` IPC handler via `defmethod`. No `will-navigate` in `core.cljs` |
| Call sites | `handler.cljs:63-66` (`.showItemInFolder`). `core.cljs:253, :260, :290` — `shell.openExternal` to hardcoded docs.logseq.com URLs |
| Path validation | `utils/to-native-win-path!` normalizes to platform format; `when-let` short-circuits on nil |
| Context menu | `core.cljs:set-app-menu!` via Electron's template API. Asset-click context menu is renderer-side (ClojureScript) |
| `shell.openPath` usage | **NOT FOUND in main-process sources read** |

**Notable pattern:** Community reports MIME/default-app bugs ([discuss.logseq.com/t/6203](https://discuss.logseq.com/t/clicking-on-asset-link-such-as-a-pdf-doesnt-open-the-pdf-with-default-app/6203)) suggesting the Electron build *does* delegate somewhere — but the site was not located in the main tree. **UNCERTAIN** on precise call path.

Source read depth: spot-check
...
| Property | Value |
|---|---|
| Interception | Main-process. `packages/desktop/app/javascripts/Main/Window.ts:91` (`setWindowOpenHandler`) and `:99` (`on('will-navigate')`) |
| Call sites | `Window.ts:93` and `:105` — both `shell.openExternal(url)`. **No `shell.openPath` found** |
| Path validation | Both call sites pass `url` through unchanged. Whether a scheme allowlist exists upstream was not verified first-hand. **UNCERTAIN** |
| Context menu | Renderer-side, not visible in `Main/` |
| `shell.openPath` usage | Not found |

**Notable pattern:** **Classic two-handler pattern** — `setWindowOpenHandler` + `will-navigate` both redirecting to `shell.openExternal`. The pattern Electron docs recommend.

Source read depth: spot-check (scheme validation UNCERTAIN)
