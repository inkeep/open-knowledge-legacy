...
2. Exposes 8 tools to the AI client (read, write, edit, list, search, undo, redo, update\_frontmatter)
...
| S7     | MCP stdio server                    | Product     | `open-knowledge mcp` starts stdio MCP server. Connects to running Hocuspocus. 8 tools work (read, write, edit, list, search, undo, redo, update\_frontmatter).       |
...
- `edit_document` (surgical find-replace, supports dry\_run)
...
- `packages/app/` — new package, React editor + presence + design system + Vite plugin (moved from init\_spike)
