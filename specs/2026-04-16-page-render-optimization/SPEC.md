...
- **EditorActivityPool** (`packages/app/src/components/EditorActivityPool.tsx`): iterates `pool.entries` and renders an `<Activity>` per entry. React 19.2's Activity mounts/unmounts effects based on mode, preserving children's state in the hidden mode.
  ...
  **Scroll preservation requires per-Activity scroll containers** (discovered during Phase 7 QA; fix in commit `b3fdbcd8`). `<Activity mode="hidden">` unmounts the hidden subtree's DOM. If the scroll container lives ABOVE the Activity boundary (wrapping all mounts), `scrollHeight` collapses when the active Activity goes hidden; the browser auto-clamps `scrollTop` to `0`; on return the DOM re-inflates but React does not restore the scroller state. The fix: each Activity entry wraps its editor(s) in a `ScrollPreservingContainer` that (a) captures `scrollTop` via a scroll listener — `display:none` zeroes `scrollTop` before any layout effect can read it, so scroll-listener capture is the reliable timing — and (b) restores via a layout effect + `ResizeObserver` that retries until the subtree has grown tall enough to honor the target `scrollTop`. `<Activity>` preserves React state; per-mount scroll containers preserve DOM scroll state. Both are required for G5.
