...
status: Draft (RHF Harness Amendment — 2026-04-30)
...
> **2026-04-30 RHF Harness Amendment.** FR-3, FR-5, FR-37 amended; FR-41/42/43 added; D64/65/66 added in §10. The L1 client / L2 headless / L3 persistence-hook validation topology (FR-11, D45) is unchanged. Settings pane's L1 surface adopts `react-hook-form` v7 + the shadcn `Form` primitive (`useForm` resolver-less per D64; `binding.subscribe → form.reset(next, {keepDirtyValues: true})` per D65; section list hand-written per D66). `folders[]` becomes editable via `useFieldArray` with a new `TagPillInput` primitive. Design rationale: [`evidence/rhf-binding-harness.md`](evidence/rhf-binding-harness.md). Two-PR rollout: PR1 = harness parity refactor (no behavior change); PR2 = `folders[]` section.
...
| Must | **FR-3**: Auto-save with per-control commit (matches VS Code Settings UI) | Text inputs commit on blur or Enter; booleans/selects commit on change; no Save button | `ConfigBinding.patch()` fires after L1 client-side validation passes (D8 + D45). Implementation: `useForm` from `react-hook-form` v7 acts as the state harness; per-field commit fires through `binding.patch(buildPatch(name, value))` on `Controller.onBlur`. Resolver-less (per D64) — `bindConfigDoc.patch`'s safeParse remains the single L1 gate; rejection issues mirror to `form.setError(name, …)` for inline display via shadcn `<FormMessage>`. |
...
| D64 | **Settings pane runs RHF resolver-less.** `useForm({ defaultValues, mode: 'onBlur' })` with no `resolver: zodResolver(...)`. `bindConfigDoc.patch` is the L1 safeParse; per-field errors mirror via `form.setError(name, …)` from the rejection branch. | T | LOCKED | Yes (1-way: validation topology becomes part of the FR-11 contract — adding a resolver later would re-introduce a second L1 safeParse) | Two safeParses against the same schema is a redundant L1 and contradicts FR-11's "single safeParse run at three entry points; uniform mechanism." Cross-field constraints (e.g. `preview.baseUrl` URL + scope-as-constraint per FR-32) need the merged document, which `binding.patch` already produces; a resolver would operate on form sub-trees. **Alternatives rejected:** (a) `resolver: zodResolver(ConfigSchema)` — redundant L1 + sub-tree validation. (b) Standard-schema resolver — same rejection rationale. | `evidence/rhf-binding-harness.md` §"Decision: validation runs in the binding, not the resolver"; user direction 2026-04-30 | `useConfigForm(binding)` instantiates `useForm` without a `resolver`; `commitField` calls `binding.patch` then mirrors `Result.err` into `form.setError`. `@hookform/resolvers` package stays installed (FR-43) for parity with agents-private but is not engaged. |
| D65 | **External Y.Text updates merge into form state via `form.reset(next, { keepDirtyValues: true, keepDirty: true, keepTouched: true })`** from a `binding.subscribe` listener. Replaces the per-control `lastCommittedRef === pending` heuristic. | T | LOCKED | No (additive — could revisit if dirty-tracking semantics change in RHF) | RHF's per-field dirty-tracking is more robust than per-control-instance heuristics; `keepDirtyValues: true` is the library-supported semantic for "remote updates land on non-dirty fields, leave dirty fields alone." **Alternatives rejected:** (a) Per-control heuristic continued — works but lossy if a field has multiple render sites. (b) Always reset, ignore dirty — would stomp user-in-progress edits. | `evidence/rhf-binding-harness.md` §"Decision: external Y.Text updates merge via keepDirtyValues" + RHF v7 `form.reset` API contract | `useConfigForm` `useEffect` registers `binding.subscribe(next => form.reset(next, { keepDirtyValues: true, keepDirty: true, keepTouched: true }))`. New integration test exercises rapid-update mid-edit to guard against a `keepDirtyValues` race. |
| D66 | **Section list (`SECTIONS` in `SettingsPane.tsx`) stays hand-written in v1.** Auto-generation from a `fieldRegistry`-extended metadata layer (label/description/section/order) is deferred. **Revisit if** a second schema-driven UI surface emerges that would justify generalizing — at that point the registry extension and walker pass both serve N=2. | P | NOT NOW | No (additive — auto-generation can layer on later without behavior change) | (a) Agents-private also hand-writes field lists — pattern parity. (b) Auto-generation commits us to a UI metadata layer (labels, descriptions, ordering, control hints) that we'd own and version; with N=1 consumer, the abstraction is premature. (c) The schema walker's introspection primitives (`getLeafTypeTag` etc.) already cover control dispatch — the only thing hand-written is *labels and ordering*, which barely benefit from generalization. **Alternative rejected:** auto-generation now (rejected as N=1 generalization). | `evidence/rhf-binding-harness.md` §"Decision: section list stays hand-written"; agents-private form patterns | `SECTIONS` array in `SettingsPane.tsx` continues to enumerate fields with `{label, description, path}` tuples; render path uses the existing schema walker for control dispatch. Future PR can extend `fieldRegistry` with `{label, description, section, order}` metadata once a second consumer exists. |
