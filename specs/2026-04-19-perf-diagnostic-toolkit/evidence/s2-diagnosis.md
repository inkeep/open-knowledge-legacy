...
- **Correction (2026-04-20 post-US-007 dual-validation):** TipTap provides FIRST-CLASS APIs for this exact case — `@tiptap/core/src/Editor.ts:161` exposes `public mount(el)` documented for "when you want to mount the editor to an element that is not yet available in the DOM" and later re-mount to another element; `:190` exposes `public unmount()` preserving editor state. `@tiptap/react/src/EditorContent.tsx:155` carries the upstream TODO: `// TODO using the new editor.mount method might allow us to remove this`. The original s2-diagnosis prose framed the V2 cache as "a downstream workaround" and cited [TipTap #5761](https://github.com/ueberdosis/tiptap/issues/5761) as "closed wontfix 2024-08 proving editor hot-swap is unsupported." BOTH claims are wrong: `gh issue view 5761` confirms the issue was closed as **COMPLETED on 2025-04-18**, and its title is "In TiptapCollabProvider on name change, the content of the Editor doesn't change" — about **provider hot-swap** (different concern), not editor persistence. The V2 cache is therefore building on top of intentional, maintained APIs — not working around a rejected design. Maintenance cost on upstream bumps is proportional to the `mount()`/`unmount()` signature stability, which is first-class and surfaced in the public `Editor` class.
