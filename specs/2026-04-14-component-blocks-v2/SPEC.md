...
- **[PRE-EXISTING, NOT-THIS-PR] NG15:** Markdown round-trip fidelity gaps on document load. Opening an existing markdown file in the browser editor can produce disk diffs even when the user performs no edits — a side effect of the Observer B → Observer A bridge round-trip that fires on initial document load. Three independent normalization classes were identified during PR #165 testing: (1) ordered-list renumbering of non-sequential items (e.g., `24.` becomes `15.` in a list that started at `9.`), (2) over-cautious backslash-escape injection (`~`, `_` escaped defensively even inside inline code spans where they can't mis-parse), (3) blank-line canonicalization between a list-item's opening paragraph and its indented code-fence child. **Pre-existing — not introduced, exacerbated, or mitigated by PR #165.** Originally reported in Slack before this PR was authored. PR #165 does not touch `server-observers.ts`, `persistence.ts`, or the mdast-to-markdown serializer in any way that would affect the initial-load round-trip. Full reproduction methodology, root-cause trace, and fix-option assessment (ranked by scope) in `evidence/markdown-roundtrip-fidelity-gaps.md`. Recommended path: a follow-up PR first addresses Option A (suppress Observer A on initial bridge-population) — the smallest intervention that eliminates the most surprising user-visible symptom ("I opened a file and git shows changes"). Classes 1-3 belong in a dedicated markdown-fidelity workstream with PBT fixture coverage.
...
- **FR-8** Descriptor registry: `Map<string, JsxComponentDescriptor>` with runtime lookup. Wildcard `'*'` descriptor is always registered and acts as fallback. Registry source: **committed built-ins manifest at `packages/core/src/registry/built-ins.ts`** (18 components: 16 fumadocs-ui + Mermaid + Audio shadcn wrappers per D3).<br>_[Corrected 2026-04-21 post-ship: shadcn wrappers at `packages/app/src/components/ui/{mermaid,audio}.tsx` were NOT authored; PR #165 initially shipped inline `MermaidPlaceholder` + `AudioPlaceholder` stubs in `componentMap.tsx`. **Later the same day the non-functional `MermaidPlaceholder` was removed from the registry** (greenfield: don't claim to ship capability the code doesn't deliver) — registry is now **17 components**; `<Mermaid />` user content auto-converts to `rawMdxFallback` (nested CM source editor) via the wildcard path in `JsxComponentView`. `AudioPlaceholder` was renamed to `Audio` — functionally a real HTML5 `<audio>` element from day 1 (playback works); the `Placeholder` suffix was misleading, not the code. The shadcn-chrome follow-up for Audio is preserved as future work. Full shipped-state audit + un-deferral decision framework in `evidence/mermaid-audio-rendering-deferred.md`; renderer landscape research in `reports/mermaid-rendering-options-for-mdx-editors/`.]_ User-registered custom components (formerly D9/D10, now NG13) are out of scope; full prior analysis preserved in `evidence/custom-components-deferred.md`. The wildcard path still serves any non-built-in component names encountered in user documents (legacy MDX, future fumadocs additions not yet manifested). **Extensibility seam:** the `Map<string, JsxComponentDescriptor>` interface is designed as a stable seam for NG13. A future re-spec merges user descriptors into the same Map at startup with zero schema or registry-structure change (`userDescriptors.forEach(d => registry.set(d.name, d))`). See `evidence/custom-components-deferred.md` §1 for the preserved design work this seam is intended to support.
- **FR-9** NodeView (block): descriptor-dispatch at render. **Per the always-visible-content invariant (Precedent #26): NodeViewContent is ALWAYS rendered, never `display: none`.** Branches:
...
- **FR-24** Render-failure editability — **invalid-state nested CodeMirror** (Precedent #26 unified mechanism). `ComponentErrorBoundary` catches a thrown render → NodeView swaps render branch from "live React" to "nested CM editor" showing the block's source (reconstructed from attrs+children if `sourceDirty`, or `sourceRaw` if pristine). User edits the source in CM (per the always-visible-content invariant — source IS visible AND editable, never read-only). On commit (blur or throttled), block-scoped `parseWithFallback` re-parses; if the result re-renders cleanly, NodeView switches back to live React; if still throws, returns to CM; if parses to a different valid component, dispatches replaceNode; if parses to rawMdxFallback, dispatches replaceNode to rawMdxFallback. **Symmetric with rawMdxFallback (parse failure)** — both invalid states use the same nested CM mechanism (§9.14). No Retry button (auto-retry happens on every commit). No read-only chrome. No childrenFallback separate path (children are part of the source the user is editing).
...
### CB — Context Bridge Registry (FR-27, FR-29, §9.15, Precedent #25)
...
| CH04 | Registered + `hasChildren: false` | zero | Live React render + `<NodeViewContent>` with **CSS zero-footprint** (`min-height: 0; margin: 0; padding: 0`) — NEVER `display: none` per Precedent #26 / NG7a. No collapsed-margin artifacts; content always DOM-present for Y.js sync. NodeSelection still works; drag/delete/PropPanel work. |
...
### NCM — Nested CodeMirror in ProseMirror (FR-30..FR-35, §9.14, Precedent #24)
...
| SH03 | Widen content expression from `''` (atom) to `block*`: pre-existing jsxComponent nodes with no children (they couldn't have children pre-widening) render as empty containers | No crash on load. Wildcard path applied for unknown names. Empty-container placeholder (FR-16a) or **CSS zero-footprint `<NodeViewContent>`** (FR-9 — NEVER `display: none` per Precedent #26 / NG7a) applies depending on descriptor and hasChildren flag. Assertion: PM doc validates against widened schema; no schema-check throws. |
...
| VR13 | `Mermaid` with a flowchart fixture × {light, dark} | Diagram renders; colors match (Mermaid's own theme must match docs-site's Mermaid theme). |<br>_[Corrected 2026-04-21 post-ship: same correction as the breadcrumb at FR-8 (line 160 above); `evidence/mermaid-audio-rendering-deferred.md`. **VR13 is void** — the `Mermaid` descriptor was removed from the registry 2026-04-21 (placeholder stub was non-functional). `<Mermaid />` user content now auto-converts to `rawMdxFallback` (nested CM source editor). When a real Mermaid renderer is un-deferred, VR13 gets rewritten against the new implementation.]_
| VR14 | `Audio` with test MP3 × {light, dark} | Audio player chrome (shadcn wrapper) matches reference. |<br>_[Corrected 2026-04-21 post-ship: same correction as the breadcrumb at FR-8 (line 160 above); `evidence/mermaid-audio-rendering-deferred.md`. VR14 not implemented — the shipped `Audio` component renders a real HTML5 `<audio>` element (playback works) but without shadcn chrome. Follow-up work item with concrete scope + current lean (AI Elements AudioPlayer on `media-chrome`) captured in the evidence file.]_
...
**18-of-21 components resolve:** 16 fumadocs (verified in `node_modules/fumadocs-ui@16.1.0/dist/components/`) + 2 shadcn wrappers to write (`packages/app/src/components/ui/{mermaid,audio}.tsx`). 3 docskit deferred to Future Work (`@inkeep/docskit` not installed).<br>_[Corrected 2026-04-21 post-ship: same correction as the breadcrumb at FR-8 (line 160 above); `evidence/mermaid-audio-rendering-deferred.md`. The two shadcn wrappers were not authored — PR #165 shipped inline placeholder stubs in `componentMap.tsx` instead. Later on 2026-04-21 the Mermaid placeholder was removed entirely (non-functional stub → tech debt under greenfield directive); registry is now **17-of-21** (16 fumadocs + functional HTML5-Audio wrapper). Mermaid un-defer trigger: first customer insert of `<Mermaid />` in authored content OR concerted VR*-coverage push.]_
...
export interface JsxComponentMeta {
  ...
  hasChildren: boolean;                   // PropPanel/slash-menu hint; NodeViewContent always renders per Precedent #26
...
**Invalid-state nested CodeMirror** uses the same `createNestedCMExtensions` factory + NodeView pattern as `rawMdxFallback` (§9.14, Precedent #24). The CM instance is initialized with the block's effective source — `sourceRaw` if pristine, otherwise reconstructed via the γ pattern. On commit (blur, throttle, or Cmd+Enter), `parseWithFallback` re-parses the CM text scoped to this block:
...
**Render-failure ≡ parse-failure (Precedent #26 unified mechanism):**
...
function JsxComponentView({ node, editor, getPos, selected }: NodeViewProps) {
  ...
  // BRANCH 3: Invalid-state nested CM (Precedent #26)
  ...
  return (
    <NodeViewWrapper className={`jsx-component-wrapper ${selected ? 'is-selected' : ''}`}>
       ...
       *  Precedent #26: no Retry button, no read-only chrome, no childrenFallback —
      ...
      >
        <Component {...primitiveProps}>
          {/* Precedent #26: NodeViewContent ALWAYS rendered. When empty (hasChildren:false + 0 children),
...
| `packages/core/src/registry/built-ins.ts` | same | ✅ adapt — 16 fumadocs + 2 shadcn wrappers (Mermaid, Audio); add `description`, `searchTerms` (verbatim T1 curation), `emptyChildName` for containers (`Steps→'Step'`, `Tabs→'Tab'`, `Cards→'Card'`, `Files→'File'`) |<br>_[Corrected 2026-04-21 post-ship: same correction as the breadcrumb at FR-8 (line 160 above); `evidence/mermaid-audio-rendering-deferred.md`. Mermaid descriptor + `mermaidProps` removed from `built-ins.ts` on 2026-04-21 (placeholder stub was non-functional). Audio descriptor remains; the functional HTML5-audio wrapper in `componentMap.tsx` was renamed from `AudioPlaceholder` → `Audio`. Manifest is now 17 components.]_
...
**Sync architecture (direct PM dispatch, NOT y-codemirror.next — D13 + Architectural Precedent #24 below):**
...
| D3 | Built-ins P0: 18 components (16 fumadocs + Mermaid + Audio shadcn wrappers). Docskit (Video/Frame/CodeGroup) → Explored-tier Future Work with package-install trigger | P | **LOCKED** | NO | worldmodel §1 — docskit dep absent; Mermaid/Audio wrapper files absent; 16 fumadocs resolve cleanly | `evidence/worldmodel.md` §1 |<br>_[Corrected 2026-04-21 post-ship: same correction as the breadcrumb at FR-8 (line 160 above); `evidence/mermaid-audio-rendering-deferred.md`. The 18 descriptors all shipped initially but Mermaid + Audio were backed by placeholder stubs rather than shadcn wrappers. Later on 2026-04-21 the non-functional Mermaid stub was removed from the registry entirely (greenfield: 17-of-18 descriptors now real); Audio's `AudioPlaceholder` was renamed to `Audio` (it was always functional — real HTML5 `<audio>` element; the "Placeholder" suffix was the only stub-like thing about it).]_
...
7. Implement NodeView (block-only per NG14; jsxInline is thin shape with no NodeView) + PropPanel (block) with Precedent #26 unified invalid-state CM (ComponentErrorBoundary + InvalidStateCMEditor per §9.7 Branch 3)
