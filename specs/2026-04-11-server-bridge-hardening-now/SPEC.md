...
**Last updated:** 2026-04-11 (post-audit finalization)
**Baseline commit:** 2d35736 (verified by auditor against this SHA)
**Links:**
...
| **Must** | S4.R1 — `setupObservers` init-time throws are caught, provider destroyed, entry evicted | `packages/app/src/editor/provider-pool.ts:92-111` wraps the `setupObservers` call in try/catch. On throw: call `this.destroyEntry(entry)`, remove from `this.entries` and `this.lruOrder`, reset `activeDocName` if active, call `this.notify()`, then re-throw with `[ProviderPool] setupObservers init failed for <docName>: <original>`. | Destroy-and-evict matches existing pool lifecycle. |
...
`packages/app/src/editor/provider-pool.ts:92-111`. The `onSynced` event handler calls `setupObservers` synchronously. On throw, the error propagates into HocuspocusProvider's internal event emitter (silently swallowed or re-emitted as `'error'`). `entry.syncState` is already `'synced'`, `entry.observerCleanup` stays `null`, entry remains cached. Subsequent `open()` calls return the broken entry forever.

Test coverage: `provider-pool.test.ts` uses `DUMMY_WS` so `onSynced` never fires — the setup path is dark at the unit level.
...
| A4 | `provider.on('synced', onSynced)` fires once per connection lifecycle in common case | MEDIUM | Verify during S4 implementation by reading HocuspocusProvider source | S4 implementation | Active |
...
- **SCOPE:**
  ...
  - `packages/app/src/editor/provider-pool.ts` — LINES 92-111 (wrap setupObservers call)
