...
- **Electron 41 fuses enabled at package time:** `runAsNode: false`, `enableNodeCliInspectArguments: true` (required for Playwright `_electron.launch`), `enableEmbeddedAsarIntegrityValidation: true`, `onlyLoadAppFromAsar: true`, `enableNodeOptionsEnvironmentVariable: false`. Post-sign `@electron/fuses read` verification is a required release-pipeline step per FU-2 Class 8 (fuses × signtool clobber — electron-builder issue #9428).<br>_[Corrected 2026-04-23 post-ship: `runAsNode` amended to `true` for M6a. The bundled `ok.sh` wrapper requires `ELECTRON_RUN_AS_NODE=1` in packaged builds (VS Code / Atom precedent). Defense-in-depth preserved: EnableNodeOptionsEnvironmentVariable stays DISABLED, asar integrity + only-load-from-asar unchanged. Authoritative fix in `packages/desktop/scripts/target-fuses.mjs` header + `specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md` §10 D-M6-R11.]_
...
### J4 — Opening a second project (new window is the default)

Every project open — whether the first of the session, the fifth, or triggered from Recent — creates a **new BrowserWindow** with its own `utilityProcess`. There is no "switch in current window" UX in v0 (retired 2026-04-20 per D3 revision — new-window was explicitly confirmed as the better experience because users routinely want multiple projects visible side-by-side).

1. User triggers an open from any surface — File → Open Recent, the Project Navigator Recent list, File → New Project, File → Clone from GitHub…, or an `openknowledge://` deep-link.
2. Main spawns a new BrowserWindow + utilityProcess for the target (`acquireServerLock` → `listen(0)` → `updateServerLockPort` → awaits `ready`; same sequence as J1 steps 6-8).
3. Existing window(s) are untouched. User now has N+1 windows, one per project.
4. The Project Navigator window stays on the Navigator view — it is a persistent launcher, not an editor shell (per D24 revision). User closes it manually when done launching.
...
- "First-URL wins" contract: the pool does not support mutation of `wsUrl` after construction. Because each project opens in its own window (J4, D3 revised), a pool is constructed once per window for its one project and never re-pointed.
...
### 8.6 Project Navigator (persistent launcher window — D24 revised 2026-04-20)

**The Navigator is its own BrowserWindow — a persistent launcher.** On app launch (no last-opened project, or Option held at launch), main creates a Navigator window. Every project pick from that window spawns a **new** editor BrowserWindow + utilityProcess (per D3 revised); the Navigator window itself stays on the Navigator view. Closing all editor windows leaves the Navigator open; clicking the Dock icon when no windows are open re-opens it. This matches the GitHub Desktop "Choose a repository" / TextMate project-chooser pattern rather than VS Code's same-window recents (which pairs with switch-in-place — a UX we explicitly rejected per D3).

The Navigator window has **no utilityProcess attached** — it is a UI-only surface. It renders the same React bundle as editor windows, loaded with a mode flag (`?mode=navigator` or a preload-injected `window.okDesktop.config.mode: 'navigator'`) so the React tree renders the Navigator component instead of the editor shell. No separate Vite entry, no second bundle; shared code path.
...
**Why not same-window conditional render (original D24):** The CS1 survey that drove the original D24 decision (4/5 apps use same-window conditional render) was coupled to a click-to-switch-in-current-window UX. Under new-window-default (D3 revised), the Navigator no longer needs to transition-in-place — it only needs to *launch* new windows. Keeping it as a persistent launcher is simpler: no transition animation, no window-type morphing, no need for the Navigator React path to co-exist with editor state in the same tree.
...
│   Navigator stays open for launching more.)            │
│                                                        │
└────────────────────────────────────────────────────────┘
```
...
- **Click** → main spawns a new BrowserWindow + utilityProcess for the target. Navigator window stays on the Navigator view (per D24 revised). No modifier needed — new-window is the only open semantics in v0.
- **Right-click / context menu** → Show in Finder, Remove from Recent, Locate…
- A recent project whose folder no longer exists renders dimmed with "Missing" badge; right-click offers "Remove from Recent" or "Locate…".
...
- **App launch:** electron-updater check → restore state → if `lastOpenedProject` and **not** Option-held → open that project's editor window directly (Navigator not shown). Else → open the Navigator window.
- **User opens a project (J3/J4):** new editor BrowserWindow + new utilityProcess. Existing windows (including the Navigator) untouched. This is the only open semantics in v0 (per D3 revised).
- **User closes last editor window, Navigator still open:** app stays running; Navigator remains available for launching more projects.
- **User closes every window:** macOS convention — app stays running (Dock icon visible). Click Dock icon → re-open the Navigator window.
...
> Close it there before editing here.
> [ Cancel ]  [ Show lock in Finder ]"

Hard-refuse. No read-only fallback in v0 — the cross-machine read-only mode (former D50) was retracted 2026-04-20 because reliable cross-machine detection requires the `server.lock` to propagate faithfully through the sync service, which Dropbox / iCloud / Google Drive do NOT guarantee (they ignore or delay `.lock` files, create conflict-copies that fragment the signal, etc.). Without a reliable detection signal there is no reliable UX to build on top of it. See §13 Future Work for revisit conditions.
...
- **`asarUnpack`** — native modules + platform-specific siblings + git:
  ...
  asarUnpack:
    ...
    - "**/simple-git/**"
  ```
- **Electron 41 fuses** (flipped at package time, before sign): `runAsNode: false`, `enableCookieEncryption: true`, `enableNodeOptionsEnvironmentVariable: false`, `enableNodeCliInspectArguments: true` (**required** for Playwright `_electron.launch`), `enableEmbeddedAsarIntegrityValidation: true`, `onlyLoadAppFromAsar: true`. Close the CVE-2025-55305 attack surface.<br>_[Corrected 2026-04-23 post-ship: same correction as the breadcrumb at line 157 above — `runAsNode` amended to `true` for M6a. Authoritative fix in `packages/desktop/scripts/target-fuses.mjs` header + `specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md` §10 D-M6-R11.]_
...
The Electron main process does NOT re-implement MCP wiring. It invokes the existing `runInit(...)` flow from `packages/cli/src/commands/init.ts`, which already:<br>_[Corrected 2026-04-23 post-ship: M6b invokes a NEW surgical export `writeUserMcpConfigs()` (NOT `runInit`) to avoid `runInit`'s project-scoped side effects (`ensureProjectGit` / `initContent` / `scaffoldLaunchJson` / `upsertRootInstructions` / `collectLegacyProjectConfig`) — a packaged-app `cwd` would otherwise `git init /` at every first launch. Trigger is also user-scoped (fires on first desktop-app launch per user, not per project) — the marker lives at `~/.open-knowledge/mcp-status.json`. Authoritative fix in [specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md](../2026-04-21-m6-cli-and-mcp-wiring/SPEC.md) D-M6-R1 + D-M6-R8._
...
2. `runInit` is **synchronous** (not `async`) and returns `InitCommandResult { contentCreated, contentSkipped, editors, rootInstructions, preview? }`. Scaffolds `.open-knowledge/` via `initContent(cwd)` (idempotent) and writes MCP entries for each editor.<br>_[Corrected 2026-04-23 post-ship: `runInit` IS async (signature is `async function runInit(opts: InitCommandOptions): Promise<InitCommandResult>` — `packages/cli/src/commands/init.ts:464`); the "synchronous" claim was a factual error. Also superseded: M6b does not call `runInit` — same correction as the breadcrumb at line 822 above (M6b uses the new surgical `writeUserMcpConfigs` export). Authoritative fix in [specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md](../2026-04-21-m6-cli-and-mcp-wiring/SPEC.md) D-M6-R8 + audit M10._
...
```
instead of the CLI-default `{"command": "npx", "args": ["-y", "@inkeep/open-knowledge", "mcp"]}`. This means P1 (no Node.js installed) gets working AI-tool MCP integration the moment they install Command-Line Tools — no terminal required. Preserves G3.<br>_[Corrected 2026-04-23 post-ship: `cliPath` is hybrid (D-M6-R9) — prefers `/usr/local/bin/ok` when the symlink is ownership-verified against the current bundle, otherwise falls back to the bundle-absolute path `${bundleRoot}/Contents/Resources/cli/bin/ok.sh`. The fallback branch means M6b-accepted P1 users get working MCP WITHOUT having to click "Install Command-Line Tools…" — the M6a install is decoupled from the MCP path (D-M6-R2). Authoritative fix in [specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md](../2026-04-21-m6-cli-and-mcp-wiring/SPEC.md) D-M6-R2 + D-M6-R9._
...
- **~~R3 — Switch-in-current-window state management.~~** **Retracted 2026-04-20 alongside D3 revision.** The switch-in-place UX was removed in favor of new-window-default (see revised J4 and §8.7). Utility lifetime is now bound 1:1 to window lifetime, which eliminates the teardown-and-respawn dance this risk described. Each project window boots its own utilityProcess at creation and tears it down at window close — no mid-lifetime utility swaps.
...
| D3 | LOCKED (revised 2026-04-20) | Product | Multi-window project model with persistent-launcher Project Navigator. One BrowserWindow ↔ one project ↔ one utilityProcess ↔ one contentDir (enforced by `server.lock`). **New-window is the default and only open semantics in v0** — every project pick (from Navigator, File → Open Recent, deep-link, etc.) spawns a new editor window. Switch-in-current-window UX explicitly retired — users routinely want multiple projects visible side-by-side; new-window-default matches that mental model (GitHub Desktop / TextMate pattern). Cmd+Click modifier dropped as redundant; context-menu covers Show in Finder / Remove from Recent. | §6 J4, §8.6, §8.7, §8.8. R3 retracted alongside this revision. | HIGH | 1-way door for UX; architecture reversible only via consolidation refactor |
...
| D24 | LOCKED 2026-04-16 (revised 2026-04-20) | Technical | **Navigator is its own persistent-launcher BrowserWindow** (revised from same-window conditional render). Under D3 revised (new-window-default), every project pick spawns a fresh editor BrowserWindow; the Navigator window stays alive on the Navigator view, acting as a launcher. No utilityProcess attached to the Navigator window. Shared React bundle loaded with a `mode: 'navigator'` flag (preload-injected via `window.okDesktop.config.mode`) so the tree renders the Navigator component. GitHub Desktop / TextMate project-chooser pattern. The original CS1 finding (4/5 apps use same-window conditional render) was coupled to switch-in-place UX, which D3 revised rejected — keeping Navigator as its own window is simpler than same-window morphing once switch-in-place is off the table. | §8.6 (revised), D3 revised. | HIGH | Reversible — can merge back into editor window later if user feedback shows dual-window launcher is too much |
...
| D44 | LOCKED 2026-04-17 (revised 2026-04-20) | Product | **J7b collision dialog three-case, lock-holder-identity-dispatched.** (a) Another Electron window owned by this app (pid ∈ our window map): "Focus existing window" — `targetWindow.focus()`. (b) CLI `ok start` sibling (pid alive, hostname matches localhost, not in our map): "Quit the CLI server and continue" — main SIGTERMs sibling pid, waits 10s for lock release (SIGKILL fallback), re-acquires via new utility spawn. Safe because `ok start`'s state is minimal (L1/L2 debouncers flush on destroy). (c) Foreign host (hostname mismatch — cross-machine sync): hard-refuse dialog with "Cancel" + "Show lock in Finder" (read-only fallback removed alongside D50 retraction 2026-04-20 — cross-machine detection via lock-file hostname is unreliable through Dropbox/iCloud sync). Industry divergence: zero of 9 surveyed apps (T3) show collision dialog; all do silent focus. Our dialog is a deliberate product choice surfacing project-as-first-class-entity ownership. D48 diagnostic error shape includes `{pid, hostname, processName, startedAt, worktreeRoot}`. | Scenario A, T3 research, D48, D50 (retracted). | HIGH (all three cases — case c now hard-refuse, no TENTATIVE surface) | Reversible — can drop to silent-focus-same-machine + dialog-only-for-foreign-host if user feedback shows dialog is noise |
...
| D50 | **RETRACTED 2026-04-20** | Product + Technical | ~~Cross-machine iCloud/Dropbox sync read-only fallback.~~ The premise was that hostname-in-`server.lock` would propagate through sync services reliably enough to detect cross-machine concurrent opens. User callout 2026-04-20 surfaced the real issue: **we have no reliable way to identify the cross-machine situation** — Dropbox/iCloud/Google Drive frequently ignore or delay `.lock` files, create conflict-copies that fragment the signal, and have no guaranteed ordering on lock writes vs lock reads across machines. Without a reliable detection signal, the novel read-only UX has no trustworthy trigger and would fire incorrectly (false positives on stale lock fragments, false negatives when the lock hasn't synced yet). Case (c) in D44 revised to hard-refuse. Cross-machine concurrent editing is deferred to §13 Future Work with explicit revisit conditions (a non-filesystem coordination channel — mDNS local peer discovery, a cloud relay, or a CRDT-level awareness protocol — would be the minimum primitive to build on). | User callout 2026-04-20 on sync-service unreliability; supersedes D50 TENTATIVE. | N/A (retracted) | N/A |
...
| D52 | LOCKED 2026-04-17 (revised 2026-04-20) | Product | **Electron ships the CLI on PATH via "Install Command-Line Tools…" menu item; primary bin is `ok` with `open-knowledge` as backward-compat alias (supersedes D11; aligns with PR #170).** Reverses D11's "drop the CLI shim install." File → "Install Command-Line Tools…" creates symlinks `/usr/local/bin/ok` AND `/usr/local/bin/open-knowledge`, both pointing at the bundled CLI binary inside `app.asar.unpacked/cli/`. First click triggers macOS admin prompt (same UX as VS Code's `Shell Command: Install 'code' command in PATH`). Once per machine. Aligns with [`reports/cli-command-name-ok-okb/REPORT.md`](../../reports/cli-command-name-ok-okb/REPORT.md) (PR #170, merged 2026-04-17): ship `ok` as primary, keep `@inkeep/open-knowledge` as npm package name (ripgrep / fd / bat / bun / jj long-package-short-bin precedent). **`packages/cli/package.json` on origin/main already ships `"bin": { "open-knowledge": "./dist/cli.mjs", "ok": "./dist/cli.mjs" }`** — verified 2026-04-20 on main. No upstream coordination remains; the bundled CLI inherits both bins automatically through the shared `extraResources` packaging. **Load-bearing for P1:** Electron's `runInit` (§8.11), when writing MCP configs on behalf of Electron, prefers the bundled CLI path over `npx` — `{"command": "/usr/local/bin/ok", "args": ["mcp"]}` instead of `{"command": "npx", "args": ["-y", "@inkeep/open-knowledge", "mcp"]}`. P1 (no Node.js installed) gets working AI-tool MCP integration without any terminal contact (preserves G3). The `open-knowledge` alias symlink ensures pre-existing `.mcp.json` entries resolve. CLI-origin MCP configs (written by `ok init` from a terminal) continue to use `npx @inkeep/open-knowledge mcp` since CLI users have Node by definition.<br>_[Corrected 2026-04-23 post-ship: same correction as the breadcrumb at line 861 above — the literal `{"command": "/usr/local/bin/ok", "args": ["mcp"]}` embedded in this D52 prose is now the hybrid branch; bundle-absolute is the fallback when `/usr/local/bin/ok` fails the ownership check or is absent. Authoritative fix in [specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md](../2026-04-21-m6-cli-and-mcp-wiring/SPEC.md) D-M6-R2 + D-M6-R9._ | D11 (superseded), §8.11, §8.12, PR #170 (`reports/cli-command-name-ok-okb/REPORT.md`), G3. | HIGH | Reversible — fall back to `open-knowledge` only by dropping the `ok` symlink + reverting MCP config preference |
...
- **Cross-machine concurrent editing (retired D50).** Originally proposed as a read-only fallback when `server.lock` hostname differs from `os.hostname()`. Retired 2026-04-20 because there is no reliable way to detect the cross-machine situation via filesystem-level signals — Dropbox / iCloud / Google Drive frequently ignore or delay `.lock` files, create conflict-copies, and offer no ordering guarantees. **Promote when:** a non-filesystem coordination channel exists — candidates include mDNS / Bonjour local-peer discovery for same-LAN Macs, an opt-in cloud relay (e.g., an Inkeep-hosted awareness server — would also unlock cross-device real-time collaboration, NG5), or a CRDT-native awareness protocol piggybacking on the clone-from-github shared remote. Without one of those primitives the feature has no trustworthy trigger. See D50 retraction + D44 case (c) hard-refuse.
...
**Definition of done:**
- First-project-open triggers MCP consent dialog (detected editors: Claude Code, Claude Desktop, Cursor, VS Code, Codex, Windsurf via `detectInstalledEditors(cwd)`).<br>_[Corrected 2026-04-23 post-ship: same correction as the breadcrumb at line 822 above — the trigger is user-scoped (fires on first packaged-app launch per user via `~/.open-knowledge/mcp-status.json` marker), not project-scoped (D-M6-R1). Authoritative fix in [specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md](../2026-04-21-m6-cli-and-mcp-wiring/SPEC.md)._
- "Install Command-Line Tools…" menu item creates `/usr/local/bin/open-knowledge` symlink per D52 (admin prompt once).
- `runInit({ cwd, editors, mcp: true, force: false })` writes MCP configs using the bundled CLI path (`/usr/local/bin/open-knowledge mcp`) NOT `npx` (D52).<br>_[Corrected 2026-04-23 post-ship: M6b uses the new `writeUserMcpConfigs` export (not `runInit`); cliPath is the hybrid resolver's output (`/usr/local/bin/ok` when ownership-verified, bundle-absolute `.../cli/bin/ok.sh` otherwise) — same corrections as breadcrumbs at lines 822 + 861 above. Authoritative fix in [specs/2026-04-21-m6-cli-and-mcp-wiring/SPEC.md](../2026-04-21-m6-cli-and-mcp-wiring/SPEC.md) D-M6-R2 + D-M6-R8 + D-M6-R9._
