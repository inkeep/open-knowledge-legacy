...
**`.ok/` is gitignored — both project-root `.ok/` AND every nested `<folder>/.ok/`.** Per-machine local agent operating context. Templates and folder defaults don't sync via git. (D18.)
...
2. **Optionally descending N levels into subfolders** when the caller passes `depth: N` (D13 — see §5.1). Templates from descendants are surfaced for visibility but flagged with `scope: "descendant"` (D17).
...
3. Frontmatter precedence (high → low): **agent's explicit frontmatter > template frontmatter > folder cascade (§4.1)**. Template tags REPLACE cascaded tags per D6 (templates are the source of tags at create time, not cascade). Agent's explicit frontmatter wins per-scalar over both.
...
write_template({
  ...
  body: "# {Meeting Title}\n\n**Attendees:**\n...",
  frontmatter: {
    title: "Meeting Prep Notes",                  // SHOULD be present (D14 soft contract)
    description: "Use before a meeting...",       // SHOULD be present (D14 soft contract)
    tags: ["meeting", "prep"],
  },
})
```
...
### 7.2 Metadata convention (D14 — soft contract)
...
**Behavioral change for existing rules: tag cascade switches from concat-and-dedup to last-wins / replace.** Today's `folders[]` mechanism (`packages/cli/src/content/folder-rules.ts:47-51`) concatenates tags across ALL matching glob rules and deduplicates them — so a rule `specs/**` carrying `tags: [spec]` plus a more-specific `specs/*/evidence/**` carrying `tags: [evidence]` produces `[spec, evidence]` for evidence files. Under the new mechanism (D6), tag cascade in nested `.ok/frontmatter.yml` files is REPLACE — `specs/.ok/frontmatter.yml` declaring `tags: [spec]` plus `specs/2026-04-25-foo/evidence/.ok/frontmatter.yml` declaring `tags: [evidence]` produces `[evidence]` only at evidence files. Migration MUST declare the FULL desired tag set in each nested file (e.g., `tags: [spec, evidence]` at the evidence-folder level) — otherwise existing docs lose tags they previously inherited. Any rule that relied on tag accumulation across overlapping globs gets its merged tag list materialized at the leaf folder during migration.
...
The walker (`loadNestedIgnoreFiles` in `content-filter.ts:309` — already correctly checks `entry.name` per descent) is unaffected.
...
- **[NOT NOW]** NG-T1: Templating engine with variable substitution (`${user}`, `${date}`). The template is a starting point, not a macro — agents reason about variable content directly, and macros are structurally lower-leverage than letting the agent edit after instantiation. Revisit if v2 research (§16) surfaces a use case where agent-driven editing isn't sufficient (e.g., human-only template instantiation flows).
...
| D18 | 2026-05-01 | P    | Both project-root `.ok/` AND every nested `<folder>/.ok/` are gitignored by default. `ok init` adds the entry; nested `.ok/` directories are covered by the same root-level glob (`**/.ok/`). | Per-machine local agent operating context; users wanting shared templates manually adjust `.gitignore`. The "root + nested" scope is explicit because nested `.ok/` directories live in the user content tree and could otherwise be mistakenly committed. |
...
| Must     | FR16   | CLAUDE.md / AGENTS.md STOP rule "Folder defaults live in `config.yml`'s `folders[]`" updated to reflect §8 BEFORE FR1 lands. The current STOP rule explicitly forbids per-folder `.frontmatter.yml` sidecars; an implementation agent reading it will refuse to create the nested files this spec introduces. Sequence FR16 first to avoid runtime-contradiction failure modes. | Manual review; grep -n "Folder defaults live in" CLAUDE.md AGENTS.md returns the new wording |
...
2. **Implementation order:**
   - **FR16 first** (CLAUDE.md / AGENTS.md STOP-rule update). The current STOP rule explicitly forbids per-folder `.frontmatter.yml` sidecars; without this update, an implementation agent reading the rule will refuse to create the nested files this spec introduces.
   - FR-CF1 second (content-filter fix is independently useful and unblocks nested `.ok/` indexing).
   ...
   - FR7 (corrigendum on NG10 in config-edit-paths SPEC) at PR-merge time.
