...
| D35 | Legacy refs in history repo deleted on first-run post-upgrade. Sweep is **classification-based via `parseWriterId`**, not a simple `$NF == "server"` regex. Matches all legacy writer patterns: `refs/wip/<branch>/server`, `refs/wip/<branch>/human-server` (actively written today by `parkBranch(..., sessionId='server')` at standalone.ts:1044 → shadow-repo.ts:722), and any other pre-spec writer IDs that don't match the new `{agent-*, principal-*, classified}` taxonomy. Idempotent sweep in `initHistoryRepo` completion. No archive, no rename. | Tech | LOCKED | No | **Corrected via audit-batch-c.** Regex-based sweep would miss `human-server` pattern — load-bearing because it's actively written by the current parkBranch path. Classification-based sweep via `parseWriterId` + new-taxonomy allowlist handles all legacy patterns idempotently. Greenfield waives backward compat. | `evidence/history-and-sweep.md` Q26 + shadow-repo.ts:722 + standalone.ts:1044 | Closes Q9. Metric + bracket-prefixed log on sweep. |
...
| D57 | **Y.Map rename: `Y.Map('activity')` → `Y.Map('agent-flash')`.** The Y.Map currently keyed `'activity'` in code (api-extension.ts:1089,1174,1707; TiptapEditor.tsx:236; agent-flash-source.ts:67; test-harness.ts:538) is renamed to `'agent-flash'`. Purpose is driving ephemeral flash-animation UX for agent writes — "agent-flash" accurately names the thing. Disambiguates from D49's separate "activity log" server-side store. | Tech | LOCKED | Yes (Y.Doc schema) | **Per audit-batch-d H3.** Three concepts used "activity" in names today: (1) `Y.Map('activity')` flash side-channel in code; (2) D49 server-side activity log; (3) D25 UM scope `activityMap`. Rename (1) to `agent-flash` eliminates all conflation cleanly. Under greenfield + clean-precedents directive. | audit-batch-d H3 | Code rename at 6 sites + D25 UM scope uses `flashMap` name; schema migration handled by greenfield (no persistence — Y.Doc state is re-created per session). |
