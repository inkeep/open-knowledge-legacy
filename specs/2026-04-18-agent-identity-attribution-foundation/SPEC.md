...
- `Y.Map('activity')` (current code at `api-extension.ts:1089, 1174, 1707` + 4 other sites; will rename to `Y.Map('agent-flash')` per D57), contributor-tracker, AgentFocus — three side-channels at the same write call sites.
...
| D19 | Branch-switch park is per-session: each active session's ref advances with a park commit (subject `park: <old-branch> -> <new-branch>`). Park is not a collapse-to-single-ref. **Implementation scope (explicit, per audit-batch-c H3):** requires non-trivial refactor — current `parkBranch(shadow, branch, sessionId, docs)` at `shadow-repo.ts:712-797` writes one commit bundling all docs for a single `sessionId`. Under D19+D40: caller iterates active sessions; signature shifts to `parkBranch(shadow, branch, writer: WriterIdentity, session_docs)`; `readParkedState` similarly per-session; restore loop in `standalone.ts` walks session refs and reconciles each. | Tech | LOCKED | Partial — park mechanics can evolve, but per-session preservation is load-bearing. | Greenfield directive favors semantic preservation of per-session history even through branch switches. | Conversation 2026-04-18 + `shadow-repo.ts:712-797` | Non-trivial refactor owned by D19+D40; not a "one-line reorder" (that is D39's separate scope). |
...
| D39 | Park serialization isolation: wrap the per-session park loop inside `doc.transact(fn, PARK_SNAPSHOT_ORIGIN)` so Y.js's internal transaction queue serializes the capture atomically vs concurrent transacts. `PARK_SNAPSHOT_ORIGIN` is a new typed `PairedWriteOrigin` (`paired: true`) so observers short-circuit as with other paired writes. Also move `setBatchInProgress(true)` BEFORE the park loop (ordering correctness). | Tech | LOCKED | No | **Corrected via audit-batch-c P1.** Previous phrasing "known tolerated microsecond-late transact" was pragmatism-mode. Under greenfield + best-correctness, the race is eliminable by wrapping in a transact — Y.js serializes. Note: per-session parkBranch signature refactor itself (one-commit-per-doc-list → loop-over-sessions) is the load-bearing work under D19 + D40, NOT this D39 mutex-ordering change. | `evidence/history-and-sweep.md` Q33 (updated) | New `PARK_SNAPSHOT_ORIGIN` typed constant; transact wrap in `standalone.ts`; order-correct `setBatchInProgress` placement. |
