...
- **FR-11 (Transaction-effect capture / y-lite).** Each agent transaction's text effect is captured via `YTextEvent.delta` (Quill Delta ops — per D22; NOT `transaction.changed` which is wrong shape) inside a `ytext.observe` callback bound to the session's write path. The delta is persisted to the server-side activity-log store (per D49), keyed by `(docName, session_id, transact_index)` with timestamp.
