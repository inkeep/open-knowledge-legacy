...
| Must | FR20: Shadow-repo layout shared via server exports (not reimplemented in CLI) | `packages/server/src/shadow-repo.ts` adds three exports consumed by the CLI reader: `getShadowRepoPath(projectRoot): string \| null` (returns canonical shadow dir or null when absent, covering both integrated and standalone modes), `getWipRefPattern(branch): string` (returns `refs/wip/<branch>/`), `parseWriterId(id): { type: 'agent' \| 'human' \| 'upstream' \| 'server' \| 'unknown', isAgent: boolean \| null }`. CLI's `readShadowLog` imports these — no regex or path reimplementation CLI-side. Server-side layout evolution is picked up via workspace-dep bump, not spec rework. | Bot review 2026-04-13 🟠 "Shadow-repo CLI reader couples to internal server layout without abstraction boundary" — D22 accepted; DEP-1 scope expands to cover these three exports |
...
- `packages/server/src/shadow-repo.ts` maintains the bare git repo at `.git/openknowledge/` with per-writer WIP refs. **Read-only consumption by CLI is net-new** (D18) and requires three new server-side exports per D22/FR20 (`getShadowRepoPath`, `getWipRefPattern`, `parseWriterId`).
...
- **Data model:** No persistent state. **Single unified `EnrichedMeta` shape with nullable fields** per FR14/D20 — agents get a stable contract regardless of cardinality:
  ...
  interface ShadowCommit {
    ...
    isAgent: boolean | null;                   // via parseWriterId() server export (FR20)
...
- **3P dependencies (all locked):** `shell-quote` v1.8.3+ (D7, parse only); `just-bash` (D14 — actual npm name; spec text said `@vercel/just-bash` colloquially, maintained by cramforce at Vercel); `simple-git` (D18, CLI direct-read of shadow repo — already transitively present).
...
| D22 | **Shadow-repo layout shared via server exports** (not reimplemented CLI-side) | X | LOCKED | Soft — workspace boundary | Bot review 2026-04-13 🟠 "Shadow-repo CLI reader couples to internal server layout." DEP-1 adds `getShadowRepoPath`, `getWipRefPattern`, `parseWriterId` exports to `packages/server/src/shadow-repo.ts`; CLI consumes as workspace imports. | Bot review + maintainer accept 2026-04-13; FR20 | DEP-1 scope expands slightly into `packages/server/` (one file, three new exports); CLI's `shadow-log.ts` imports instead of re-implementing |
...
| CLI coupling to shadow-repo on-disk layout (`refs/wip/<branch>/<writer-id>`) | LOW (mitigated in FR20/D22) | MED | FR20 mandates the sharing: `packages/server/src/shadow-repo.ts` exports `getShadowRepoPath`, `getWipRefPattern`, `parseWriterId`; CLI reader imports them. Any server-side layout change is picked up by workspace-dep bump, not spec rework. | Tim |
...
- **SCOPE:** new `packages/cli/src/content/enrichment.ts` (shared `enrichPath()` helper returning the unified nullable `EnrichedMeta` per D20/FR14); new `packages/cli/src/content/shadow-log.ts` (simple-git direct-read per D18, using server-exported layout helpers); `packages/cli/src/mcp/tools/read-document.ts` and `search.ts` (migrate to shared helper — D13; drop folder catalog lookup per D19); **`packages/server/src/shadow-repo.ts`** (add three exports per D22/FR20: `getShadowRepoPath`, `getWipRefPattern`, `parseWriterId`; this is the only server-package edit).
- **EXCLUDE:** all other `packages/server/` paths (persistence, file-watcher, reconciliation, etc.); `packages/app/`, `packages/core/`, CRDT bridge code; any `fs.stat` mtime plumbing (§15 Future Work); any new backlinks endpoint (§15 Future Work); any new HTTP endpoint for shadow-repo (D18 rejected)
...
- **ASK_FIRST:** any change to the `EnrichedMeta` shape (public contract — 1-way door per D20); any server-package edits beyond the three exports; any dependency additions beyond `simple-git` (already transitively present)
