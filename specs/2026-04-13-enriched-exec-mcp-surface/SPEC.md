...
| Must | FR14: **Single unified `EnrichedMeta` shape** (no silent schema divergence) | One interface, fields are **explicitly nullable** so agents have a stable contract regardless of cardinality: `{ path, title?, description?, tags[], backlinkCount: number \| null, backlinks: BacklinkEntry[] \| null, history: ShadowCommit[] \| null, historySource: 'shadow-repo' \| 'shadow-repo-absent' \| null, projectHistory: GitCommit[] \| null, projectHistorySource: 'git' \| 'git-absent' \| null }`. For multi-path output (`ls`, `grep`, `find`, N>1), all rich fields are `null` by convention (N-amplification avoidance). For single-path `cat` output, fields populate when data is available. `catalogCategory` field **removed** per D19. `modified` deferred to §15. Rich shape augmented per D24/D25: both shadow-repo AND project git log surface; full backlinks list (not count-only). Per D30: `structuredContent.enrichedPaths` is `EnrichedEntry[]` — a union of `EnrichedMeta` (files) and `DirectoryMeta` (directories, discriminated by `type: 'directory'`). `DirectoryMeta = { path, type: 'directory', directMdCount, recursiveMdCount, childDirCount, mostRecentMd?, truncated }` — folder content summary (replaces the on-demand catalog view for `ls`). | Bot review R1 🟠 "Multi-path vs single-path shapes diverge silently" (D20) + user feedback 2026-04-14 (D24, D25, D30) |
...
- **Data model:** No persistent state. **Single unified `EnrichedMeta` shape with nullable fields** per FR14/D20 — agents get a stable contract regardless of cardinality:
  ...
  - **Directory entries** (D30): `structuredContent.enrichedPaths` is `EnrichedEntry[] = (EnrichedMeta | DirectoryMeta)[]`. `DirectoryMeta` is returned for ls entries classified as directories via `fs.stat`:
    ```ts
    interface DirectoryMeta {
      path: string;
      type: 'directory';                       // discriminator
      directMdCount: number;                    // .md/.mdx files directly inside
      recursiveMdCount: number;                 // bounded recursive count (cap 1000)
      childDirCount: number;                    // subdirs directly inside (excluding .git, node_modules, etc.)
      mostRecentMd?: { path: string; title?: string; updatedAt: string };
      truncated: boolean;                       // true when scan hit the 1000-entry cap
    }
    ```
    Replaces the on-demand catalog view promised by D26 for `ls`.
...
| D24 | **`read_document` + exec rich enrichment surface BOTH shadow-repo activity AND project git log** (revises D12 "shadow-repo instead of git log") | X | LOCKED | No | Post-impl session (2026-04-14, tim-inkeep + Claude): D12's swap lost signal — files committed to project git without going through OK have their author name + commit message collapsed to lossy `upstream` imports in the shadow repo. Files never edited by OK have empty "Recent activity" which is worse than the old behavior. Fix: fan out both sources in parallel, render as two sections. Shadow-repo still owns agent-vs-human attribution (D12 rationale preserved). | User 2026-04-14 session; evidence/enrichment-data-gaps.md addendum | New `packages/cli/src/content/project-log.ts` using simple-git against project `.git/`; EnrichedMeta rich shape gains `projectHistory: GitCommit[] \| null` + `projectHistorySource: 'git' \| 'git-absent' \| null` (FR14 updated). Shipped as `[V0-24.1]`. |
| D25 | **Backlinks rendered as full list in single-path rich enrichment** (not count-only) | T | LOCKED | No | FR14 originally limited to `backlinkCount: number \| null` (D20 unified shape). User feedback: for single-path reads an agent wants the full list so it can see what links to this page. Multi-path slim keeps `backlinks: null` to avoid N-amplification. | User 2026-04-14 session | EnrichedMeta gains `backlinks: BacklinkEntry[] \| null`; read_document output restores `### Backlinks (N)` section with entries. Shipped as `[V0-24.1]`. |
| D26 | **Catalog auto-generation removed** (teardown, not just deprecation) | X | LOCKED | No | D19 deprecated folder-level INDEX.md frontmatter semantically, but the watcher still auto-regenerated INDEX.md files on every content change — pure vestigial cost (nothing consumed them post-V0-24). `exec("ls <dir>")` + per-file enrichment renders an equivalent live view on demand without persisted artifacts. | User 2026-04-14 session | Deleted `packages/cli/src/content/mirror-catalog.{ts,test.ts}`, `catalog.{ts,test.ts}`, `catalog-store.{ts,test.ts}`; ~900 LOC removed. Dropped `catalog` from `RegisterAllToolsOptions`. Runtime: 385 tracked `INDEX.md` files deleted from repo. Shipped as `[V0-24.2]`. |
| D27 | **`init` scaffolds config-only** (no `articles/`/`research/`/`external-sources/` content subdirs) | X | LOCKED | No | Scaffold dirs were opinionated clutter for projects with existing docs structure. `content.dir` already tells Open Knowledge where content lives. Lifecycle (ingest → research → articles) stays as a conceptual pattern in AGENTS.md — projects create subfolders only when they need them. | User 2026-04-14 session ("why even make them") | `init.ts` SCAFFOLD_DIRS deleted; AGENTS.md template rewritten to describe lifecycle as optional pattern relative to content.dir. Shipped as `[V0-24.2]`. |
| D28 | **MCP `INSTRUCTIONS` built at server-connect time from live config** (not a static const) | T | LOCKED | No | Agents previously needed to read `config.yml` to know where content lives. With `buildInstructions(config)`, the actual `content.dir`/`include`/`exclude` values are baked into the instructions every agent receives on connect — no file-read required. | User 2026-04-14 session ("why cant we just pass the actual config and use it instead of telling the llm to read it") | `server.ts` INSTRUCTIONS const → function; takes Config, interpolates live values. Shipped as `[V0-24.2]`. |
| D29 | **Workflow tools (ingest / research / consolidate) interpolate `content.dir` into output text** — no more hardcoded `.open-knowledge/articles/` paths | T | LOCKED | No | Companion to D28. Each tool's instructional output previously prescribed specific paths (`.open-knowledge/external-sources/` etc.); now they interpolate the live content.dir value and describe the three-tier lifecycle as an optional pattern. | User 2026-04-14 session | ingest/research/consolidate `register()` signatures gain `config: Config`; `buildBody(...)` takes `contentDir` and inlines it. Shipped as `[V0-24.2]`. |
| D30 | **`ls` output enriches directories with `DirectoryMeta`** (not just the .md files inside) | T | LOCKED | No | Post-`[V0-24.2]` catalog teardown uncovered a regression: `exec("ls specs/")` against a directory containing only subdirs produced empty `enrichedPaths` — agents saw folder names but no content hint (file counts, most-recent md). The old persisted `INDEX.md` catalogs filled this role; D26 removed them without an on-demand replacement. Fix: classify ls-extracted paths via `fs.stat`; directories get a `DirectoryMeta` enrichment (`directMdCount`, `recursiveMdCount`, `childDirCount`, `mostRecentMd`, `truncated`) computed via a bounded recursive walk (cap 1000 entries). This IS the on-demand catalog view promised by D26. | User 2026-04-14 session ("shouldnt ls enrichment be better? like its supposed to include what that folder contains") | `enrichedPaths` element type widens to `EnrichedEntry = EnrichedMeta \| DirectoryMeta` (discriminated by `type: 'directory'`). New `enrichDirectory()` in `enrichment.ts`; `extractReferencedPaths` no longer filters ls entries by extension. Shipped as `[V0-24.3]`. |
| D31 | **MCP contract prescribes aggressive `[[wiki-link]]` authoring** across INSTRUCTIONS, write-tool descriptions, and workflow-tool bodies | P | LOCKED | No | Open Knowledge's value compounds with link density — backlinks are the primary navigation surface for agents, graph queries (`get_hubs`/`get_orphans`) reveal structure, and under-linked documents become islands. Prior INSTRUCTIONS mentioned wiki-links only in passing (frontmatter section); no nudge existed toward over-linking, redlinks, or `[[Page]]` vs Markdown `[text](./page.md)`. Now prescriptive across every authoring surface. Also caught a D26/D27 loose end: `init-content.ts` was silently stale — still referenced `.open-knowledge/catalogs/*/INDEX.md` (deleted by D26) and scaffold subdirs (deleted by D27). Rewritten to interpolate live `content.dir` (D28 pattern now extended to init-content) and drop the dead step 4 (catalog frontmatter editing). | User 2026-04-14 session ("abuse and use links to other documents a lot") | Six touch-points: (a) MCP connect-time INSTRUCTIONS gains a "Linking — lean on `[[wiki-links]]` aggressively" section; (b) init template AGENTS.md (`content/init.ts`) gains the same section; (c) `write_document` / `edit_document` DESCRIPTIONs get shorter nudges; (d) `research.ts` buildBody gains a "Link aggressively" step; (e) `consolidate.ts` buildBody gains the same; (f) `init-content.ts` rewritten — interpolates `content.dir`, removes dead catalog references, adds Link step. `ingest.ts` unchanged (raw preservation — wiki-linking external source content would violate the "no interpretation" rule). All guidance favors `[[Page]]` over `[text](./page.md)` since only wiki-links participate in the backlinks index. Shipped alongside `[V0-24.3]` (commits 05a89d1 + 4ff2754 + 0420042). |
