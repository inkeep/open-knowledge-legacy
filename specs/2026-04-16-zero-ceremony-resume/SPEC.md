...
| Port 3000 conflict across multiple projects (non-Claude clients, terminal-first, or MCP-spawn-first) | Medium           | High (silent-fail mode observed 2026-04-16) | Short term: `ok ui --port N` flag; document. Long term: D-033 (kernel-allocated default + dual-stack bind).                                    | Implementer (D-033 follow-up) |
...
- **Kernel-allocated **`ok ui`** default port + dual-stack bind (D-033).** Post-ship finding. Proposed: (A) change `ok ui` default port from 3000 to 0, (B) bind dual-stack loopback (`::` with `IPV6_V6ONLY=0`) to fail loud on collision. Enables G4 mechanically; Claude Code preview pane compatible via existing autoPort + D-032 proxy mode. Observed collision 2026-04-16.
