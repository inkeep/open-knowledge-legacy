...
| D-033 | **Post-ship finding: multi-project **`ok ui`** port-3000 collision.** `ok ui` default port 3000 (D-021) collides across contentDirs — per-contentDir locks don't see other projects' port claims. Dual-family loopback split (one process on `[::1]:3000`, another on `127.0.0.1:3000`) defeats OS-level EADDRINUSE. Mitigation plan (A+C, additive follow-up): (A) `ok ui` default port `0` kernel-allocated — MCP `previewUrl` already dereferences `ui.lock.port`, Claude Code's `launch.json` + autoPort + D-032 proxy mode still work; (B) two-socket loopback bind — explicitly listen on BOTH `[::1]` and `127.0.0.1` on the same port. The original spec wording ("`::` with `IPV6_V6ONLY=0`") was revised during implementation after empirical testing on macOS confirmed that a `::` wildcard bind with `ipv6Only:false` does NOT prevent a subsequent `127.0.0.1` bind on the same port — only explicit two-socket binding achieves cross-family collision-fail-loud. Two-socket also preserves loopback-only (no LAN exposure) which `::` wildcard did not. | X    | **DIRECTED (follow-up)** | No                        | Spec-time reasoning missed (i) per-contentDir lock scope leaves cross-project port claims unprotected, (ii) dual-stack loopback split defeats EADDRINUSE. Observed 2026-04-16 session.                                                                                                             | Session 2026-04-16; `lsof -iTCP:3000 -sTCP:LISTEN` showed stale `ok start` on `[::1]:3000` coexisting with `ok ui` on `127.0.0.1:3000`                                                                                     | Supersedes §14 "Port 3000 conflict" row; enables G4 mechanically in follow-up; no changes to shipped FR-1.1/D-021 until follow-up bet. |
| D-034 | **Post-ship finding: MCP stdio doesn't keep collab alive; cached port goes stale on server restart.** Idle-shutdown (D-017) counts `/collab` WebSocket upgrades only; MCP stdio uses HTTP fetch to `/api/*` endpoints so it's invisible to the counter. When no browser tab is open, MCP is the sole active client but does not hold the server alive. Server dies on 30-min idle; MCP's cached `http://localhost:<port>` is now a dead port; every subsequent tool call fails with "Server unreachable: fetch failed" and user must manually reconnect MCP (`/mcp` in Claude Code). Mitigation plan (additive follow-up): (A) MCP stdio opens a persistent WebSocket to `/collab` on startup as pure keep-alive signal — no new primitives, leverages existing WS-count mechanism; (B) MCP stdio re-reads `server.lock` on `fetch` failure and retries/respawns per existing FR-1.4 path.                                                                                                                                                                               | X    | **DIRECTED (follow-up)** | No                        | FR-1.6 intentionally excluded DirectConnections (to avoid CC1 + agent sessions keeping server alive forever — D-017). That was correct. But MCP's HTTP fetch path was not considered as a "client" at all — gap surfaced 2026-04-16 after observed 30-min idle-shutdown during a live MCP session. | Session 2026-04-16; MCP tool-call trace returning "Server unreachable: fetch failed"; FR-1.6 origin of intent                                                                                                              | Eliminates "tool calls fail silently after idle-shutdown" failure mode; no change to shipped FR-1.4/FR-1.6/D-017 until follow-up bet.  |
...
| Port 3000 conflict across multiple projects (non-Claude clients, terminal-first, or MCP-spawn-first) | Medium           | High (silent-fail mode observed 2026-04-16) | Short term: `ok ui --port N` flag; document. Long term: D-033 (kernel-allocated default + two-socket loopback bind).                           | Implementer (D-033 follow-up) |
| MCP stdio stale port after collab restart / idle-shutdown                                            | Medium           | High (bad UX observed 2026-04-16)           | Short term: user manually reconnects MCP (`/mcp` in Claude Code). Long term: D-034 (persistent WS keep-alive + lock re-read on fetch failure). | Implementer (D-034 follow-up) |
...
- **Kernel-allocated **`ok ui`** default port + two-socket loopback bind (D-033).** Post-ship finding. Proposed: (A) change `ok ui` default port from 3000 to 0, (B) bind both `[::1]` and `127.0.0.1` as separate sockets on the same port so a second same-port bind on either family fails loud with EADDRINUSE. Enables G4 mechanically; preserves loopback-only (no LAN exposure). Claude Code preview pane compatible via existing autoPort + D-032 proxy mode. Observed collision 2026-04-16.
- **MCP stdio keep-alive + self-healing reconnect (D-034).** Post-ship finding. Proposed: (A) MCP stdio holds a persistent WebSocket to `/collab` so idle-shutdown counter includes MCP; (B) MCP stdio re-reads `server.lock` on fetch failure and retries or re-spawns per existing FR-1.4 path. Eliminates "tool calls fail silently after server restart" UX. Observed 2026-04-16.
