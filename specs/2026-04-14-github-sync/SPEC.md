...
**Status:** Ready for implementation. Direction LOCKED (36 active decisions, 2 WITHDRAWN); all P0 open questions resolved.
...
- **Auto-sync.** Shadow-git L2 pipeline commits to shadow on every flush (unchanged). Sync engine squashes content changes into one parent commit per push cycle (every 60s) and fetches from origin every 30s (D26, D33, D37). Same `commitWip` plumbing, different `GitHandle`. Status badge + conflict banner are the only new UI surfaces for happy-path users.
...
- **[SUPERSEDED]** NG14: ~~Squash / rewrite auto-commit history before push.~~ Superseded by D33 (squash-before-push is the locked architecture). One parent commit per push cycle; no commit noise on origin.
- **[NOT NOW]** NG15: "Watching" remote for real-time updates (webhook or long-poll). v1 uses interval-poll fetch (30s per D37).
...
4. L1 auto-saves to disk → persistence L2 (30s idle) commits to shadow (WIP commit with timestamp, unchanged from today).
5. ~60s later: sync engine push tick → squashes content changes into one parent commit ("Auto-save: Updated onboarding.md, pricing.md"), pushes to origin. Badge briefly shows "Syncing..." → "Synced."
...
| Priority | ID | Requirement | Acceptance criteria | Notes |
|---|---|---|---|---|
| Must | FR21 | Remote detection on startup | On `createServer()` after `initShadowRepo()` and HEAD-drift check: run `git remote -v`. If no remote OR `sync.enabled=false`, sync engine stays `dormant`; UI shows no sync badge. **Decoupled intervals (D37):** pull uses `sync.pullIntervalSeconds` (default 30s, jittered ±15% → 25.5–34.5s); push uses `sync.pushIntervalSeconds` (default 60s, jittered ±15% → 51–69s). Prevents thundering-herd when multiple developers restart editors simultaneously (Syncthing ±25%, GitHub Desktop ±30s/hour precedent). | Gate for all sync functionality. Jitter applies to both periodic cycles independently; shadow L2 is debounce-based and doesn't need jitter. |
...
| Must | FR28 | Checkpoint version (renamed from Save Version per D21 revised) | Creates: (a) shadow checkpoint (existing behavior, `refs/checkpoints/<branch>/<sha>` with WIP refs as parents; resets WIP refs after), (b) parent-git named commit on current branch (user-provided message via dialog, or auto-generated from changed files), (c) parent-git lightweight tag `ok/v<N>` (sequential). Auto-pushed on next sync cycle. Stays in header as icon-only button (no text label); tooltip "Checkpoint version" on hover (per D21 revised). | Rename "Save Version" → "Checkpoint version" throughout UI copy, CLI output, toast messages. |
...
| Must | FR34 | Config schema: `sync.*` section | Keys: `sync.enabled` (three-state: absent=undecided, `true`=on, `false`=off — see D34, D36), `sync.pushIntervalSeconds` (default 60 — see D37), `sync.pullIntervalSeconds` (default 30 — see D37), `sync.autoCommit` (default true), `sync.autoPush` (default true), `sync.autoPull` (default true), `sync.commitMessage` (default `"auto"` = descriptive file-list per D25; or template string). Per-repo override via workspace config `.open-knowledge/config.yml`. | Consistent precedence: CLI flags > env > workspace > user > defaults |
...
- **Performance:** Clone of a 10MB repo completes in <30 seconds on broadband. Sync fetch cycle (idle) completes in <2 seconds on typical repo. DiffView in `conflictMode` loads <500ms for files <50KB. Push-time squash commit overhead <100ms (single `writeTree` + `commitTree` + `updateRef`).
- **Reliability:** Clone/push/pull failure leaves no partial state. Shadow commits at L2 (unchanged); parent commits at push-time only (D26). Force-push never happens automatically (NG2).
...
- **Operability:** `[clone]`, `[auth]`, `[head-drift]`, `[sync]`, `[sync-error]`, `[parent-commit]` structured logs. Sync state machine transitions logged at INFO. Error classification logs at WARN. Failed retries log at ERROR after exhaustion.
...
- **M5: Sync round-trip latency.** L1 save → appeared on origin (for the user's own commits). Target: <2 minutes p50 at default push=60s interval (D37).
...
│ ┌─ Project loaded ───────────────────────────────────────────┐   │
│ │ EditorHeader                                                │   │
│ │ ├── Checkpoint version (FR28; icon-only, tooltip on hover)  │   │
...
└────────────────────────────────┬─────────────────────────────────┘
         ...
         └───────────────────────┬────────────────────────┘
                                 │
┌────────────────────────────────▼──────────────────────────────────┐
│ Server: createServer({ contentDir })                               │
│                                                                    │
...
│ ├── Persistence L2 (existing, unchanged):                          │
│ │   commitToWipRef() commits to shadow only                        │
...
│ │   │   auth-error/disabled                   │                    │
│ │   │                                         │                    │
│ │   │ pull tick (30s, ±15% jitter):           │                    │
│ │   │  1. fetch origin                        │                    │
│ │   │  2. if behind + no conflicts: merge     │                    │
│ │   │                                         │                    │
│ │   │ push tick (60s, ±15% jitter):           │                    │
│ │   │  1. squash content changes into one     │                    │
│ │   │     parent commit (D33)                 │                    │
│ │   │  2. push to origin                      │                    │
│ │   │  3. on failure: classify + retry/pause  │                    │
│ │   │                                         │                    │
│ │   │ credential helper:                       │                    │
│ │   │  -c credential.helper=                   │                    │
...
│ └── Rollback, checkpoint version: user-initiated                    │
│                                                                    │
│ Shutdown (destroy, CC8-ordered):                                   │
│ 1. Stop watchers                                                   │
│ 2. Drain agent sessions                                            │
│ 3. L1 flush                                                        │
│ 4. L2 flush (shadow only)                                          │
│ 5. Stop SyncEngine                                                 │
...
sync:
  enabled: true              # three-state: absent=undecided, true=on, false=off (D34, D36)
  pushIntervalSeconds: 60   # push cadence (D37); ±15% jitter applied
  pullIntervalSeconds: 30   # pull cadence (D37); ±15% jitter applied
  autoCommit: true
  autoPush: true
  autoPull: true
  commitMessage: "auto"     # "auto" = descriptive file-list (D25)
...
- `[parent-commit]`: push-time commit result, tree hash, files changed, push result
...
| Push-time parent commit | Parent commit fails (dirty tree from external stage) | Git exit code non-zero | Shadow unaffected (L2 is shadow-only); parent retries next push cycle | Sync may briefly show "ahead" state |
...
| `mergeControls` limitation | Custom render function edge cases | Source-level verified viable (D27 LOCKED HIGH) | Custom render function within mergeControls | Verified — STOP_IF retained as safety net |
...
| D20 | Sync-D1 | **Auto-sync is OPT-IN, not default-on.** When a remote is detected, sync stays dormant until user explicitly enables it. Opt-in surfaces (auth-tier-dependent, optimized for non-interactive init — see D36): **(a) Config/CLI flag** (`sync.enabled: true` in workspace config, or `--sync` CLI flag) — primary path for agent-driven init; **(b) Tier A one-time prompt** — on first app open when remote detected + `gh` credentials available + `sync.enabled` absent from workspace config, show non-blocking prompt "This project has a GitHub remote. Auto-sync your changes? [Enable] [Not now]" — either choice persists to config, never shown again; **(c) Tier B/C sign-in prompt** — on first app open when remote detected + no stored credentials, show "This project is connected to GitHub. Sign in to sync your changes with your team. [Sign in]" — auth completion auto-enables sync (`sync.enabled: true` written on auth success); **(d) Badge `available` state** — fallback discovery for users who dismissed or missed the prompt. Once enabled: aggressive but batched — shadow commits at L2 (30s idle), parent commits at push-time only (squash-before-push, see D33), auto-pull on interval. **Timing RESOLVED via D37:** push=60s, pull=30s (decoupled intervals). | P | LOCKED (direction + UX); **OPEN** (timing) | No | Nick direction (2026-04-15): "they opt in to auto-commit." Miles direction (2026-04-15): optimize for non-interactive agent-driven init; auth-tier-aware prompts; signing in IS the opt-in for non-`gh` users. Research finding: NO production tool defaults to full-auto bidirectional git sync. Opt-in addresses industry concern #4 ("push is a public action — requires explicit intent"). CRDT layer structurally mitigates concerns #1 (merge conflicts) and #3 (no mobile merge). | `evidence/why-full-auto-git-sync-rare.md` — comprehensive survey; 0/6 tools default to full-auto | Three-state config: absent = undecided (prompt eligible), `true` = enabled, `false` = disabled. Agent sets config pre-init → prompt suppressed. |
| D21 | Sync-D2 | **Renamed "Save Version" → "Checkpoint version." Stays in header as icon-only button** (no text label). Tooltip on hover: "Checkpoint version." Same functionality: shadow checkpoint + parent-git named commit + `ok/v<N>` tag. Renaming: "Save" implies work is at risk, which is misleading when auto-sync is running. "Checkpoint" communicates "mark this moment as a milestone" without implying unsaved state. Icon-only saves header space for SyncStatusBadge coexistence. | P | **LOCKED (revised 2026-04-15)** | No | Original Nick direction demoted to overflow; Miles revised (2026-04-15): keep in header as icon-only with rename. Both Save Version and sync badge can coexist — they're different actions (intentional milestone vs. continuous sync). "Checkpoint" naming avoids confusion with auto-sync's continuous save semantics. | Nick direction (original); Miles revision (2026-04-15) | Rename throughout: UI copy, CLI output, toast messages, API responses. `ok/v<N>` tag pattern unchanged. Q-M5 coordination item resolved by this revision — no placement conflict. |
...
| D34 | new 2026-04-15 | **Auto-sync is opt-in, not default-on.** `sync.enabled` has three states: absent (undecided — prompt eligible), `true` (enabled), `false` (disabled). Opt-in is auth-tier-aware and optimized for non-interactive init (see D36): config/CLI flag is the primary agent path; Tier A gets a one-time prompt on first app open; Tier B/C users opt in by signing in (auth completion = sync enablement). Once enabled, full auto-sync activates (auto-commit of watched files + auto-pull + squash-push). | P | **LOCKED** | No | Research finding: NO production tool defaults to full-auto bidirectional git sync (0/6 surveyed). Developer tools actively reject auto-push (GitHub Desktop #2191 closed, VS Code #14885 closed). Opt-in addresses concern #4 ("push is a public action — requires explicit intent"). Nick: "they opt in to auto-commit." Miles: optimize for agent-driven non-interactive init; auth-tier-aware prompts. | `evidence/why-full-auto-git-sync-rare.md` | Aligns with industry posture while offering the capability. Once opted in, UX is Figma-like invisible sync. |
...
| D36 | new 2026-04-15 | **Opt-in UX is auth-tier-aware and non-interactive-init-friendly.** Three principles: (1) Init is non-interactive by default — agents and scripts set `sync.enabled` via config or `--sync` flag; no interactive prompts during init. (2) Auth tier determines the first-open prompt: **Tier A** (`gh` available) gets a one-time non-blocking prompt ("This project has a GitHub remote. Auto-sync your changes? [Enable] / [Not now]") because there is no auth surface to attach opt-in to; **Tier B/C** (no credentials) gets a sign-in prompt ("This project is connected to GitHub. Sign in to sync your changes with your team. [Sign in]") and auth completion auto-enables sync — signing in IS the opt-in. (3) Prompt eligibility is determined by `sync.enabled` config state: absent = undecided (show prompt once), `true`/`false` = decided (never prompt). Either prompt choice persists the decision to workspace config. | P | **LOCKED** | No | Miles direction (2026-04-15): optimize for agent-driven passive init where no human is present at setup time. Tier A users never go through auth UI, so they need an explicit sync decision moment. Tier B/C users demonstrate intent by signing in — a separate "enable sync?" step is redundant. Config-absent vs config-false distinction eliminates repeat prompts without a separate "has been prompted" flag. | D20, D34 (opt-in principle); FR29 badge `available` state (fallback discovery) | Auth and sync-opt-in are orthogonal: auth = "do we have credentials?" (per-invocation); sync-opt-in = "should the engine run?" (persisted config). Credential helper is wired up regardless of `sync.enabled` — manual `open-knowledge sync/push/pull` still works with auto-sync off. |
| D37 | new 2026-04-15 | **Decoupled push/pull intervals: push=60s, pull=30s.** Pulls are cheap read-only fetches; faster pull cadence means teammates see each other's changes sooner (~30s worst-case for inbound). Push at 60s balances "cloud sync" feel with reasonable load. Both well within GitHub rate limits (0.033 ops/sec vs 15 ops/sec soft limit, 1000x under). ±15% jitter still applies per FR21 (push: 51-69s, pull: 25.5-34.5s). Config keys: `sync.pushIntervalSeconds` (default 60), `sync.pullIntervalSeconds` (default 30). | T | **LOCKED** | No | Nick leaned push=60s, pull=30s; Miles confirmed. Research validated all cadences (30s/60s/120s) safe against GitHub rate limits. Prior art: SiYuan 30s, Logseq 60s, Obsidian-Git 10min, GitHub Desktop 1hr. Decoupled intervals match SiYuan's approach (separate push/pull timers). | `evidence/c1-git-editor-sync-dynamics.md`, `evidence/why-full-auto-git-sync-rare.md`, [GitHub rate limits discussion](https://github.com/orgs/community/discussions/44515) | Replaces single `sync.intervalSeconds: 120` from FR21. FR21 jitter (±15%) applies to both independently. |

| D38 | new 2026-04-15 | **Conflict abort behavior: `git merge --abort` + clear `conflicts.json` + sync paused until manual retry.** Four sub-decisions: **(1) Git operation:** `git merge --abort` reverts working tree to pre-merge state. **(2) State cleanup:** `conflicts.json` is cleared (deleted). Git state and app state stay in sync — no stale conflict metadata. On retry, the sync engine re-runs `git merge`, re-detects conflicts, writes fresh `conflicts.json`. The merge is fast (<2s per NFR) so the re-detection cost is negligible. **(3) Sync stays paused** until the user manually retries via badge popover "Retry merge" action or `/api/sync/trigger`. No auto-retry — prevents nag loop where the same conflict banner reappears every 30s. Badge transitions from `conflict` (orange) to `disabled` with tooltip "Sync paused — conflicts were not resolved. [Retry merge]". **(4) Partial resolution is all-or-nothing.** Abort discards any per-file or per-hunk resolutions the user made. Matches standard `git merge --abort` semantics. Saving partial progress (per-file strategies, per-hunk merge state) deferred to Future Work. **(5) Toast on abort:** `toast.info("Merge canceled — your changes are safe. Sync paused until you're ready to resolve.")` using existing sonner pattern. | P/T | **LOCKED** | No | Miles direction (2026-04-15). Clearing conflicts.json is simpler than caching stale conflict metadata — the retry path re-merges and re-detects anyway. Sync-paused-until-retry matches the user's intent ("I'll deal with this later") and avoids the nag loop that auto-retry would create. All-or-nothing abort is the simplest correct behavior for v1; partial-resolution persistence is a future iteration. | D27 (conflict resolver), FR27 (conflict persistence), FR29 (badge states) | Future Work: save per-file whole-file strategies (`mine`/`theirs`) across abort for replay on retry. Per-hunk merge state replay is substantially more complex and lower priority. |
...
| Q6 | Where does "Clone from GitHub..." live in the header? | P | P0 | No | **DELEGATED** to implementer. D21 revised: Checkpoint version stays in header as icon-only. Clone from GitHub goes in overflow menu or File menu. | **Resolved** (from clone precursor, updated by D21 revised) |
...
| ~~Q-M5~~ | ~~Save Version UI coordination~~ | P | P2 | No | **RESOLVED 2026-04-15:** D21 revised — renamed to "Checkpoint version," stays in header as icon-only button. No demotion to overflow, so no placement conflict with existing UI. | **Resolved** |
| Q-M6 | Should sync-CLI support `--dry-run`? | P | P2 | No | Add if user feedback requests. | **Open** — Future Work |
| ~~Q-TIMING~~ | ~~Sync interval: push + pull cadences.~~ | T | P0 | No | **RESOLVED 2026-04-15:** Decoupled intervals — `sync.pushIntervalSeconds: 60`, `sync.pullIntervalSeconds: 30`. Pulls are cheap reads; faster pull = teammates see each other's changes sooner. 60s push balances "cloud sync" feel with reasonable load. Both well within GitHub rate limits (0.033 ops/sec vs 15 ops/sec soft limit). ±15% jitter still applies per FR21. See D37. | **Resolved** |
| ~~Q-OPTINUX~~ | ~~Opt-in flow UX.~~ | P | P0 | No | **RESOLVED 2026-04-15:** Auth-tier-aware opt-in (D36). Config/CLI flag for agent init (non-interactive). Tier A: one-time prompt on first app open. Tier B/C: sign-in prompt; auth completion = opt-in. Badge `available` as fallback discovery. Three-state config (absent/true/false) prevents repeat prompts. | **Resolved** |
| ~~Q-ABORTUX~~ | ~~Conflict abort UX.~~ | P | P0 | No | **RESOLVED 2026-04-15:** D38 locks full abort behavior. `git merge --abort` + clear `conflicts.json` + sync paused until manual retry + all-or-nothing (no partial progress saved) + reassuring toast. Badge shows `disabled` with "Retry merge" in popover. | **Resolved** |
...
| Auto-commits produce git log noise on origin | Low | Low | Mitigated by D33 (squash-before-push): one commit per push cycle, not N. Shadow retains per-edit granularity. | Nick |
...
- ~~**Squash auto-commit history before push (F8).**~~ **SUPERSEDED by D33** (squash-before-push is the locked architecture). One parent commit per push cycle; no commit noise.
...
- **AI-generated commit messages for auto-commit + Checkpoint version.** Research shows table-stakes in commercial editors. Deferred pending cost + model dependency evaluation.
...
**Incoherencies flagged as open questions (ALL RESOLVED):**
...
- ~~Q-M5~~ **RESOLVED 2026-04-15:** D21 revised — renamed to "Checkpoint version," stays in header as icon-only button. No demotion to overflow, so no placement conflict.
- Q-M6: Should sync-CLI commands support `--dry-run`? Deferred to user demand.

**Decisions previously uncertain — ALL RESOLVED:**
- ~~D27 (Sync-D8)~~ **RESOLVED 2026-04-15:** `@codemirror/merge mergeControls` fitness confirmed via source-level analysis → LOCKED HIGH.
- ~~D28 (Sync-D9)~~ **RESOLVED 2026-04-15:** Token refresh strategy resolved — v1 without refresh → LOCKED HIGH.
...
**Post-merge steps (ALL COMPLETED):**
1. ~~Run /audit + /spec-challenger on merged spec~~ — completed Session 4
2. ~~Dispatch OSS git-sync research pass (resolves Q-M1, Q-M2; refines D27, D28)~~ — completed Session 3
3. ~~Resolve remaining merge-driven coordination items~~ — Q-M3, Q-M4 resolved via D9; Q-M5 resolved via D21 revision
4. ~~Verify completeness gate~~ — all P0 open questions resolved (Session 5)
5. ~~Transition Status~~ — "Ready for implementation" (Session 5)
