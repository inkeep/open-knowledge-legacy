...
| Must | FR12 | Editor empty-state: ProjectPicker component | Full-screen component shown when no project is loaded (no active documents). Three cards: "Clone from GitHub" / "Open folder on disk" / "Start fresh." "Clone from GitHub" opens the clone dialog (FR13). | New React component in `packages/app/src/components/` |
...
- **CLI:** Four commands (`start`, `init`, `mcp`, `preview`). No `clone`. No project-picking UI.
- **Editor:** Assumes content dir already exists. No empty-state. No "open project" flow. `FileTree.tsx` shows "No files yet." when empty but offers no onboarding action.
