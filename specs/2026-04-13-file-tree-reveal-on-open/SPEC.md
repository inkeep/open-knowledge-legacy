...
| D9 | Accessibility: active row has `aria-current="page"`; activation does not move focus. Rely on `aria-expanded` transitions for screen-reader announcement of newly-revealed ancestors (no extra `aria-live` region). Non-active rows use default `tabindex` (Tab-reachable in document order). | Product | LOCKED (amended 2026-04-13 post-review) | No | Don't steal focus: activations come from other widgets the user is intentionally interacting with. `aria-current="page"` is the standard pattern for "current item in a navigation set." SRs already announce `aria-expanded` changes; a dedicated `aria-live` region adds verbosity without new information. **Amendment:** originally specified a roving tabindex (active row `tabIndex={0}`, others `-1`). Pre-QA review (2026-04-13) flagged this as an a11y regression for keyboard-only users — partial roving tabindex without `ArrowUp`/`ArrowDown` key handlers makes non-active rows permanently unreachable. Pragmatic resolution: drop the partial implementation, keep `aria-current` only, accept default Tab order. Full WAI-ARIA Treeview (role=tree, arrow keys, focus management) deferred to Future Work. | User confirmation 2026-04-13 on R1a (A), R1c (B); design-challenge H1; pre-QA review M2 (2026-04-13) | Keyboard users get default Tab order; spatial orientation via `aria-current` and ancestor `aria-expanded`. |
...
- **Full WAI-ARIA Treeview keyboard navigation.** D9 originally specified roving tabindex; reviewer flagged that partial roving tabindex (without `ArrowUp`/`ArrowDown` handlers) is worse than default Tab order. The complete pattern — `role="tree"` / `role="treeitem"`, arrow-key navigation between rows, `ArrowLeft`/`ArrowRight` to collapse/expand, `Home`/`End` for first/last — is a significant effort and should land as a dedicated spec. Triggers to revisit: user reports of keyboard inefficiency in large trees, or a11y audit explicitly requiring treeview semantics.
