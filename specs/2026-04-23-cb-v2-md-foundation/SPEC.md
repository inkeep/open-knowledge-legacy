...
- **[NOT NOW]** NG22: Math / KaTeX inline + block. Markdown-pipeline concern, not descriptor.<br>_[Corrected 2026-04-29 post-ship: block math promoted to a 6th canonical descriptor `<Math>` (Phase 1) under [`specs/2026-04-29-math-canonical-and-syntax/SPEC.md`](../2026-04-29-math-canonical-and-syntax/SPEC.md). Phase 2 will add `$$…$$` + ` ```math ` parse paths via compat descriptors. Inline math (`$x$`) remains deferred there as NG-M11.]_
...
| ~~D-MF13~~ *(SUPERSEDED by D-MF17)* | ~~Callout descriptor has no foldable props.~~ Reversed in-session after challenger Angle-1 finding: the "compose `<Accordion><Callout>`" workaround is an OK-specific idiom not reflected in AI-agent training distribution, which defaults to Obsidian's `> [!note]-` foldable form. D-MF15 graceful-degradation preserves authoring intent but renders flat for the primary audience. See D-MF17 for the live decision. | P | **SUPERSEDED** | — | See D-MF17. Original rationale preserved for audit trail.<br>_[Corrected 2026-04-30 post-ship: confirmed in code — `Callout.tsx:64-151` ships `collapsible` + `defaultOpen` props per the D-MF17 reversal. Clipboard walker's chevron refactor in [`specs/2026-04-29-clipboard-component-contract-and-byte-preservation/SPEC.md`](../2026-04-29-clipboard-component-contract-and-byte-preservation/SPEC.md) FR-13 depends on these props being present. The SUPERSEDED annotation already covered the directive flip; this breadcrumb adds the audit-trail link to where the props are referenced for the clipboard pipeline._ |
