...
| FR-3 | `Video` descriptor with `src` string (required) + `title` string + `controls` boolean (default true) + `autoPlay`/`muted`/`loop`/`playsInline` booleans + `poster` string + `preload` enum + `children` reactnode (for `<track>` passthrough). **Pure HTML5 `<video>` wrapper** — no YouTube/Vimeo URL sniffing, no iframe emission, no `start` prop. Matches Mintlify's explicit-iframe pattern; Fumadocs has no Video component at all. Users authoring YouTube/Vimeo embeds write raw `<iframe>` in MDX (per Mintlify docs). | Per `reports/cb-v2-video-superset-research/REPORT.md` §"Cross-platform comparison" (narrowed per D-MF12 to match Mintlify/Fumadocs surface) |
| FR-4 | `Audio` descriptor with `src` string (required) + `title` string + `autoplay`/`loop`/`muted` booleans + `preload` enum + `children` reactnode (for `<source>`/`<track>` passthrough). `hasChildren: true` flipped from current false | Per `reports/cb-v2-audio-superset-research/REPORT.md`; controls always on (NG7: no confidently-broken chrome) |
