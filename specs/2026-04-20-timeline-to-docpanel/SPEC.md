...
**Situation:** The document timeline (version history) currently lives in a standalone right-side Sheet overlay (`TimelinePanel.tsx`), separate from the DocPanel sidebar which contains Outline, Backlinks, Outgoing Links, and Graph tabs. Versioning is per-file — the history API is scoped to `?docName=`. The editor has three modes: wysiwyg, source, and diff. Selecting a timeline entry puts the editor into diff mode.<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_
...
4. Clicks a version entry → main editor area switches to diff view showing changes between that version and current.<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_
5. Clicks another entry → diff updates to show that version vs. current. **No panel close/reopen needed.**
6. Clicks "Now" or exits diff → editor returns to previous editing mode.<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_
7. Optionally clicks "Restore" to roll back to a selected version.

### File switch during history browsing

1. User is viewing a diff (timeline tab open, entry selected).<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_
...
| ~~Must~~ Dropped   | FR-5 | ~~History button in EditorHeader activates Timeline tab~~ | Post-implementation decision (commit `7c2528f6`): the History button was removed entirely rather than rewired. The Timeline tab in DocPanel is the sole discovery surface. See D5 (updated). | Drop rationale: "timeline is always accessible via the DocPanel tab — no need for a separate header button."                                                                      |
| ~~Should~~ Dropped | FR-6 | ~~"Save Version" accessible from Timeline tab~~           | Post-implementation decision (commit `7c2528f6`): a duplicated Save Version button in the Timeline tab was removed — the EditorHeader button is the single surface.                          | Drop rationale: commit message — "Remove the duplicated Save Version button from TimelineContent (already in the EditorHeader)."                                                  |
...
- Entry selection: `TimelinePanel.onEntrySelect` → `EditorPane.handleEntrySelect` → sets `previewEntry` + `editorMode='diff'`<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_
...
- Primary flow: User clicks Timeline tab → entry list renders → user clicks entry → `onEntrySelect` bubbles to `EditorPane` → sets diff mode + `previewEntry` → `EditorArea` fetches historical content → renders `DiffView`.<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_
- Shadow paths:
  - **File switch during diff:** `activeDocName` changes → `useEffect` clears `previewEntry` + exits diff mode → Timeline tab re-fetches new file's history.<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_
...
| D5 | ~~History button in EditorHeader activates timeline tab + expands panel~~ → **History button removed from EditorHeader; Timeline tab in DocPanel is the sole discovery surface** | T            | LOCKED (superseded 2026-04-20) | No              | Post-implementation re-evaluation: the Timeline tab is always visible in the DocPanel tab bar, so a dedicated header affordance adds UI chrome without increasing discoverability. Commit `7c2528f6` drops the button, `onTimelineToggle` prop, and the `requestTimelineTab` one-shot plumbing. FR-5 reclassified as Dropped.                                                                                                                                                                                                                                                                                                          | commit `7c2528f6`                                                   | EditorHeader no longer exposes a History button. Users reach the timeline via the DocPanel Timeline tab. If discoverability proves insufficient, revisit with a different affordance (not a re-added button). |
| D6 | Diff mode persists across DocPanel tab switches                                                                                                                                  | X            | LOCKED                         | No              | Diff is about the document, not the active tab. Switching from Timeline to Backlinks while viewing a diff keeps the diff visible — the EditorHeader diff controls (Exit preview, Restore, layout toggle) and the diff-mode banner provide persistent context regardless of active tab. Exiting diff requires explicit user action: EditorHeader "Exit preview" or Timeline tab "Now" button.<br>\_\[Corrected 2026-04-27 post-ship: per-entry diff renders inline in the DocPanel side pane via ActivityPanelDiffView; main editor is no longer hijacked. Authoritative fix in [[specs/2026-04-24-timeline-diff-to-sidepane/SPEC]].]\_ | Design challenge #5                                                 | Tab-local "Now" escape (FR-8) is important for when user returns to Timeline tab while still in diff mode.                                                                                                    |
...
- **Requirements:** FR-1 through FR-4 (Must). FR-5 and FR-6 dropped post-implementation (see §6 and D5). FR-7, FR-8 (Should). See §6.
...
- **STOP\_IF:** The change requires modifying the history API contract, or requires new React context providers beyond prop threading.
- **ASK\_FIRST:** Any change to the EditorMode state machine beyond adding the `activeDocName` cleanup effect.
