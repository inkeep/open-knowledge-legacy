...
- **Explicit `application/octet-stream` fallback for sirv+mrmime misses.** `.mkv` is not in mrmime's mime table; sirv emits empty Content-Type (safe with `nosniff` but UX-broken — video won't play inline). Narrow mime-table extension in the sirv middleware. Defer until a user surfaces that the download-prompt UX isn't good enough for their workflow.
