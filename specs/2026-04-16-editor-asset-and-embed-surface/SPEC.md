...
## Post-finalization amendment (2026-04-24b) — dogfood-driven asset-click parity closure

**Summary.** User dogfooded the 2026-04-24a amendment on the local dev server and surfaced a dead-end click flow: dropping `vale_15.m4v`, clicking the chip, new tab opened at `http://localhost:5174/vale_15.m4v` which served `text/html` (Vite's SPA fallback returning `index.html`) — the tab rendered the editor shell instead of the video. Investigation traced this single surface bug to four distinct defects where shipped behavior silently refuted the spec's locked decisions. All four closed in three commits (A: serve surface; B: renderer surface; C: this amendment).

**Defects closed:**

1. **Serve-side violated D-M accept-all.** `ContentFilter.isExcluded` step-3 sibling-asset rule gated serve by a narrow `ASSET_EXTENSIONS` allowlist (~18 extensions), so anything outside the set fell through to Vite's SPA fallback as `text/html`. The inline comment at `packages/core/src/constants/upload.ts:80-83` admitted the allowlist was sized to match `WIKI_EMBED_EXTENSIONS` — **shape statement, not security rationale**. R7's claim that *"content-type at sirv time determined by extension (browser offers download rather than inline-execute for unknown types)"* was silently refuted because the filter rejected the request pre-sirv. Closed in Commit A: widen `ASSET_EXTENSIONS` to common user-drop extensions (video: m4v/mkv/avi/flv/wmv/mpeg/mpg; audio: flac/aac/opus; images: apng/heic/heif/tiff/bmp/ico; office: docx/xlsx/pptx; text/data: csv/txt/rtf/json; fonts: ttf/otf/eot) with defense-in-depth preserved (`.ts`/`.py`/`.sh` stay excluded); new `INLINE_RENDERABLE_EXTENSIONS` set drives `Content-Disposition: inline` for renderable / `attachment` for everything else (HedgeDoc GHSA-x74j-jmf9-534w posture, Docmost's extension-gated dispatch); Vite SPA-fallback hardening returns 404 for asset-extension paths sirv didn't serve.

2. **FR-A5 `createNodeInteractionBridgePlugin` was never landed.** Originally scoped in the 2026-04-23 asset-click amendment, the renderer-side plugin that would wire drop-time `wikiLinkEmbed` chip clicks through `dispatchAssetClick` was never written — `packages/app/src/editor/asset-dispatch/dispatcher.ts:5-6` referenced *"Commit 4 wires both"* and Commit 4 was not landed. The drop-time chip rendered via core's `renderHTML` as bare `<a target="_blank">`, which fell through to browser default new-window behavior. In Electron, `setWindowOpenHandler` intercepted and checked `matchAssetUrl` against `ASSET_EXTENSIONS`; for anything outside the narrow set, it returned `{action: 'deny'}` silently — dropping `.m4v` + clicking produced nothing, no error, no feedback. Closed in Commit B: app-layer `packages/app/src/editor/extensions/wiki-link-embed.ts` extension (NEW FILE) with `addNodeView()` mirroring the `wiki-link.ts` pattern (module-level counter + InteractionLayer registration + in-place DOM update on attr changes + per-branch DOM for image vs chip). Non-image chip clicks route through `handlePrimary` → `dispatchAssetClick`; image `<img>` renders inline with no registration (click is a PM-selection operation, unchanged).

3. **`classifyMarkdownHref` leading-slash regression.** Commit 3 of the 2026-04-24a amendment (server-absolute URLs) flipped post-roundtrip hrefs from doc-relative to `/`-prefixed; the classifier's leading-slash → `external` branch at `link-targets.ts:80` killed `internal-link.ts:131-133`'s asset-dispatch guard. `sourceForm === 'wikiembed'` entered the branch, but `target.kind === 'external'` (not `'asset'`) immediately bailed. Post-reload asset clicks opened the PropPanel instead of the file. Closed in Commit B via guard-softening at `internal-link.ts:131-150` (enter the asset branch when `sourceForm === 'wikiembed'` AND the href has an asset extension, regardless of `classifyMarkdownHref` return kind) + `resolveAssetProjectPath` leading-slash handling (accept `/`-leading paths as project-root-relative; containment preserved). **Classifier taxonomy cleanup — moving the asset-ext branch above the leading-slash guard in `classifyMarkdownHref` itself — is distinct scope and deferred to a follow-up PR.** See [`evidence/classifier-taxonomy-cleanup.md`](evidence/classifier-taxonomy-cleanup.md) for the Option A vs Option B blast-radius analysis + Docmost/Obsidian peer-editor comparison + the half-web/half-vault root-cause framing + three long-term resolution paths.

4. **`EXECUTABLE_BLOCKLIST_EXTENSIONS` missing macOS installer classes.** Pre-fix, `.dmg`/`.pkg`/`.scpt`/`.applescript`/`.terminal`/`.prefpane` reached `shell.openPath` unblocked — a user dropping `malicious.dmg` and clicking would mount the installer. `.webloc`/`.inetloc`/`.fileloc` (CVE-2022-22590 class) also absent. Cross-platform package formats (`.jar`/`.appimage`/`.deb`/`.rpm`/`.msix`/`.appx`/`.ipa`/`.apk`) and Windows shortcut classes (`.pif`/`.scr`/`.lnk`/`.url`) similarly unblocked. Closed in Commit A: widened blocklist covers all categories. Blocklist is strictly additive — no existing legitimate wiki-embed targets or ASSET_EXTENSIONS members land in the new set.

**What did not change:** upload-side D-M accept-all, streaming pipeline, emit-shape dispatch core logic (FR-1a matrix structure retained, set contents widened), basename index, managed-rename image-ref rewrite, FR-6 CC1 `ch:'files'` asset events, classifier's external-vs-asset-vs-doc taxonomy (the guard-softening localizes the fix without touching the classifier itself).

### Future work captured from this amendment

- **Classifier taxonomy cleanup for server-absolute asset hrefs (Option A).** See [`evidence/classifier-taxonomy-cleanup.md`](evidence/classifier-taxonomy-cleanup.md) for the full trade-off + coordinated edits map. Requires a backlink-graph design decision (new `asset` node kind vs external-fallthrough). Trigger: after 2026-04-24b merges, OR when a user surfaces that server-absolute asset hrefs aren't being tracked by backlinks.
- **Half-web / half-vault reconciliation (architectural).** The classifier inherited both Obsidian's vault-path shape AND markdown's external-URL kind. The leading-slash conflation is the seam. Long-term: either embrace Obsidian syntactic-only classification (drop `external` kind, derive from href-shape per consumer) OR embrace Docmost URL namespacing (emit assets as `/api/files/{id}`). Both are architectural pivots requiring their own spec. Trigger: second or third taxonomy-seam bug, OR an explicit product decision to prioritize one persona's fidelity.
- ~~**Explicit `application/octet-stream` fallback for sirv+mrmime misses.**~~ **Closed in the 2026-04-24b amendment (mrmime patch follow-up).** `packages/app/src/server/asset-serve-middleware.ts` extends mrmime's `mimes` record at module load with `m4v → video/mp4`, `mkv → video/x-matroska`, `flac → audio/flac`. Research confirmed this is mrmime's documented extension mechanism, the canonical MIME choices per IANA / RFC 9639 / WordPress Trac #24993 / Mozilla bug 875573, and safe under `X-Content-Type-Options: nosniff` (video/audio MIME types are never treated as scriptable by browsers under nosniff regardless of file contents). Chrome's `<video>` dispatch now plays `.m4v` / `.mkv` / `.flac` inline in the new tab. Pinned by `asset-serve-middleware.integration.test.ts` (video/mp4 + video/x-matroska + audio/flac assertions) and `asset-click-dispatch.e2e.ts` P9.21.
- **Unify `SCRIPTED_DOCUMENT_EXTENSIONS` drift** between `packages/cli/src/commands/ui.ts:48-56` (has `xhtml`) and `packages/core/src/constants/upload.ts:71-77` (has `svgz`/`mhtml` but no `xhtml`). Extract to a single source-of-truth set; import from both consumers.
- **Generic `createNodeInteractionBridgePlugin` factory** — when the second atom node type lands (Video/Audio/PDFViewer per D-F typed-component-nodes), extract the shared NodeView wiring from `wiki-link-embed.ts` into a factory. Inline-per-node is simpler for now.
- **Desktop E2E coverage for `shell.openAsset` IPC on `.m4v`.** P9.21 in `asset-click-dispatch.e2e.ts` covers the web path + the Electron IPC call-site mock. A full Electron E2E (spawn actual Electron, assert `shell.openPath` argument) requires extending the Playwright-Electron harness in `packages/desktop/tests/smoke/` — scope for a desktop-testing-infra PR.

**Authoritative cross-reference.** `~/.claude/plans/lets-do-this-transient-jellyfish.md` — full three-commit plan with decision table + reused-utilities map + verification section. Evidence file [`evidence/classifier-taxonomy-cleanup.md`](evidence/classifier-taxonomy-cleanup.md) captures the classifier follow-up's full scope.
