...
**Resolution.** Ship a TS-only engineering-health spec that applies the greenfield directive ("no deferred tech debt, architectural correctness over pragmatism") to the markdown pipeline. 21 P0 functional requirements (R3 split into R3a/R3b, R5 split into R5a/R5b; R24 added post-decomposition after a blocker diagnosis):
...
| **R24** | P0 | Structural PM↔mdast mark-hydration correctness | **Added during implementation (2026-04-16) after US-009/012/014 diagnosis** (evidence/r6-failure-modes.md §"Correction (US-009 iteration)"). Three coupled fixes needed to land the remaining Emphasis + Backslash CommonMark idempotence cases: (a) **Replace `hydrateMarks` in `@handlewithcare/remark-prosemirror`** with an outside-in greedy algorithm (per pseudocode in r6-failure-modes.md §"Proper fix"). Current algorithm partitions by `marks[0]`, loses nested emphasis+strong shape when spans share one mark. Fix via extension to the existing `bun patch` at `patches/@handlewithcare%2Fremark-prosemirror@0.1.5.patch`. (b) **Remove `excludes: '_'` from the `Code` mark extension**. Schema widening (precedent #9 allows widening) — enables emphasis to cover inline-code spans. Requires audit of editor render paths (italic-within-code visual rendering) — if editor render breaks, the fix is a render-layer adjustment, not a schema revert. (c) **Context-aware backslash-before-entity policy** in `to-markdown-handlers.ts` text escape — mdast-util-to-markdown's context-sensitive escaping currently produces incorrect output for `\*not emphasized*` and HTML-entity-decode adjacent sequences. Target deltas: Emphasis 127/132 → 132/132; Backslash 11/13 → 13/13; enables 2 `describe.skip`-guarded PBTs in US-014 to be unskipped. Green = all 19 formerly-NORMALIZE sections at 100% CommonMark idempotence (completing US-012), all 6 R14 PBTs enabled+green (completing US-014), US-009 AC fully satisfied. |
...
| MH-D23 | R24 added post-decomposition after US-009/012/014 diagnosis surfaced a shared structural blocker | Technical | LOCKED | No | Implementation iteration 9 (and re-confirmed in 12 + 14) empirically diagnosed that Emphasis 5/132 + Backslash 2/13 failures are NOT safeText escape bugs — they are structural: `hydrateMarks` partitions by `marks[0]` losing nested shape; `Code` mark's `excludes: '_'` prevents emphasis-over-inline-code; backslash-before-entity policy is context-sensitive. Under greenfield directive ("no deferred tech debt, architectural correctness"), three options were considered: (a) accept partials with deferral note, (b) extend spec with R24, (c) follow-up spec. Chose (b) — the diagnosis is fully-characterized in-tree (`evidence/r6-failure-modes.md`), the fix targets (`hydrateMarks` rewrite + schema widening + text handler) are clearly scoped, and we already hold a `bun patch` on the library, so the mechanism is in place. Out-of-spec deferral would be the wrong precedent. |
