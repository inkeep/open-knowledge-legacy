...
**The `onSyncError` callback added during PR #38's review loop** is the lateral surface that S4 completes. Runtime sync errors now call back (packages/app/src/editor/provider-pool.ts:106-108), but init-time throws during `setupObservers()` are still unguarded. S4 closes the symmetric gap.
