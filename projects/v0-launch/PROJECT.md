...
| **[Mike](#mike--knowledge-graph--content--search)**               | V0-8 graph view, V0-12 slug correctness                                                                                                              | V0-5 rename+backlink, V0-11 graph panels, V0-3 backlinks push, V0-21 dead-links | V0-13 suggest\_links              | V0-25 SQLite schema   |
| **[Andrew](#andrew--platform--ops--system-level-infrastructure)** | — (all shipped: V0-1 [#99](https://github.com/inkeep/open-knowledge/pull/99), V0-2 server [#106](https://github.com/inkeep/open-knowledge/pull/106)) |                                                                                 | V0-20 desktop build prep          |                       |
...
- **V0-1 — full feature shipped.** Spec + impl merged together: `server-lock.ts` (189 LOC), `process-alive.ts`, `standalone.ts` Phase-6 CC8 shutdown ordering, `commands/mcp.ts` `discoverServerUrl()` for zero-config port, Vite plugin lock participation, shared `resolveContentDir`/`resolveLockDir`, 19 unit tests + `standalone.test.ts` + `mcp.test.ts`. [PR #99](https://github.com/inkeep/open-knowledge/pull/99) (cafed34). **Andrew's V0-1 work is complete.**
- **V0-2 — server primitive only.** CC1 broadcaster, `__system__` Y.Doc, `isSystemDoc()` skip surface. [PR #106](https://github.com/inkeep/open-knowledge/pull/106) (88351e1). **Andrew's server-side work is complete.** Client-side consumer (ProviderPool pin, `FileSidebar` subscriber) is Dima's follow-up — see `specs/2026-04-13-v0-2-sidebar-push/SPEC.md` §9-10; no PR open as of 2026-04-14.
...
- **V0-7** — V0-1 lock infra now landed ([PR #99](https://github.com/inkeep/open-knowledge/pull/99)) — CC6 sequencing unblocked.
...
- **V0-2 client consumer owner** — server-side shipped ([PR #106](https://github.com/inkeep/open-knowledge/pull/106)); client-side consumer tagged to Dima but no branch/PR open. Confirm pickup or reassign.
...
**Owners.** **Miles** end-to-end (1 with his change-attribution / shadow-git territory — feature-owner principle applies). Sarah consulted on visual design (dot + tooltip micro-interaction). Backend infra already shipped (PR #62 degraded-boot signal); Miles wires the UI.
...
**Lateral.** Mike's Story 2 (suggest\_links, V0-13) depends on this — Unicode bug propagates to false-negatives. V0-5 (rename) shares rewrite infrastructure if PQ9 lands on option (a).
...
**Status.** ✅ **Shipped — [PR #99](https://github.com/inkeep/open-knowledge/pull/99) (cafed34, 2026-04-13).** Spec + full impl landed together: `server-lock.ts` (acquire/update/read/release + `ServerLockCollisionError`), `process-alive.ts` shared PID check, Phase-6 shutdown ordering in `standalone.ts`, `discoverServerUrl()` in `commands/mcp.ts` for zero-config port, Vite plugin lock participation, shared `resolveContentDir`/`resolveLockDir`, 19 unit tests plus `standalone.test.ts`, `mcp.test.ts`, `paths.test.ts`, `content-filter.test.ts`. CLAUDE.md updated with server-lock + CC8 documentation.
...
**Status.** Spec finalized (`specs/2026-04-13-v0-2-sidebar-push/SPEC.md`, all 9 OQs closed). ✅ **Server-side primitive shipped — [PR #106](https://github.com/inkeep/open-knowledge/pull/106) (88351e1, 2026-04-13)**: `CC1Broadcaster`, `__system__` direct connection at startup, `isSystemDoc()` skip surface across 8 subsystems, reserved-name policy, L1 integration test. **Andrew's server-side obligation is complete.** Client-side consumer (ProviderPool pin, `main.tsx` mount, `FileSidebar` subscriber, L2 Playwright) owned by Dima — no branch/PR open as of 2026-04-14; confirm pickup.
...
**Forward.** If this works well: post-v0, evaluate deprecating `read_document`, `list_documents`, `search` in favor of `exec("cat")`, `exec("ls")`, `exec("grep")`. Reduces tool count from \~14 to \~5 (exec + write\_document + edit\_document + undo + redo). Root PROJECT.md XQ1 resolves.
...
- **V0-13** (suggest\_links): Tim owns MCP tool registration. Mike is primary — see Mike section.
...
**Status / owner signals.** Not started. PR #40 (open spec) defines MCP write\_file conventions — coordinate API shape. PR #53 (open) adds wiki-link context menu — UI pattern reusable.
