...
3. No `agentId`, no principal → no contributor entry (anonymous, preserves the D22 invariant for the bootstrap-failure case).
...
**Default summaries.** When no summary is provided, MCP-driven `rename_document` and `rollback_to_version` substitute a server-generated default (`"Renamed <from> → <to>"` or `"Restored to <sha-short>"`) through the same `normalizeSummary` path. `write_document`, `edit_document`, and `rename_folder` have no default — an absent summary means an absent bullet. (`rename_folder` deliberately omits a default because one folder-level summary would be applied to every affected-doc contributor entry; users are better served by a single explicit summary or none than by a synthesized "Renamed folder X → Y" copy on every doc.)
