...
<Callout type="info" title="Historical record">
These validation results are from the init spike (March 2026). They confirmed the foundation architecture decisions that the project is built on. The codebase has evolved since -- see [Architecture](/internals/architecture) and [Service Topology](/internals/service-topology) for current state.
</Callout>
...
| V6 | **PASS** | Component block renders React component with live editing, survives markdown round-trip |
...
4. **JsxComponent extension** -- originally intercepted `code` tokens with `lang === 'jsx-component'`; now parses directly from `mdxJsxFlowElement` mdast nodes via the remark-prosemirror handler pipeline
...
## V6: Component block editing

Originally validated as a void node with `atom: true` that intercepted fenced code blocks. Since Component Blocks v2, the `jsxComponent` extension is a non-atom block container (`content: 'block*'`, `isolating: true`) parsed directly from `mdxJsxFlowElement` mdast nodes. Known components (18 built-in fumadocs-ui descriptors) get live React preview, auto-generated PropPanel, and editable children. Unknown components get a name badge with editable children. The γ serialization pattern preserves byte-identical round-trip for unedited nodes (`sourceRaw`) and reconstructs from structured state for edited nodes.
