...
createServer(options)          ← synchronous, returns ServerInstance
  ...
  └─ ready = initAsync()       ← async background init
       ├─ History repo init    ← git plumbing (~50-200ms)
...
destroy()
  ...
  ├─ Phase 5: History release  ← write last-known-head + destroyHistoryRepo() — always runs
...
- **Phase 5 before Phase 6:** The history writer lock protects git plumbing; release it before the process-level server lock so a restart sees a clean history root even if Phase 6 throws.
...
If the history repo failed to initialize (the `historyRef.current` closure is nullish), rescue writes are skipped entirely and all still-loaded documents are reported as lost.
...
After a successful `destroy()`, all writes that reached the server-side Y.Doc are flushed to disk (L1) and committed to the history git repo (L2). The guarantee boundary is:

| Layer                                  | Guaranteed by `destroy()` |
| -------------------------------------- | ------------------------- |
| Server Y.Doc → markdown on disk (L1)   | Yes                       |
| Disk → history git commit (L2)         | Yes                       |
