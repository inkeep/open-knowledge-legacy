...
destroy()
  ...
  ├─ Phase 5: History release  ← write last-known-head + destroyShadowRepo() — always runs
...
If the history repo failed to initialize (the `shadowRef.current` closure is nullish), rescue writes are skipped entirely and all still-loaded documents are reported as lost.
