...
createServer(options)          ← synchronous, returns ServerInstance
  ...
  └─ ready = initAsync()       ← async background init
       ...
       └─ HEAD watcher start   ← .git/HEAD polling (if .git/ exists)
```
...
destroy()
  ...
  ├─ Phase 4: L2 flush         ← persistence.flushPendingGitCommit() — disk → git
...
- **Phase 3 before Phase 4:** L1 (`onStoreDocument`) writes markdown to disk and schedules the L2 git-commit timer. Running L2 before L1 would drain an empty commit queue -- this was the original ordering bug that caused silent data loss.
...
- **Clean exit:** `log.info` with payload `{ flushedCount, durationMs }` and message `"shutdown flushed N documents in Mms"`.
...
```json
{"level":"info","msg":"[server] shutdown flushed 3 documents in 247ms","flushedCount":3,"durationMs":247}
```
...
The client-side transmission buffer (~16ms between a keystroke and its WebSocket flush to the server) is a separate, smaller concern. For CLI users, this residual window is an order of magnitude smaller than the 2--10 second server-side window this fix closed. For Electron, the client-side drain barrier is handled by the desktop app's `ProviderPool.flushAllProviders()`.
