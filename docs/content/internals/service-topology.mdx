...
Located at `<projectRoot>/.git/open-knowledge/` (SPEC 2026-04-21-shadow-repo-single-mode). If the project has no `.git/` when the server starts, `ensureProjectGit` auto-runs `git init --initial-branch=main` before the shadow is created (fail-fast on git missing). Pre-spec integrated shadows at `.git/openknowledge/` are silently rename-migrated in-place once per repo via a `[shadow-migration]` log entry. The shadow repo provides auto-save history and attribution without touching the project's own git state.
...
- **Structured commit bodies** -- every commit carries one `ok-actor:` JSON line per contributing actor (principal, agent_session, agent_type, display_name, color_seed, docs); `parseOkActor` / `formatOkActor` in `packages/core/src/shadow-repo-layout.ts` are the schema helpers
