...
`/api/*` routes are also the boundary for local companion integrations. The gbrain search integration adds `/api/gbrain/status` and `/api/gbrain/search` to the API Extension: renderers use relative fetches, while the server owns fixed-argument `gbrain` CLI execution, source-path matching, result normalization, and source filtering before data reaches the sidebar.
