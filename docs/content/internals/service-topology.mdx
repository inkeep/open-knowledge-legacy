...
- Y.Doc serialized to markdown — body composed from `Y.XmlFragment`, frontmatter recomposed from `Y.Map('metadata')` (one entry per property — `Y.Text` for editable strings, `Y.Array<Y.Text>` for lists, primitives for atomics). When the per-key map still matches a legacy single-string mirror, the original YAML bytes are written verbatim so comments, blank lines, and scalar styles round-trip losslessly
