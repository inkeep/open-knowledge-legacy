...
30 min no /collab clients  ← idle-shutdown fires
      ...
      └─ await destroy()
           ├─ Phase 1–5 (watchers, sessions, L1, L2, history lock)
           └─ Phase 6: release server.lock  ← LAST, in try/finally
```

**Only `/collab` WebSocket upgrades count.** DirectConnections opened by the CC1 broadcaster and `AgentSessionManager` are invisible by design (D-017). A stale agent session doesn't keep the server alive overnight — an acceptable trade (NG10) since everything important is already persisted to the history repo.
