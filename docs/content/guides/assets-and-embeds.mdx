...
Nothing is written back to `.obsidian/` or to `.open-knowledge/config.yml`. When a vault provides a value, it takes precedence over `config.upload` — the pragmatic assumption is "if there's a vault, follow the vault."
