...
The `clone` command clones the repository, scaffolds `.ok/` via `init`, and starts the editor. Pass `--json` for machine-readable JSONL progress output (used by the editor's subprocess relay).
...
Tokens are stored in your OS keychain via `@napi-rs/keyring` (macOS Keychain, Windows Credential Manager, Linux Secret Service). If the keychain is unavailable, tokens fall back to `~/.ok/auth.yml` with `0600` permissions.
...
Sync uses opinionated defaults that are not YAML-tunable. The previous `sync.*` schema fields (`pullIntervalSeconds`, `pushIntervalSeconds`, `autoCommit`, `autoPush`, `autoPull`, `commitMessage`) were dropped to keep the engine's lifecycle in one place — pull every 30 s, push every 60 s, auto-commit + auto-push + auto-pull on, default commit message. Loose-mode YAML preserves any leftover `sync.*` keys on disk, but the engine ignores them; run [`ok config migrate`](/guides/cli-reference#config-migrate) to clean them up.

See the [Configuration guide §Sync configuration](/guides/configuration) for the full rationale.

### Disabling sync

Sync auto-enables when a remote is detected. There is no user-facing on/off switch; the engine starts when a remote exists and stays out of the way otherwise. Sync also disables automatically when a push is rejected due to branch protection — a toast explains the reason.
