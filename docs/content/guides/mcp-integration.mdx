...
| `frontmatter_patch`                           | Connected | Set / create / delete frontmatter properties via JSON Merge Patch (RFC 7396)                 |
...
### `frontmatter_patch` — typed property edits

Frontmatter is stored per-property in the CRDT — concurrent edits to *different* keys (a human editing `title` in the property panel while an agent updates `tags`) merge cleanly at the field level, no last-write-wins overwrite. Use `frontmatter_patch` to set, create, or delete properties via JSON Merge Patch (RFC 7396):
...
A key with a value sets or creates; a key with `null` deletes; missing keys are unchanged. Atomicity is reject-or-commit — if any value fails schema validation the whole patch rejects with a per-key error report.

Type inference picks the widget class from value shape (ISO 8601 string → date, boolean → boolean, number → number, `string[]` → list, else text). Pass an optional `types` map to override per key — useful when an ISO 8601 string should stay as plain text instead of becoming a date.

`edit_document` rejects FM-intersecting find/replace calls with HTTP 400 and a hint pointing at `frontmatter_patch`. Body edits stay on `edit_document`; frontmatter edits route through `frontmatter_patch`. Mixing the two in one batch — body edit via `edit_document`, FM edit via `frontmatter_patch` — is the supported pattern.

### `summary` on write tools

The five write tools — `write_document`, `edit_document`, `frontmatter_patch`, `rename_document`, `rollback_to_version` — accept an optional `summary: string` describing the intent of the edit in outcome phrasing (e.g. `"Fixed token-refresh race"`, not `"Added 3 lines"`). Summaries are persisted per-contributor to the shadow-repo attribution journal and render as bullets on the document's [[timeline|Timeline]] row, so readers can scan recent agent activity without opening each diff.
