...
| Tool                                          | Description                                                                                  |
| --------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `init-content`                                | Bootstrap knowledge articles from the codebase                                               |
| `ingest`                                      | Capture an external source (URL or file) as raw reference material                           |
| `research`                                    | Gather sources and write provisional research findings                                       |
| `consolidate`                                 | Promote research into a canonical article                                                    |
| `read_document`                               | Read a document with enriched frontmatter + shadow-repo history                              |
| `search`                                      | Search document contents                                                                     |
| `exec`                                        | Read-only bash-like commands (cat / ls / grep / find / head / tail / wc / sort / uniq / cut) |
...
| `suggest_links`                               | Suggest missing wiki-links for a target page                                                 |
| `write_document`                              | Write content to a document through the CRDT                                                 |
| `edit_document`                               | Find-and-replace on a document through the CRDT                                              |
| `rename_document`                             | Rename a document and rewrite inbound links                                                  |
| `rollback_to_version`                         | Restore a document to a historical shadow-repo commit                                        |
| `get_history`                                 | Query the shadow-repo version timeline                                                       |
| `save_version`                                | Checkpoint the project to a named shadow-repo commit                                         |

All docName-producing tools emit `previewUrl` in their result (single-doc tools top-level, list-producing tools per-row with an additional top-level `ui: {baseUrl, port}` block).
