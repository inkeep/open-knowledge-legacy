...
Then remove the `content.include` and `content.exclude` keys from your `config.yml` by hand. If you forget, OK refuses to start with a source-located `REMOVED_KEY` error pointing at the offending lines, so you can't miss them.
