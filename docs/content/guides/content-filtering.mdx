...
Drop a `.okignore` next to your `.gitignore` at the project root. It uses gitignore syntax, parsed by the [`ignore`](https://www.npmjs.com/package/ignore) npm library — a JavaScript implementation of the gitignore pattern spec (Git itself uses a separate C implementation; the spec is shared but the engines are not).
