...
The Timeline lives as a tab in the **document panel** on the right (the same panel that houses Outline, Backlinks, Outgoing Links, and Graph). Click the **Timeline** tab (clock icon); click the **Hide panel** button (top-right of the editor area) to hide the document panel entirely. Switching to a different document automatically refetches the Timeline for that document.
