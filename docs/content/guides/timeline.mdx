...
When a restore succeeds, any other rows you had expanded auto-collapse — their diffs were comparing against a baseline that no longer exists. Re-expand them to see fresh diffs against the just-restored content.

This is the same restore-to-a-prior-snapshot semantic that's lived in Open Knowledge since the panel shipped, just relocated from a sticky bar above the editor to a per-row affordance in the panel itself. The Activity Panel's per-agent **Undo** is a different operation — see [[agent-activity-panel]] — scoped to a single agent's session-undo stack rather than a wholesale snapshot replacement.
