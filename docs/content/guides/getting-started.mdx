...
3. Injects the Open Knowledge section into the editor's agent instruction file — `CLAUDE.md` for Claude Code, `.cursorrules` for [[Cursor]], `.windsurfrules` for [[Windsurf]]. This is what teaches the agent to use `exec`, `write_document`, and the other tools correctly.
