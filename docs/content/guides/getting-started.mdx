...
---
<Callout type="info" title="Installed from the macOS DMG?">
The desktop app launches a [first-launch MCP consent dialog](/guides/mcp-integration#desktop-app-first-launch-consent) that writes user-level MCP entries to every detected editor — so your AI tools can already see Open Knowledge without any terminal commands. This guide covers the CLI-driven path for project-scoped scaffolding (`.ok/`, `AGENTS.md`, `launch.json`). The two paths coexist; run `ok init` on any project where you want that scaffolding landed.
</Callout>
...
This clones the repository, scaffolds `.ok/`, and starts the editor. Authentication is handled automatically -- if you have the [GitHub CLI](https://cli.github.com/) installed, its session is reused. Otherwise, you'll be prompted to sign in via Device Flow or paste a Personal Access Token.
...
1. Scaffolds `.ok/` with `AGENTS.md`, `config.yml`, and `.gitignore`, plus a project-root `.okignore` file (commented header explaining gitignore syntax — edit it to exclude folders or files from the document index). No content subdirectories are created — content lives wherever `config.yml` points, defaulting to the repo root. If the project has no `.git/` yet, `init` runs `git init --initial-branch=main` first so Open Knowledge's attribution journal has a parent repo to hang off of.
...
| VS Code        | `<App Support>/Code/User/mcp.json`                                     | User-global |
...
.ok/
  ...
  config.yml         — Workspace config (content dir, server host, MCP tool caps)
  .gitignore         — Excludes per-machine runtime state (cache, locks, sync-state, principal identity, spawn diagnostics)
  cache/             — Derived data (gitignored)
.okignore            — Project-root exclusion patterns (gitignore syntax) for the document index
.git/                — Initialized via `git init --initial-branch=main` if not already present
.git/ok/             — Attribution journal (bare git repo; see Service Topology)
...
This works before or after `init` and reflects the current state of your config. Useful after editing `.okignore`.
...
<Cards>
  ...
  <Card title="Configuration" href="/guides/configuration">
    Customize ports, content directory, and server behavior.
  </Card>
  <Card title="Content filtering" href="/guides/content-filtering">
    Exclude files via `.gitignore` and `.okignore` (gitignore syntax).
  </Card>
