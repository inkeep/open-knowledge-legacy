...
1. Scaffolds `.ok/` with `AGENTS.md`, `config.yml`, and `.gitignore` (no content subdirectories — content lives wherever `config.yml` points, defaulting to the repo root). If the project has no `.git/` yet, `init` runs `git init --initial-branch=main` first so Open Knowledge's attribution journal has a parent repo to hang off of.
