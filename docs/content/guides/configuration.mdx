...
Like scalar fields, per-row edits commit on blur or Enter — but the wire write is always the **whole `folders[]` array**, mirroring the all-or-nothing transactional semantics of the `set_folder_rule` MCP tool and `applyFolderRulesUpsert` core helper. Validation runs against the merged config; if any rule fails (e.g. an empty `match` slips through, or a sibling row is invalid for an unrelated reason), no rules land on disk and the offending **Match** field flashes.
