...
Uploads stream to disk end-to-end with on-the-fly sha256, so there is no user-facing upload cap — drop a 500 MB video and it works. The only rejection axis is disk fullness, which surfaces as HTTP 507 (`storage-full`). Legacy configs that still carry `upload.maxBytes` parse cleanly but log a one-time deprecation WARN at boot so you can clean up `config.yml`.

Projects that contain an `.obsidian/app.json` file pre-populate `attachmentFolderPath` and `emitFormat` from the vault's `attachmentFolderPath` + `useMarkdownLinks` settings at server startup (non-destructive — nothing is written back to `.obsidian/`). Precedence is **user > vault > default**: anything you set in `config.upload` wins, and vault detection only fills in where you were silent. To pin a setting explicitly, set it in `.open-knowledge/config.yml` — you do not need to touch `.obsidian/`.
