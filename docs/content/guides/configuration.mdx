...
The schema artifact is published to npm at `@inkeep/open-knowledge/dist/config-schema.json`, generated from the same Zod source the runtime validates with. SchemaStore registration ships a `fileMatch` of `**/.open-knowledge/config.{yml,yaml}` so editors that consult SchemaStore (most JetBrains IDEs by default) auto-detect without needing the magic comment.
