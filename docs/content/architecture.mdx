...
- **JsxComponent** -- custom void node extension for embedding React components as fenced code blocks with `jsx-component` info string, slash insertion, and inline primitive-prop editing
...
The `JsxComponent` extension intercepts `code` tokens with `lang === 'jsx-component'` at priority 60 (above codeBlock's 50). Known built-ins (`Callout`, `Tabs`, `CodeGroup`, `Steps`, `Accordion`, `Card`, `Embed`) are inserted from a slash menu in WYSIWYG mode, rendered through a registry-backed `ReactNodeViewRenderer`, and expose an inline prop panel for supported primitive props. Prop edits rewrite the stored JSX string in place, and the updated fenced block remains visible in source mode through the existing observer sync path.
