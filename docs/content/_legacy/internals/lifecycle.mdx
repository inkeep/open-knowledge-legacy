...
| `ok stop`   | SIGTERM live `ok start` + `ok ui` processes. Also targets `foreign-host` entries whose PID is locally live (macOS hostname drift across DHCP/VPN/sleep). Leaves stale, corrupt, and truly-cross-host locks alone — those belong to `ok clean`. Exits 1 only on EPERM kill failure. |
| `ok clean`  | Prune stale (dead-pid, regardless of hostname) or corrupt-JSON lockfiles. Leaves live and `foreign-host`-with-live-PID locks alone.                                              |
