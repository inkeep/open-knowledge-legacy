...
- ContentFilter drops excluded files (`.gitignore` + `.okignore` rules)
...
- Disk bytes read directly from `Y.Text('source')` per the Y.Text-is-truth contract (precedent #38) — `prependFrontmatter(stripFrontmatter(ytext).frontmatter, stripFrontmatter(ytext).body)`, equivalent to `ytext.toString()` byte-for-byte. So both FM (indentation, scalar styles, comments, blank lines) AND body (source-form delimiters like `__foo__`, ATX trailing hashes, indented code blocks) round-trip losslessly. The XmlFragment serialization step that the pre-contract pipeline used was deleted.
- Quiescence gate (`isDocQuiescent(doc)`): defers the write if `afterAllTransactions` hasn't settled since the last user-origin transaction. After 8 consecutive deferrals (~16 s of sustained typing under the default 2 s debounce) the gate force-flushes anyway and emits `persistence-force-flush-during-burst`. Bounds staleness so collaborative bursts can't leave material work undurable.
- Pre-write sanity check: asserts `normalizeBridge(ytext) === normalizeBridge(serialize(fragment))`. On mismatch, emits `bridge-invariant-violation` (site `'persistence'`) and writes ytext bytes anyway — Y.Text wins by contract; fragment is wrong and gets re-derived on the next settlement
...
| Content filter  | Basic .md filtering                                   | Full `.gitignore` + `.okignore` (root + nested)                                        |
