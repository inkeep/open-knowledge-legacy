...
> **Legacy guide.** This page reflects an earlier schema that included `server.*`, `github.oauthAppClientId`, and `mcp.*` fields. Those fields were removed; their values are now built-in constants in `@inkeep/open-knowledge-core` and runtime overrides survive as the `--host` flag, `HOST` / `OPEN_KNOWLEDGE_GITHUB_CLIENT_ID` / `OK_MCP_AUTOSTART` env vars. The `set_config` MCP tool was removed alongside them. See the current [Configuration reference](/reference/configuration) and [MCP reference](/reference/mcp) for the supported surface.
...
```mermaid
graph LR
  A[Zod defaults] --> B["~/.ok/config.yml"]
  B --> C["./.ok/config.yml"]
...
| ------------------------------------- | ---------------------------- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `content.dir`                         | `string`                     | `"."`                     | Content directory relative to project root. Exclusions live in `.okignore` (root + nested) — see [Content filtering](/guides/content-filtering).                                                       |
...
| `OK_BRIDGE_VIOLATION_DEBOUNCE_S`  | Debounce window (in seconds) for the bridge-invariant watchdog's per-`(site, doc)` rate-limiter. Default `60`. Lower values surface every violation; higher values throttle a single misbehaving (site, doc) tuple from drowning telemetry. The first event in any window is always emitted; subsequent identical events bump `bridgeInvariantViolationsSuppressed`. The separate `bridge-tolerance-applied` event has its own per-`(site, class)` debounce. |
| `OK_BRIDGE_THROW_ON_VIOLATION`    | Force-throw mode for the bridge-invariant watchdog. When `1`, `BridgeInvariantViolationError` propagates regardless of `NODE_ENV` (default behavior throws under `NODE_ENV=test` only). Use during local debugging to surface bridge regressions loudly. When `0`, suppresses throws even in tests — handy when running a known-broken seed under the fuzzer without aborting. |
