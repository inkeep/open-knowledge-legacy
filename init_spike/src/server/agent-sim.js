...
async function agentWriteRaw() {
    const res = await fetch(`${BASE_URL}/api/agent-write`, { method: 'POST' });
    return (await res.json());
}
async function agentWriteMarkdown(markdown, position = 'append') {
...
if (count > 1) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        console.log(`  Write ${i + 1}/${count}: ${result.ok ? 'OK' : 'FAIL'} ${result.timestamp ?? result.error}`);
        if (i < count - 1) {
...
//# sourceMappingURL=agent-sim.js.map