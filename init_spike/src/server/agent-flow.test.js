...
function getDoc(conn) {
    ...
    if (!doc)
        throw new Error('DirectConnection has no document');
    return doc;
}
/** Get the default XmlFragment from a DirectConnection */
function getFragment(conn) {
...
describe('Agent write → Editor reflection', () => {
    ...
    test('agent write during source mode: non-conflicting paragraphs merge on toggle-back (three-way merge)', async () => {
        ...
        const userEdited = snapshotMarkdown.replace('Paragraph A — user will edit this in source', 'Paragraph A — USER EDITED IN SOURCE MODE');
        ...
        const result = threeWayMerge(getDoc(conn), fragment, snapshotMarkdown, userEdited, mdManager, schema);
    ...
    test('conflicting divergence: user and agent edit same paragraph — user wins, document valid', async () => {
        ...
        const userEdited = snapshotMarkdown.replace('Paragraph A — both will edit this', 'Paragraph A — USER VERSION');
        // Step 4: Agent ALSO edits paragraph A via DirectConnection (conflict!)
        // Agent modifies the first paragraph's text in-place
        await conn.transact((doc) => {
            const frag = doc.getXmlFragment('default');
            const firstParagraph = frag.get(0);
            const textNode = firstParagraph.get(0);
    ...
    test('agent in-place modification to existing paragraph preserved when user does not edit it', async () => {
        ...
        const userEdited = snapshotMarkdown.replace('Paragraph A — user will edit this', 'Paragraph A — USER EDITED');
        // Step 4: Agent modifies paragraph B IN-PLACE (not an append — a modification)
        await conn.transact((doc) => {
            const frag = doc.getXmlFragment('default');
            const secondParagraph = frag.get(1);
            const textNode = secondParagraph.get(0);
            textNode.delete(0, textNode.length);
...
//# sourceMappingURL=agent-flow.test.js.map