...
- **Personal KM** tools (Obsidian) nailed markdown file ownership and editor polish, but are philosophically single-player and treat agents as silent file-mutators.
...
| **U3** | **"WYSIWYG and source are the same document."** I flip to raw markdown to fix an escaped character, then flip back — same cursor, same state, and agent edits that landed while I was in source mode are still there. | Every WYSIWYG competitor: source and rich modes are two different editors (if source exists at all). Obsidian's Live Preview is the closest but single-player. | Bidirectional observer bridge between `Y.XmlFragment('default')` and `Y.Text('source')` inside one Y.Doc. → §3 L3/L4 |
...
|---|---|---|---|---|
| **Personal KM / "second brain"** | Obsidian (1.5M users, 21K-⭐ `obsidian-skills` agent ecosystem) | Markdown-canonical files users own. Best-in-class single-player editor (Live Preview, 2.7K plugins). Deep agent *format literacy* via `obsidian-skills` — teaches 33+ agents to read/write Obsidian formats. Zero LLM compute in product. | **Philosophically single-player** (6-year-old FR, 2.2K votes, deliberately unanswered). Silent agent writes (LWW, no presence, no attribution). Proprietary app, desktop-only, no web/embed story. `obsidian-skills` is format literacy, not co-creation. | Keep the file ownership + agent format literacy. Add multiplayer, agent-as-visible-peer, draft/review primitives, and an embeddable web editor. |
...
5. **True OSS core with a cloud grow-up path.** Only AFFiNE (MIT) and Chroma (Apache-2.0) are genuinely OSS — and AFFiNE has the wrong format, Chroma is the wrong category.
